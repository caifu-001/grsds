...
// === 安全管理 API 代理 (P0-2 修复) ===
// 所有需 service_role 的操作统一走 Supabase Edge Function
async function callAdmin(op,table,opts){
  opts=opts||{};
  try{
    var bodyObj={op:op,table:table,method:opts.method||'GET',payload:opts.payload||null,query:opts.query||'*',filters:opts.filters||null};
    // 透传额外参数（如 auth 操作的 userId/password）
    if(opts.userId!==undefined)bodyObj.userId=opts.userId;
    if(opts.password!==undefined)bodyObj.password=opts.password;
    var body=JSON.stringify(bodyObj);
    var r=await fetch(SUPAFUNC_BASE,{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+(await sb.auth.getSession()).data.session?.access_token||''},body:body});
    if(!r.ok){var txt=await r.text();console.error('callAdmin failed:',r.status,txt);return{data:null,error:{message:'Admin API error: HTTP ' + r.status + ' - ' + (txt||'empty response')}}};
    return await r.json();
  }catch(e){console.error('callAdmin error:',e);return{data:null,error:{message:e.message||'Network error'}};}
}
window.addEventListener('error',function(e){console.error('[global err]',e.message,e.filename,e.lineno)});
...
let clientTags=[],clientAttachments=[],leadEditId=null,leadAssignId=null,allLeads=[],leadFormSource='pool';
...
// === Auth ===
var loginFailCount=0,loginLockUntil=null;
function switchMode(){
...
function checkPwStrength(pw){
  if(!pw||pw.length<8)return{ok:false,level:0,msg:'密码至少8位'};
  var hasLetter=/[a-zA-Z]/.test(pw),hasDigit=/[0-9]/.test(pw),hasSpecial=/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(pw);
  if(!hasLetter)return{ok:false,level:1,msg:'需包含字母'};
  if(!hasDigit)return{ok:false,level:1,msg:'需包含数字'};
  if(!hasSpecial)return{ok:false,level:2,msg:'需包含特殊字符（如!@#$%等）'};
  if(pw.length>=12)return{ok:true,level:3,msg:'密码强度：强'};
...
function trErr(msg){
  ...
  if(msg.indexOf('locked')>=0||msg.indexOf('security')>=0||msg.indexOf('rate')>=0||msg.indexOf('many requests')>=0||msg.indexOf('too many')>=0)return'密码错误次数过多，账号已临时锁定。请联系超级管理员或公司管理员重置密码';
  return msg;
}
var captchaCode='';
function refreshCaptcha(){
  ...
  for(var i=0;i<40;i++){ctx.fillStyle='rgba('+Math.floor(Math.random()*180)+','+Math.floor(Math.random()*180)+','+Math.floor(Math.random()*220)+',0.25)';ctx.fillRect(Math.random()*canvas.width,Math.random()*canvas.height,3,3)}
  for(var i=0;i<4;i++){ctx.strokeStyle='rgba(100,120,200,0.25)';ctx.beginPath();ctx.moveTo(Math.random()*canvas.width,Math.random()*canvas.height);ctx.lineTo(Math.random()*canvas.width,Math.random()*canvas.height);ctx.stroke()}
  for(var i=0;i<captchaCode.length;i++){ctx.font='bold '+(22+Math.random()*8)+'px "Segoe UI","Microsoft YaHei",Arial';ctx.fillStyle='rgb('+Math.floor(Math.random()*140)+','+Math.floor(Math.random()*90)+','+Math.floor(Math.random()*200)+')';var x=8+i*30+Math.random()*8;var y=30+Math.random()*8;ctx.save();ctx.translate(x,y);ctx.rotate((Math.random()-0.5)*0.45);ctx.fillText(captchaCode[i],0,0);ctx.restore()}
}
async function handleLogin(){
  var er=document.getElementById('login-err'),btn=document.getElementById('login-btn');
  // 登录频率限制 (P2-10 修复)
  if(loginLockUntil&&Date.now()<loginLockUntil){var sec=Math.ceil((loginLockUntil-Date.now())/1000);er.textContent='登录过于频繁，请'+sec+'秒后重试';return}
  if(loginFailCount>=5){loginLockUntil=Date.now()+60000;loginFailCount=0;er.textContent='密码错误次数过多，请1分钟后重试';return}
  var captchaInput=document.getElementById('login-captcha').value.trim().toUpperCase();
  er.textContent='';
  if(captchaInput!==captchaCode){er.textContent='验证码错误，请重新输入';refreshCaptcha();document.getElementById('login-captcha').value='';document.getElementById('login-captcha').focus();return}
  ...
  try{
    if(loginMode==='login'){
      ...
      if(r.error){loginFailCount++;er.textContent=trErr(r.error.message);return}
      loginFailCount=0;await onLoginSuccess(r.data.user);
    }else{
...
async function onLoginSuccess(user){
  ...
  // Upsert profile
  var profile=null;
  try{var r2=await sb.from('profiles').upsert({user_id:user.id,email:user.email},{onConflict:'user_id'}).select('*').single();profile=r2.data}catch(e){console.error('profile upsert failed:',e);throw new Error('无法加载用户档案: '+(e.message||JSON.stringify(e)))}
  try{sb.from('profiles').update({last_login_at:new Date().toISOString()}).eq('user_id',user.id).then(function(){}).catch(function(){})}catch(e){}
  ...
  // 获取所有公司成员关系
  var memberships=[];
  console.log('profile after upsert:',{role:profile&&profile.role,company_id:profile&&profile.company_id,active_company_id:profile&&profile.active_company_id});
  try{
    var memResp=await sb.rpc('get_my_memberships');
    if(!memResp.error&&memResp.data)memberships=memResp.data;
  }catch(e){console.error('get_my_memberships error:',e)}

  // 兼容旧数据：如果 memberships 为空但有 profile.company_id
  if(memberships.length===0&&profile&&profile.company_id){
    memberships.push({company_id:profile.company_id,company_name:'默认公司',role:profile.role==='super_admin'?'admin':(profile.role||'member'),status:'active'});
  }

  allMyCompanies=memberships;
  console.log('allMyCompanies set to:',JSON.parse(JSON.stringify(allMyCompanies)));
  ...
  if(memberships.length===0){
    ...
    if(pend&&pend.length>0&&pend[0].status==='pending'){showPendingScreen(pend[0].name);return}
    // 检查邀请
    var {data:invites}=await sb.from('invitations').select('*').eq('to_email',user.email).eq('status','pending');
    if(invites&&invites.length>0){showMyInvitations(invites);return}
    showCompanyRegScreen();return;
  }

  if(memberships.length===1){
    // 只有一家公司，直接激活
    currentCompanyId=memberships[0].company_id;
    currentCompanyRole=memberships[0].role||'member';
    currentCompanyName=memberships[0].company_name||'';
  }else{
    // 多家公司，尝试从 profiles.active_company_id 恢复
    var activeCid=profile&&profile.active_company_id;
    var found=memberships.find(function(m){return m.company_id===activeCid});
    if(found){
      currentCompanyId=found.company_id;
      currentCompanyRole=found.role||'member';
      currentCompanyName=found.company_name||'';
    }else{
      // 弹公司选择器
      showCompanySelector(memberships);
      return;
    }
  }

  // 更新 profiles.active_company_id
  try{await sb.from('profiles').update({active_company_id:currentCompanyId,role:isSuperAdmin?'super_admin':currentCompanyRole}).eq('user_id',user.id)}catch(e){}
  // Load departments and set currentUser.department for permission checks
  if(currentCompanyId){
    var {data:depts}=await sb.from('departments').select('*').eq('company_id',currentCompanyId);
    if(depts&&depts.length){
      allDepartments=depts;
      if(profile&&profile.department_id){
        var myDept=depts.find(function(d){return d.id===profile.department_id});
        if(myDept)currentUser.department=myDept.name;
      }
    }
  }
  // Sync currentUserRole for code that still uses the old variable
  currentUserRole=currentCompanyRole;
  document.getElementById('main-screen').classList.remove('hidden');
  updateCompanyUIBadge();
  if(isSuperAdmin||currentCompanyRole==='admin')document.getElementById('tab-admin').classList.remove('hidden');
  ...
  pollNotifications();
  writeOpLog('login','auth',currentUser.id,currentUser.email,'用户登录');
  switchTab('analytics');
}

// === 多公司：切换激活公司 ===
async function switchActiveCompany(companyId){
  var mem=allMyCompanies.find(function(m){return m.company_id===companyId});
  if(!mem)return;
  try{await sb.rpc('set_active_company',{p_company_id:companyId})}catch(e){console.error('set_active_company failed:',e)}
  currentCompanyId=companyId;
  currentCompanyRole=mem.role||'member';
...
function updateCompanyUIBadge(){
  var badge=document.getElementById('current-company-badge');
  if(badge){
    badge.textContent=currentCompanyName||'未选择公司';
    badge.title='点击切换公司';
  }
}

// === 多公司：公司选择器 ===
function showCompanySelector(memberships){
  console.log('showCompanySelector incoming:',JSON.parse(JSON.stringify(memberships)));
  ...
  var html='<div class="modal" style="background:var(--card);border-radius:12px;padding:24px;max-width:420px;width:90%;max-height:80vh;overflow-y:auto">';
  html+='<h3 style="margin:0 0 16px">选择公司</h3>';
  html+='<p style="color:var(--muted);font-size:14px;margin:0 0 16px">你在以下公司有职位，请选择进入哪家公司：</p>';
  for(var i=0;i<memberships.length;i++){
    var m=memberships[i];
    var roleLabel=m.role==='admin'?'管理员':m.role==='editor'?'编辑':'成员';
    html+='<div onclick="closeModal();switchActiveCompany('+m.company_id+')" style="cursor:pointer;padding:12px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px;transition:all 0.2s" onmouseenter="this.style.borderColor=&#39;var(--primary)&#39;;this.style.background=&#39;var(--bg)&#39;" onmouseleave="this.style.borderColor=&#39;var(--border)&#39;;this.style.background=&#39;transparent&#39;">';
    html+='<div style="font-weight:600">'+escHtml(m.company_name||'未命名公司')+'</div>';
    html+='<div style="font-size:13px;color:var(--muted)">'+roleLabel+'</div>';
    html+='</div>';
  }
  html+='<button class="btn-sm btn-sm-primary" onclick="closeModal();showCompanyRegScreen()" style="width:100%;margin-bottom:8px">＋ 注册新公司</button>';
  html+='<button class="btn-sm" onclick="closeModal()" style="width:100%">取消</button>';
  html+='</div>';
...
function closeModal(){
  ...
  for(var i=overlays.length-1;i>=0;i--){
    var ov=overlays[i];
    if(ov.style.zIndex&&parseInt(ov.style.zIndex)>=20000){ov.remove();break}
  }
  // Clean up temp overlays
  var allOv=document.querySelectorAll('.modal-overlay');
  for(var j=allOv.length-1;j>=0;j--){
    var o=allOv[j];
    if(parseInt(o.style.zIndex||'0')>=20000)o.remove();
  }
}

// === 多公司：我的邀请列表 ===
function showMyInvitations(invitations){
  var overlay=document.createElement('div');
  overlay.className='modal-overlay';
  overlay.style.cssText='position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.5);z-index:20000;display:flex;align-items:center;justify-content:center';
  var html='<div class="modal" style="background:var(--card);border-radius:12px;padding:24px;max-width:450px;width:90%;max-height:80vh;overflow-y:auto">';
  html+='<h3 style="margin:0 0 16px">入职邀请</h3>';
  html+='<p style="color:var(--muted);font-size:14px;margin:0 0 16px">以下公司邀请你加入：</p>';
  for(var i=0;i<invitations.length;i++){
    var inv=invitations[i];
    html+='<div style="padding:12px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">';
    html+='<div style="font-weight:600;margin-bottom:6px">公司ID: '+inv.from_company_id+'</div>';
    html+='<div style="display:flex;gap:8px">';
    html+='<button class="btn-sm btn-sm-primary" onclick="handleInvite('+inv.id+',&quot;accept&quot;)">接受</button>';
    html+='<button class="btn-sm" onclick="handleInvite('+inv.id+',&quot;reject&quot;)">拒绝</button>';
    html+='</div></div>';
  }
  html+='<button class="btn-sm" onclick="closeModal()" style="width:100%;margin-top:8px">关闭</button>';
  html+='</div>';
  overlay.innerHTML=html;
  overlay.onclick=function(e){if(e.target===overlay)closeModal()};
  document.body.appendChild(overlay);
...
async function handleInvite(invitationId,action){
  try{
    var r=await sb.rpc('handle_invitation',{p_invitation_id:invitationId,p_action:action});
    if(r.error){showToast(r.error.message||'操作失败');return}
    closeModal();
    if(action==='accept'){
      showToast('已加入公司！正在刷新...');
      // Reload memberships and activate
      var memResp=await sb.rpc('get_my_memberships');
      if(!memResp.error&&memResp.data){
        allMyCompanies=memResp.data;
        closeModal();
        location.reload();
      }
    }else{
  ...
  }catch(e){showToast('操作失败: '+(e.message||e.toString()))}
}

// === 管理面板：成员管理 ===
async function loadCompanyMembersForAdmin(){
  var list=document.getElementById('member-list');
  if(!list)return;
  list.innerHTML='<div style="text-align:center;color:var(--muted);padding:20px">加载中...</div>';
  try{
    var r=await sb.rpc('list_company_members',{p_company_id:currentCompanyId});
    if(r.error){list.innerHTML='<div class="empty-state">加载失败: '+r.error.message+'</div>';return}
    var members=r.data||[];
    if(members.length===0){list.innerHTML='<div class="empty-state">暂无成员</div>';return}
    var html='';
    for(var i=0;i<members.length;i++){
      var m=members[i];
      var roleLabel=m.role==='admin'?'管理员':m.role==='editor'?'编辑':'成员';
      var canManage=currentCompanyRole==='admin'&&m.user_id!==currentUser.id;
      ...
      html+='<div><div style="font-weight:600">'+escHtml(m.display_name||m.email||'未命名')+'</div><div style="font-size:12px;color:var(--muted)">'+escHtml(m.email||'')+(m.department_name?' · '+escHtml(m.department_name):'')+' · '+roleLabel+'</div></div>';
      if(canManage){
        html+='<div style="display:flex;gap:6px">';
        if(m.role!=='admin')html+='<button class="btn-sm" onclick="promoteMember(&quot;'+m.user_id+'&quot;)" title="提升为管理员">设管理</button>';
        html+='<button class="btn-sm" style="color:var(--danger)" onclick="removeMemberFromCompany(&quot;'+m.user_id+'&quot;)">移除</button>';
        html+='</div>';
      }
      html+='</div>';
    }
    list.innerHTML=html;
  }catch(e){list.innerHTML='<div class="empty-state">加载失败</div>';console.error(e)}
}

async function promoteMember(userId){
  if(!confirm('确认提升该成员为管理员？'))return;
  try{
    var r=await sb.from('company_memberships').update({role:'admin'}).eq('user_id',userId).eq('company_id',currentCompanyId);
    if(r.error){showToast(r.error.message);return}
    showToast('已提升为管理员');
    loadCompanyMembersForAdmin();
  }catch(e){showToast('操作失败: '+(e.message||e.toString()))}
}

async function removeMemberFromCompany(userId){
  if(!confirm('确认移除该成员？该成员将失去对该公司数据的访问权限。'))return;
  try{
    var r=await sb.rpc('remove_member',{p_company_id:currentCompanyId,p_user_id:userId});
    if(r.error){showToast(r.error.message);return}
    showToast('已移除');
    loadCompanyMembersForAdmin();
  }catch(e){showToast('操作失败: '+(e.message||e.toString()))}
...
function toggleMemberPanel(){
  ...
  if(hidden){panel.classList.remove('hidden');loadCompanyMembersForAdmin()}else{panel.classList.add('hidden')}
}

async function inviteMemberToCompany(){
  var email=document.getElementById('invite-email-input');
  var role=document.getElementById('invite-role-select');
  if(!email||!email.value.trim()){showToast('请输入邮箱');return}
  try{
    var r=await sb.rpc('invite_member',{p_company_id:currentCompanyId,p_email:email.value.trim(),p_role:role?role.value:'member'});
    if(r.error){showToast(r.error.message);return}
    showToast('邀请已发送！');
    if(email)email.value='';
  }catch(e){showToast('邀请失败: '+(e.message||e.toString()))}
}

// === 多公司：currentUserRole 是 profiles.role（超管保持 super_admin）
// currentCompanyRole 是当前企业内的角色（admin/editor/member）
// 不再用代理覆盖 currentUserRole，改为分别判断
...
function openSettings(){
  ...
  ss.style.display='block';ss.style.position='fixed';ss.style.top='0';ss.style.left='0';ss.style.width='100%';ss.style.height='100vh';ss.style.zIndex='10000';ss.style.overflow='auto';ss.style.background='var(--bg)';
  renderCustomTypes();
}
...
function closeSettings(){
  var ss=document.getElementById('settings-screen'),mn=document.getElementById('main-screen');
  ss.style.display='none';ss.style.position='';ss.style.top='';ss.style.left='';ss.style.width='';ss.style.height='';ss.style.zIndex='';ss.style.overflow='';ss.style.background='';
  mn.classList.remove('hidden');mn.appendChild(ss);
}

async function openMy(){
  document.getElementById('settings-screen').style.display='none';
  var ms=document.getElementById('my-screen');
  document.body.appendChild(ms);
  ms.style.display='block';ms.style.position='fixed';ms.style.top='0';ms.style.left='0';ms.style.width='100%';ms.style.height='100vh';ms.style.zIndex='10000';ms.style.overflow='auto';ms.style.background='var(--bg)';
  // Load full profile
  ...
  // Resignation
  var rs=profile.resignation_status;
  var btn=document.getElementById('my-resign-btn');
  var row=document.getElementById('my-resign-row');
  var warn=document.getElementById('my-resign-warn');
  if(rs){row.style.display='flex';document.getElementById('my-resign-status').textContent={'pending':'待审批','approved':'已通过','rejected':'已拒绝'}[rs]||rs;if(rs==='pending'||rs==='approved')btn.style.display='none';else if(currentCompanyId)btn.style.display='block'}else{if(currentCompanyId)btn.style.display='block';else btn.style.display='none';row.style.display='none'}
  if(rs==='approved'&&profile.resignation_date){warn.style.display='block';document.getElementById('my-resign-warn-date').textContent='离职日期: '+profile.resignation_date}else{warn.style.display='none'}
  // Load invitations
  var invSection=document.getElementById('my-invitation-section');
  if(!currentCompanyId){await loadMyInvitations();if(myInvitations.length>0){invSection.style.display='block';renderMyInvitations()}else{invSection.style.display='none'}}else{invSection.style.display='none'}
  enableMyEditing();
}

function closeMy(){
  var ms=document.getElementById('my-screen'),mn=document.getElementById('main-screen');
  ms.style.display='none';ms.style.position='';ms.style.top='';ms.style.left='';ms.style.width='';ms.style.height='';ms.style.zIndex='';ms.style.overflow='';ms.style.background='';
  mn.classList.remove('hidden');mn.appendChild(ms);
}
...
async function loadClients(){
  ...
  statsBar.innerHTML='';
  try{
    var clientsData,clientsError;
    // 非管理员用 callAdmin 绕过 RLS，再前端 canAccess 过滤
    if(currentCompanyRole!=='admin'&&!isSuperAdmin){
      var r=await callAdmin('select','clients',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'updated_at.desc'},{col:'',op:'limit',val:'50000'}]});
      clientsData=r&&r.data||[];clientsError=r&&r.error;
      console.log('[loadClients] callAdmin for member, data:',(clientsData||[]).length,'error:',clientsError);
    }else{
      var res=await sb.from('clients').select('*').eq('company_id',currentCompanyId).order('updated_at',{ascending:false});
      clientsData=res.data;clientsError=res.error;
    }
    if(clientsError){grid.innerHTML='<div class="empty"><div class="empty-icon">⚠️</div>加载失败<br><span style="font-size:12px;color:var(--text3)">'+h(clientsError.message)+'</span></div>';statsBar.innerHTML='';return}
    allClients=(clientsData||[]);
    console.log('[loadClients] fetched '+allClients.length+' clients, currentCompanyRole='+currentCompanyRole+' isSuperAdmin='+isSuperAdmin);
    if(currentCompanyRole!=='admin'&&!isSuperAdmin){
      console.log('[loadClients] filtering by grants, memberGrants.clients=',JSON.stringify(memberGrants.clients));
      if(allClients.length>0)console.log('[loadClients] sample client id:',JSON.stringify(allClients[0].id),' grant sample:',JSON.stringify(memberGrants.clients&&memberGrants.clients[0]));
      allClients=allClients.filter(function(cl){var ok=canAccess('clients',cl.id);if(!ok)console.log('[loadClients] DENY client '+cl.id+' not in grants (case check: '+JSON.stringify(memberGrants.clients)+')');return ok});
      console.log('[loadClients] after filter: '+allClients.length+' clients');
    }
...
async function loadCompanies(){
  try{
    var {data}=await sb.from('companies').select('*',{count:'exact'}).order('name').limit(50000);
    allCompanies=(data||[]);
    allCompaniesMap={};
...
function showCompanies(){
  var ds=document.getElementById('dir-screen');
  document.body.appendChild(ds);
  ds.classList.remove('hidden');ds.style.position='fixed';ds.style.top='0';ds.style.left='0';ds.style.width='100%';ds.style.height='100vh';ds.style.zIndex='10000';ds.style.overflow='auto';ds.style.background='var(--bg)';
  renderDir();
}
...
async function showLeads(){
  ...
  var hv=document.getElementById('home-view');if(hv)hv.classList.add('hidden');
...
function closeDir(){
  var ds=document.getElementById('dir-screen');
  ds.classList.add('hidden');ds.style.position='';ds.style.top='';ds.style.left='';ds.style.width='';ds.style.height='';ds.style.zIndex='';ds.style.overflow='';ds.style.background='';
  document.getElementById('main-screen').appendChild(ds);
}
function renderDir(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    html+='<div class="dir-item" onclick="showCompanyDetail(\''+h(c.name||'').replace(/'/g,"\\'")+'\')"><div class="di-name">'+h(c.name)+'</div>'+(rows?'<div class="di-row">'+rows+'</div>':'')+(c.base?'<span class="di-badge">'+h(c.base)+'</span>':'')+'</div>';
  }
  el.innerHTML=html;
}
function showCompanyDetail(name){
  var key=name.toLowerCase();
  var comp=allCompaniesMap[key];
  if(!comp){showToast('未找到公司信息');return}
  // 在名录内弹窗展示详情
  var rows='';
  if(comp.legal_person)rows+='<tr><td class="di-l">法人</td><td>'+h(comp.legal_person)+'</td></tr>';
  if(comp.credit_code)rows+='<tr><td class="di-l">信用代码</td><td>'+h(comp.credit_code)+'</td></tr>';
  if(comp.reg_capital)rows+='<tr><td class="di-l">注册资金</td><td>'+h(comp.reg_capital)+'</td></tr>';
  if(comp.reg_status)rows+='<tr><td class="di-l">状态</td><td>'+h(comp.reg_status)+'</td></tr>';
  if(comp.established)rows+='<tr><td class="di-l">成立日期</td><td>'+h(comp.established)+'</td></tr>';
  if(comp.address)rows+='<tr><td class="di-l">地址</td><td>'+h(comp.address)+'</td></tr>';
  if(comp.business_scope)rows+='<tr><td class="di-l">经营范围</td><td style="max-width:300px">'+h(comp.business_scope).substring(0,200)+'</td></tr>';
  var modal=document.createElement('div');modal.className='confirm-overlay';modal.style.zIndex='10100';
  modal.innerHTML='<div class="confirm-box" style="max-width:480px;max-height:80vh;overflow-y:auto"><h3 style="margin:0 0 12px">'+h(name)+'</h3>'
    +'<table style="width:100%;font-size:13px;border-collapse:collapse"><tbody>'+rows+'</tbody></table>'
    +'<div class="btns" style="margin-top:16px"><button class="btn-cancel" onclick="this.closest(\'.confirm-overlay\').remove()">关闭</button>'
    +'<button class="btn-confirm" onclick="this.closest(\'.confirm-overlay\').remove();addCompanyAsClient(\''+h(name).replace(/'/g,"\\'")+'\')">➕ 添加为客户</button></div></div>';
  document.body.appendChild(modal);
}
function addCompanyAsClient(name){
  closeDir();
  document.getElementById('search-input').value=name;
  renderList();
  var exists=allClients.find(function(c){return c.name.toLowerCase()===name.toLowerCase()});
  if(exists){openForm(exists.id);showToast('该客户已存在');return}
  // 先打开空白表单，再回填（否则 openForm 会清空所有字段）
  var fab=document.getElementById('main-fab');if(fab)fab.click();
...
function fillCompanyInfo(comp){
  ...
  if(comp.address){
    ...
    if(addr&&!addr.value.trim())addr.value=comp.address;
  }
...
async function openForm(id){
  ...
  document.getElementById('f-address').value='';formProjects=[];document.getElementById('pm-project-list').innerHTML='';
...
function searchMoveTarget(){
  ...
  var excludeId=editId||c360ClientId;
  var list=excludeId?allClients.filter(function(c){return c.id!==excludeId}):allClients;
  if(q) list=list.filter(function(c){return(c.name||'').toLowerCase().indexOf(q)>=0});
...
async function doMove(targetId,targetName){
  ...
  if(!co)return;
  if(!confirm('确认将联系人「'+(co.name||co.contact_name||'')+'」转移到「'+targetName+'」？')) return;
  if(!co._new&&co.id){
    var {error}=await sb.from('contacts').update({client_id:targetId}).eq('id',co.id);
    if(error){showToast('转移失败');return}
  }
  // Remove from current form if editing
  if(moveContactIdx>=0){formContacts.splice(moveContactIdx,1);refCt()}
  ...
  if(c360ClientId&&moveContactIdx<0){setTimeout(function(){if(c360ClientId)openClient360(c360ClientId)},500)}
}
function openMoveFrom360(contactObj){moveContactIdx=-1;moveContactObj=contactObj;document.getElementById('move-modal').classList.remove('hidden');searchMoveTarget()}
function transferFrom360(id){var co=allContacts.find(function(x){return x.id===id});if(co)openMoveFrom360(co);else showToast('联系人未找到')}
...
async function saveClient(){
  ...
  if(!editId){
    ...
    if(currentCompanyRole!=='admin'&&!isSuperAdmin){
      var grRes=await callAdmin('insert','resource_grants',{payload:{user_id:currentUser.id,company_id:currentCompanyId,resource_type:'clients',resource_id:clientId,granted_by:currentUser.id}});
...
function loadHomeDashboard(){
  ...
  sb.from('contracts').select('id,title,total_amount,end_date').eq('company_id',currentCompanyId).not('archive_status','eq','archived').order('end_date',{ascending:true}).limit(5).then(function(r){
    ...
    for(var i=0;i<r.data.length;i++){
      ...
      hasReminder=true;
      var d=new Date(c.end_date);var isOver=c.end_date<today;
      h+='<div class="reminder-item'+(isOver?' reminder-overdue':'')+'"><span class="reminder-icon">'+(isOver?'🔴':'🟡')+'</span><div class="reminder-info"><span class="reminder-title">'+escHtml(c.title||'合同#'+c.id)+'</span><span class="reminder-detail">'+(c.total_amount?' · ¥'+Number(c.total_amount).toLocaleString('zh-CN'):'')+'</span></div><span class="reminder-date">'+d.toLocaleDateString('zh-CN')+'</span></div>';
    }
    h+='</div>';
...
function loadReportSummary(){
  ...
  sb.from('contracts').select('total_amount').eq('company_id',currentCompanyId).not('archive_status','eq','archived').then(function(r){
    var total=0,paid=0;
    if(r.data)for(var i=0;i<r.data.length;i++){total+=parseFloat(r.data[i].total_amount)||0;}
...
async function renderRegionalPerf(year){
  try{
    var yStart=new Date(year,0,1).toISOString();
    var yEnd=new Date(year,11,31,23,59,59).toISOString();
    var r=await _reportQuery('clients').select('industry').gte('created_at',yStart).lte('created_at',yEnd);
    var regionMap={};
    if(r.data)for(var i=0;i<r.data.length;i++){var p=r.data[i].industry||'未知';regionMap[p]=(regionMap[p]||0)+1;}
...
async function renderARAging(year){
  try{
    ...
    var yEnd=new Date(year,11,31,23,59,59).toISOString();
    var now=new Date();
    var r=await _reportQuery('contracts').select('total_amount,end_date').not('archive_status','eq','archived').gte('created_at',yStart).lte('created_at',yEnd);
    // 按账龄分组：未到期、1-30天、31-60天、61-90天、90天+
    var aging={unexpired:0,days30:0,days60:0,days90:0,over90:0};
    if(r.data)for(var i=0;i<r.data.length;i++){
      var unpaid=parseFloat(r.data[i].total_amount)||0;
      var due=r.data[i].end_date?new Date(r.data[i].end_date):now;
...
async function renderContractStats(sel){
  try{
    ...
    for(var mi=0;mi<sel.months.length;mi++){
      var mo=sel.months[mi];
      var r=await _reportQuery('contracts').select('total_amount').gte('created_at',mo.start).lte('created_at',mo.end);
      var total=0,paid=0;
      if(r.data)for(var i=0;i<r.data.length;i++){total+=parseFloat(r.data[i].total_amount)||0;}
      labels.push(mo.shortLabel);
...
async function renderBadDebt(){
  try{
    var now=new Date();
    var r=await _reportQuery('contracts').select('id,total_amount,end_date').not('archive_status','eq','archived').not('status','eq','completed');
    ...
    for(var i=0;i<r.data.length;i++){
      var c=r.data[i];
      var unpaid=parseFloat(c.total_amount)||0;
      var due=c.end_date?new Date(c.end_date):null;
      if(!due)continue;
      var overdue=Math.round((now-due)/(1000*60*60*24));
      if(overdue<=0)continue;
      badList.push({id:c.id,total:parseFloat(c.total_amount)||0,paid:0,unpaid:unpaid,dueDate:due.toLocaleDateString('zh-CN'),overdueDays:overdue});
    }
...
function addDashboardCard(){
  ...
  if(isNaN(idx)||idx<0||idx>=cardTypes.length)return;
...
async function renderDashboardCard(card,canvas,idx){
  ...
  try{
    ...
    if(card.type==='salesFunnel'){
      ...
      var rr=await _reportQuery('clients').select('industry');
      var rmap={};if(rr.data)for(var rk=0;rk<rr.data.length;rk++){var pp=rr.data[rk].industry||'未知';rmap[pp]=(rmap[pp]||0)+1;}
      ...
      for(var cm=1;cm<=12;cm++){
        ...
        var crr=await _reportQuery('contracts').select('total_amount').gte('created_at',cms).lte('created_at',cme);
        var ctotal=0,cpaid=0;if(crr.data)for(var cj=0;cj<crr.data.length;cj++){ctotal+=parseFloat(crr.data[cj].total_amount)||0;}
...
function switchNotifTab(sub){
  ...
  document.getElementById('invitation-list').classList.add('hidden');
  if(sub==='notifications'){tabs[0].classList.add('active');loadNotifs();document.getElementById('notif-list').classList.remove('hidden')}
  else if(sub==='invitations'){tabs[2].classList.add('active');loadMyInvitations();document.getElementById('invitation-list').classList.remove('hidden')}
...
async function loadNotifs(){
  ...
  if(!currentUser)return;
  var d1=Date.now();
  var {data:notifs,error}=await sb.from('notifications').select('*').eq('user_id',currentUser.id).order('created_at',{ascending:false}).limit(50);
  console.log('loadNotifs:',Date.now()-d1+'ms, rows:',(notifs||[]).length,'err:',error);
  var unread=0;
...
async function openNotif(id,link){
  ...
  loadNotifs();
  toggleNotifPanel();
  if(link==='invitations'){switchNotifTab('invitations')}
  else if(link){var parts=link.split('/');if(parts[0])switchTab(parts[0])}
}
...
async function loadMyInvitations(){
  var el=document.getElementById('invitation-list');
  el.innerHTML='<div class="empty-state">加载中...</div>';
  if(!currentUser)return;
  console.log('loadMyInvitations: email=',currentUser.email);
  var resp=await sb.from('invitations').select('*').eq('to_email',currentUser.email).eq('status','pending').order('created_at',{ascending:false});
  console.log('invitations resp:',resp);
  var invites=resp.data;
  if(invites&&invites.length){
    // 确保公司名录已加载
    if(!allCompanies||!allCompanies.length)await loadCompanies();
    var companyMap={};if(allCompanies)for(var k=0;k<allCompanies.length;k++)companyMap[allCompanies[k].id]=allCompanies[k];
    var h='';
    for(var i=0;i<invites.length;i++){
      var inv=invites[i];
      var c=companyMap[inv.from_company_id];
      var cname=c?c.name:('公司#'+inv.from_company_id);
      var d=new Date(inv.created_at);var t=d.toLocaleDateString('zh-CN')+' '+d.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'});
      h+='<div style="padding:14px;border:1px solid var(--border);border-radius:10px;margin-bottom:10px;background:#f8fafc">';
      h+='<div style="font-weight:600;margin-bottom:4px">'+escHtml(cname)+'</div>';
      h+='<div style="font-size:13px;color:var(--text2);margin-bottom:8px">邀请你加入 · '+t+'</div>';
      h+='<div style="display:flex;gap:8px">';
      h+='<button class="btn-sm btn-sm-primary" onclick="acceptInvitation('+inv.id+')">✓ 接受</button>';
      h+='<button class="btn-sm" onclick="rejectInvitation('+inv.id+')" style="color:#ef4444">✗ 拒绝</button>';
      h+='</div></div>';
    }
    el.innerHTML=h;
  }else{el.innerHTML='<div class="empty-state">暂无邀请</div>'}
}
...
async function acceptInvitation(id){
  try{
    ...
    // get invitation
    var invs=await callAdmin('select','invitations',{query:'*',filters:[{col:'id',op:'eq',val:id}]});
    var inv=(invs&&invs.data&&invs.data.length>0)?invs.data[0]:null;
    console.log('inv found:',inv);
    if(!inv){showToast('邀请不存在');return}
    // check already member
    var mems=await callAdmin('select','company_memberships',{query:'*',filters:[{col:'user_id',op:'eq',val:currentUser.id},{col:'company_id',op:'eq',val:inv.from_company_id}]});
    if(mems&&mems.data&&mems.data.length>0&&mems.data[0].status==='active'){
      await callAdmin('update','invitations',{filters:[{col:'id',op:'eq',val:id}],payload:{status:'accepted'}});
      showToast('已是该公司成员，刷新中...');setTimeout(function(){location.reload()},800);return;
    }
    // insert membership
    var mr=await callAdmin('insert','company_memberships',{payload:{user_id:currentUser.id,company_id:inv.from_company_id,role:inv.role||'member',status:'active'}});
    console.log('membership insert:',mr);
    if(mr&&mr.error&&mr.error.code!=='23505'){showToast('加入失败: '+(mr.error.message||''));return}
    // update profiles
    await callAdmin('update','profiles',{filters:[{col:'user_id',op:'eq',val:currentUser.id}],payload:{company_id:inv.from_company_id}});
    // mark accepted
    await callAdmin('update','invitations',{filters:[{col:'id',op:'eq',val:id}],payload:{status:'accepted'}});
    showToast('已加入公司！刷新中...');
    setTimeout(function(){location.reload()},800);
  }catch(e){console.error('acceptInvitation error:',e);showToast('操作失败: '+(e.message||e.toString()))}
}

async function rejectInvitation(id){
  try{
    await callAdmin('update','invitations',{filters:[{col:'id',op:'eq',val:id},{col:'to_email',op:'eq',val:currentUser.email}],payload:{status:'rejected'}});
    showToast('已拒绝');
    loadMyInvitations();
  }catch(e){console.error('rejectInvitation error:',e);showToast('操作失败: '+(e.message||e.toString()))}
}
...
function showToast(msg,type){
  ...
  var dbg3=document.getElementById("fw-debug");if(dbg3){dbg3.innerHTML+="<br>🔔 "+msg;dbg3.style.display="block";}
...
function showProjectsDirect(){
  var pv=document.getElementById('projects-view');
  if(!pv){alert('projects-view NOT FOUND in DOM');return}
  
  // Call switchTab normally first
  switchTab('projects');
  
  // Wait 500ms for render, then do ancestor walk
  setTimeout(function(){
    var pv2=document.getElementById('projects-view');
    if(!pv2){alert('projects-view disappeared!');return}
    
    var result=[];
    result.push('=== projects-view SELF ===');
    var cs=getComputedStyle(pv2);
    result.push('display:'+cs.display+' height:'+cs.height+' overflow:'+cs.overflow+' visibility:'+cs.visibility+' position:'+cs.position);
    result.push('offsetHeight:'+pv2.offsetHeight+' offsetWidth:'+pv2.offsetWidth+' scrollHeight:'+pv2.scrollHeight+' clientHeight:'+pv2.clientHeight);
    result.push('classList:'+pv2.className+' innerHTML length:'+pv2.innerHTML.length);
    result.push('boundingRect:'+JSON.stringify(pv2.getBoundingClientRect()));
    result.push('');
    result.push('=== ANCESTOR WALK ===');
    
    var el=pv2.parentElement;
    var level=1;
    while(el && el!==document.body){
      ...
      var info='L'+level+' <'+tag+' id='+id+'> display:'+st.display+' height:'+st.height+' overflow:'+st.overflow+' visibility:'+st.visibility+' pos:'+st.position+' class:'+cls+' oh:'+el.offsetHeight+' ow:'+el.offsetWidth;
      result.push(info);
      if(st.display==='none'){result.push('  ⚠️ THIS ANCESTOR IS display:none!');}
      if(st.visibility==='hidden'||st.visibility==='collapse'){result.push('  ⚠️ THIS ANCESTOR IS '+st.visibility+'!');}
      if(parseFloat(st.height)===0&&st.overflow==='hidden'){result.push('  ⚠️ THIS ANCESTOR HAS height:0+overflow:hidden!');}
      el=el.parentElement;
      level++;
    }
    ...
    result.push('BODY: display:'+bst.display+' height:'+bst.height);
    
    // Check all views for hidden state
    result.push('');
    result.push('=== ALL VIEWS STATE ===');
    var viewIds=['home-view','client-view','order-view','admin-view','sales-view','inventory-view','suppliers-view','performance-view','reports-view','leads-view','collab-view','service-view','purchase-view','projects-view'];
    for(var i=0;i<viewIds.length;i++){
      var v=document.getElementById(viewIds[i]);
      if(v){
        var vcs=getComputedStyle(v);
        result.push(viewIds[i]+': hidden='+v.classList.contains('hidden')+' display='+vcs.display+' oh='+v.offsetHeight+' ow='+v.offsetWidth);
      }
    }
...
function switchTab(tab){
  ...
  var rv=document.getElementById('reports-view');
  ...
  // Hide all views first
    var anv=document.getElementById('analytics-view');
  var lv=document.getElementById('leads-view');
  var allViews=[cv,ov,rv,av,sv,iv];if(spv)allViews.push(spv);if(pv)allViews.push(pv);if(lv)allViews.push(lv);var colv2=document.getElementById('collab-view');if(colv2)allViews.push(colv2);var sv2=document.getElementById('service-view');if(sv2)allViews.push(sv2);if(purchv)allViews.push(purchv);var pjv=document.getElementById('projects-view');if(pjv)allViews.push(pjv);if(anv)allViews.push(anv);
  var fwv=document.getElementById('fieldwork-view');if(fwv)allViews.push(fwv);
  for(var i=0;i<allViews.length;i++)if(allViews[i]){allViews[i].classList.add('hidden');allViews[i].style.display='';allViews[i].style.visibility='';}
  // Admin sub-panels outside admin-view must also hide
  var ag=document.getElementById('admin-grants');if(ag)ag.classList.add('hidden');
  // Deactivate all tabs
  var tr=document.getElementById('tab-reports');var tan=document.getElementById('tab-analytics');
  var allTabs=[tc,to,document.getElementById('tab-service'),document.getElementById('tab-collab'),document.getElementById('tab-projects'),tr,ta,ti];if(tan)allTabs.push(tan);var tfw=document.getElementById('tab-fieldwork');if(tfw)allTabs.push(tfw);for(var j=0;j<allTabs.length;j++)if(allTabs[j])allTabs[j].classList.remove('active');
  fab.classList.add('hidden');
    ...
    console.log('[DEBUG] switchTab sales, calling switchBizTab');
    ...
    console.log('[DEBUG switchTab] inventory branch, iv hidden:',iv.classList.contains('hidden'));
    iv.classList.remove('hidden');ti.classList.add('active');
    document.getElementById('topbar-title').textContent='供应链';
    console.log('[DEBUG switchTab] inventory-view display:',getComputedStyle(iv).display,'innerHTML len:',iv.innerHTML.length);
    switchResTab('scouting');
  }else if(tab==='suppliers'){
    console.log('[DEBUG switchTab] suppliers branch');
    iv.classList.remove('hidden');ti.classList.add('active');
    document.getElementById('topbar-title').textContent='供应链';
    switchResTab('suppliers');
  }else if(tab==='reports'){
    console.log('[DEBUG switchTab] reports branch, rv hidden:',rv.classList.contains('hidden'));
    rv.classList.remove('hidden');tr.classList.add('active');
    fab.classList.add('hidden');
    document.getElementById('topbar-title').textContent='报表中心';
    console.log('[DEBUG switchTab] reports-view display:',getComputedStyle(rv).display,'innerHTML len:',rv.innerHTML.length);
    loadReportSummary();switchReportsTab('sales');
  }else if(tab==='service'){
    var sv2=document.getElementById('service-view');
    console.log('[DEBUG switchTab] service branch, sv2 found:',!!sv2);
    if(sv2){sv2.classList.remove('hidden');console.log('[DEBUG switchTab] service-view display:',getComputedStyle(sv2).display,'innerHTML len:',sv2.innerHTML.length);}
    switchServiceTab('tickets');return}
  if(tab==='analytics'){
    console.log('[DEBUG switchTab] analytics branch');
    var anv=document.getElementById('analytics-view');
    if(anv){anv.classList.remove('hidden');anv.style.display='';anv.style.visibility='visible';}
    var tan=document.getElementById('tab-analytics');if(tan)tan.classList.add('active');
    document.getElementById('topbar-title').textContent='经营分析';
    fab.classList.add('hidden');
    console.log('[DEBUG switchTab] analytics view shown');
    switchAnalyticsTab('overview');
    return;
  }
  if(tab==='projects'){
    var pjv=document.getElementById('projects-view');
    console.log('[DEBUG switchTab] projects branch, pjv found:',!!pjv);
    pjv.classList.remove('hidden');pjv.style.display='';pjv.style.visibility='visible';
    document.getElementById('tab-projects').classList.add('active');
    document.getElementById('topbar-title').textContent='项目管理';
    fab.classList.add('hidden');
    console.log('[DEBUG switchTab] projects-view display:',getComputedStyle(pjv).display,'innerHTML len:',pjv.innerHTML.length);
    if(typeof switchProjectTab==='function'){switchProjectTab('list')}else{console.error('switchProjectTab not defined')};
    return;
  }
  if(tab==='admin'){
  ...
  console.log('[DEBUG switchTab] collab branch entered');
  var colv=document.getElementById('collab-view');
  console.log('[DEBUG switchTab] collab-view found:',!!colv,'hidden:',colv?colv.classList.contains('hidden'):'N/A');
  ...
  console.log('[DEBUG switchTab] collab-view hidden after remove:',colv.classList.contains('hidden'));
  console.log('[DEBUG switchTab] collab-view display:',getComputedStyle(colv).display);
  console.log('[DEBUG switchTab] collab-view innerHTML length:',colv.innerHTML.length);
  console.log('[DEBUG switchTab] about to call switchCollabTab schedule');
  ...
  console.log("[FW] switchTab fieldwork triggered");
  var dbg2=document.getElementById("fw-debug");if(dbg2){dbg2.style.display="block";dbg2.innerHTML="FW init...<br>▶ switchTab fieldwork triggered";}
  // DEBUG: force visibility
  var fwv2=document.getElementById("fieldwork-view");
  if(fwv2){
    fwv2.style.display="block";
    fwv2.style.visibility="visible";
    fwv2.style.opacity="1";
    fwv2.style.height="auto";
    fwv2.style.overflow="visible";
    console.log("[FW] fieldwork-view offsetWidth:",fwv2.offsetWidth,"offsetHeight:",fwv2.offsetHeight);
    console.log("[FW] fieldwork-view computed display:",getComputedStyle(fwv2).display);
    console.log("[FW] fieldwork-view computed visibility:",getComputedStyle(fwv2).visibility);
  }
  // Check fw-overview
  var fwov=document.getElementById("fw-overview");
  if(fwov){
    console.log("[FW] fw-overview offsetWidth:",fwov.offsetWidth,"offsetHeight:",fwov.offsetHeight);
    console.log("[FW] fw-overview classList:",fwov.className);
  }
  // Check fw-ov-kpi
  var fwok=document.getElementById("fw-ov-kpi");
  if(fwok){
    console.log("[FW] fw-ov-kpi exists, offsetHeight:",fwok.offsetHeight);
  }else{
    console.log("[FW] ⛔ fw-ov-kpi NOT FOUND in DOM!");
  }
...
async function saveOrder(){
  console.log('[saveOrder] start',{orderEditId,orderClientId,orderStage,currentCompanyId});
  ...
  if(!orderEditId){
    ...
    console.log('[saveOrder] inserting order',{cols:Object.keys(order),vals:JSON.stringify(order)});
    var {error}=await sb.from('orders').insert(order);
    if(error){console.error('[saveOrder] insert error',JSON.stringify(error));showToast('创建失败: '+(error.message||JSON.stringify(error)));return}
    showToast('订单已创建');
  }else{
...
function formatMoney(v){
  if(v==null||isNaN(v=parseFloat(v)))return '0';
...
sb.auth.getSession().then(async function(res){
  if(res.data&&res.data.session){
    ...
    // Fire-and-forget: update last_login_at on session restore
    sb.from('profiles').update({last_login_at:new Date().toISOString()}).eq('user_id',currentUser.id).then(function(){}).catch(function(){});
    // Load profile for company/role
    var {data:profile}=await sb.from('profiles').select('role,company_id,active_company_id').eq('user_id',currentUser.id).single();
    if(profile){currentUserRole=profile.role||'user';isSuperAdmin=profile.role==='super_admin';currentCompanyId=profile.active_company_id||profile.company_id;currentCompanyRole=currentUserRole;currentCompanyName='';}
    if(!currentCompanyId){document.getElementById('login-screen').classList.remove('hidden');return}
    // Load multi-company memberships
    try{var {data:mems}=await sb.rpc('get_my_memberships');if(mems&&mems.length){allMyCompanies=mems;var cur=mems.find(function(m){return m.company_id==currentCompanyId});if(cur){currentCompanyRole=cur.role||currentUserRole;currentCompanyName=cur.company_name||'';currentUserRole=currentCompanyRole;}}}catch(e){console.error('get_my_memberships failed:',e)}
    document.getElementById('login-screen').classList.add('hidden');
    document.getElementById('main-screen').classList.remove('hidden');
    switchTab('analytics');
  ...
  if(isSuperAdmin){document.getElementById('admin-tab-companies-manage').classList.remove('hidden');document.getElementById('admin-tab-companies').classList.remove('hidden');}
    ...
    buildTypeGrid();
    buildStageGrid();
    // 先加载授权，再加载数据（否则 canAccess 返回 false 过滤所有资源）
    await loadMemberGrants();
    await loadClients();
    loadCompanies();
    loadNotifs();
...
async function checkPendingCompany(){
  ...
  if(data[0].status==='approved'){
    ...
    await sb.from('profiles').upsert({user_id:user.id,email:user.email,company_id:currentCompanyId,role:'admin',active_company_id:currentCompanyId},{onConflict:'user_id'});
    await sb.from('company_memberships').upsert({user_id:user.id,company_id:currentCompanyId,role:'admin',status:'active',joined_at:new Date().toISOString()},{onConflict:'user_id,company_id'});
    if(window._pendPoll)clearInterval(window._pendPoll);
    await loadCustomTypes();
 buildFilterSelect();
 buildTypeGrid();
...
async function loadAdminCompanies(){
  try{var r=await callAdmin('select','companies',{query:'*',filters:[{col:'',op:'order',val:'name.asc'},{col:'',op:'limit',val:'50000'}]});if(r&&r.data){allAdminCompanies=r.data;}else{console.warn('callAdmin companies no data:',r)}}catch(e){}
}
function renderAdminCompanies(){
  ...
  for(var i=0;i<allAdminCompanies.length;i++){
    ...
    var metaParts=[];
    if(co.tax_id)metaParts.push('税号: '+escHtml(co.tax_id));
    if(co.legal_person)metaParts.push('法人: '+escHtml(co.legal_person));
    if(co.reg_capital)metaParts.push('注册资金: '+escHtml(co.reg_capital));
    if(co.city)metaParts.push('城市: '+escHtml(co.city));
    if(co.address)metaParts.push('地址: '+escHtml(co.address));
    if(co.established)metaParts.push('成立: '+escHtml(co.established));
    metaParts.push('<span class="company-badge '+badge+'">'+label+'</span>');
    h+='<div class="company-row"><div class="company-info"><div class="company-name">'+escHtml(co.name)+'</div><div class="company-meta">'+metaParts.join(' · ')+'</div>'+(co.business_scope?'<div class="company-scope" style="font-size:11px;color:var(--text2);margin-top:2px;max-width:500px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escHtml(co.business_scope)+'</div>':'')+'</div><div class="dn-actions"><button class="btn-sm btn-sm-primary" onclick="openCompanyForm('+co.id+')">编辑</button><button class="btn-sm btn-sm-danger" onclick="delCompanyById('+co.id+')">删除</button></div></div>';
  }
  el.innerHTML=h;
}
...
function openCompanyForm(id){
  ...
  var of=function(k,def){return escHtml(co&&co[k]!=null?co[k]:def||'')};
  div.innerHTML='<div class="confirm-box" style="max-width:520px;max-height:85vh;overflow-y:auto"><p>'+(id?'编辑公司':'新建公司')+'</p>'
    +'<input id="co-name" placeholder="公司名称 *" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('name')+'">'
    +'<input id="co-tax-id" placeholder="统一社会信用代码（18位）" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('tax_id')+'">'
    +'<input id="co-legal-person" placeholder="法人/联系人" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('legal_person')+'">'
    +'<input id="co-reg-capital" placeholder="注册资金（如：1000000）" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('reg_capital')+'">'
    +'<input id="co-established" placeholder="成立日期（如：2020-01-03）" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('established')+'">'
    +'<input id="co-city" placeholder="城市" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('city')+'">'
    +'<input id="co-province" placeholder="省份" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('province')+'">'
    +'<input id="co-address" placeholder="注册地址" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:8px" value="'+of('address')+'">'
    +'<textarea id="co-business-scope" placeholder="经营范围" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:12px;min-height:80px;resize:vertical">'+of('business_scope')+'</textarea>'
    +'<div class="btns"><button class="btn-cancel" onclick="closeCompanyFormModal()">取消</button><button class="btn-confirm" onclick="saveCompany()">保存</button></div></div>';
...
async function saveCompany(){
  ...
  var legal=document.getElementById('co-legal-person').value.trim();
  var capital=document.getElementById('co-reg-capital').value.trim();
  var established=document.getElementById('co-established').value.trim();
  var city=document.getElementById('co-city').value.trim();
  var province=document.getElementById('co-province').value.trim();
  var address=document.getElementById('co-address').value.trim();
  var scope=document.getElementById('co-business-scope').value.trim();
  if(!name){alert('请输入公司名称');return}
  var payload={name:name,tax_id:tax||null,legal_person:legal||null,reg_capital:capital||null,established:established||null,city:city||null,province:province||null,address:address||null,business_scope:scope||null,status:'approved'};
  try{
    if(_companyFormId){
      var r=await callAdmin('update','companies',{payload:payload,filters:[{col:'id',op:'eq',val:_companyFormId}],query:'*'});
      if(r&&r.error){var t=await r.text();alert('更新失败: '+t);return}
    }else{
      var r=await callAdmin('insert','companies',{payload:payload});
      if(r&&r.error){var t=await r.text();alert('创建失败: '+t);return}
    }
...
async function deleteCompany(id,name){
  ...
  try{
    var r=await callAdmin('delete','companies',{filters:[{col:'id',op:'eq',val:id}]});
    if(r&&r.error){var t=await r.text();alert('删除失败: '+t);return}
    await loadAdminCompanies();renderAdminCompanies();
    showToast('公司已删除');
...
async function logout(){
  if(window._pendPoll)clearInterval(window._pendPoll);
  if(currentUser)try{await writeOpLog('logout','auth',currentUser.id,currentUser.email,'用户登出')}catch(e){}
...
async function approveCompany(id){
  ...
  if(comp){
    await sb.from('profiles').upsert({user_id:comp.created_by,company_id:id,role:'admin',active_company_id:id},{onConflict:'user_id'});
    await sb.from('company_memberships').upsert({user_id:comp.created_by,company_id:id,role:'admin',status:'active',joined_at:new Date().toISOString()},{onConflict:'user_id,company_id'});
  }
...
function selectCompany(c){document.getElementById('f-name').value=c.name;var d=document.getElementById('company-ac-drop');if(d)d.classList.remove('show');fillCompanyInfo(c);}
...
async function loadAdminData(force){
  if(adminDataLoaded&&!force){console.log('[loadAdminData] cached, skip');return}
  console.log('[loadAdminData] fetching...');
  // Load departments
  ...
  if(isSuperAdmin||currentUserRole==='admin'){await loadAdminCompanies();}
  if(isSuperAdmin||currentUserRole==='admin'){
    if(isSuperAdmin){
      try{var ur=await callAdmin('select','profiles',{query:'*'});if(ur&&ur.data&&ur.data.length>0){adminUserList=ur.data;allUsers=adminUserList;populatePerfSelects();}else{console.warn('callAdmin returned no data, falling back to direct query:',ur);throw new Error('admin-proxy empty')}}catch(e){console.error('callAdmin profiles failed, fallback direct:',e.message);try{var {data:fb,error:fe}=await sb.from('profiles').select('*');if(!fe&&fb){adminUserList=fb;allUsers=adminUserList;populatePerfSelects();console.log('fallback direct: '+fb.length+' users');}else{console.error('fallback direct also failed:',fe)}}catch(e2){console.error('fallback error:',e2)}}
    }else{
      try{var ur=await callAdmin('select','profiles',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId}]});if(ur&&ur.data){adminUserList=ur.data;allUsers=adminUserList.slice();populatePerfSelects();}else{console.warn('loadAdminData profiles:',ur)}}catch(e){console.error('loadAdminData users:',e)}
    }
  }
...
function renderAdminPerms(){
  ...
  var modNames={clients:'客户',orders:'订单',sales:'销售',inventory:'供应链',admin:'系统',reports:'报表'};
...
function renderAdminRoles(){
  ...
  for(var i=0;i<allRoles.length;i++){
    ...
    if(perms.inventory)labels.push('供应链:'+(permMap[perms.inventory]||perms.inventory));
...
function openRoleForm(id){
  ...
  perms=perms||{};
  var modules=['clients','orders','sales','inventory','admin','reports'];
  var modNames={clients:'客户管理',orders:'订单管理',sales:'销售管理',inventory:'供应链',admin:'系统管理',reports:'报表'};
...
async function switchBizTab(sub){
  ...
  if(sub==='orders'){
    ...
    console.log('[DEBUG] switchBizTab sales branch, allClients.length=',allClients.length);
    tabs[1].classList.add('active');
    sv.classList.remove('hidden');
    fab.classList.remove('hidden');fab.textContent='+';fab.onclick=function(){console.log('[DEBUG] FAB onclick fires');openSalesForm();};
    if(!allClients.length){console.log('[DEBUG] loading clients then calling switchSalesTab');loadClients().then(function(){console.log('[DEBUG] clients loaded, calling switchSalesTab leads');switchSalesTab('leads')});}else{console.log('[DEBUG] clients already cached, direct switchSalesTab');switchSalesTab('leads');}
...
function switchResTab(sub){
  ...
  if(sub==='scouting'){
    ...
    var fab2=document.getElementById('main-fab');if(fab2)fab2.classList.add('hidden');
    loadSuppliers();
...
async function switchAdminTab(tab){
  ...
  dt.classList.add('hidden');rt.classList.add('hidden');if(pt)pt.classList.add('hidden');ct.classList.add('hidden');if(cm)cm.classList.add('hidden');
  ...
  var gr=document.getElementById('admin-grants');if(gr)gr.classList.add('hidden');
  for(var i=0;i<subs.length;i++)subs[i].classList.remove('active');
  
  if(tab==='depts'){dt.classList.remove('hidden');subs[0].classList.add('active');await loadAdminData();renderAdminDepts();}
  else if(tab==='roles'){rt.classList.remove('hidden');subs[1].classList.add('active');await loadAdminData();renderAdminRoles();}
  else if(tab==='perms'){pt.classList.remove('hidden');subs[2].classList.add('active');await loadAdminData();renderAdminPerms();}
  ...
  else if(tab==='employees'){var ep=document.getElementById('admin-employees');ep.classList.remove('hidden');subs[5].classList.add('active');await loadAdminData();renderAdminEmployees();}
  else if(tab==='freeagents'){var fa=document.getElementById('admin-freeagents');fa.classList.remove('hidden');subs[6].classList.add('active');await loadFreeAgents();renderFreeAgents();}
  else if(tab==='resignations'){var ra=document.getElementById('admin-resignations');ra.classList.remove('hidden');subs[7].classList.add('active');await loadResignations();renderResignations();}
  else if(tab==='logs'){var lp=document.getElementById('admin-logs');lp.classList.remove('hidden');subs[8].classList.add('active');await loadOperationLogs();renderOperationLogs();}
  else if(tab==='config'){var cp=document.getElementById('admin-config');cp.classList.remove('hidden');subs[9].classList.add('active');switchConfigTab('fields');}
  else if(tab==='security'){var sp=document.getElementById('admin-security');sp.classList.remove('hidden');subs[10].classList.add('active');renderSecurityPanel();}
  else if(tab==='workflows'){(async function(){var wf=document.getElementById('admin-workflows');wf.classList.remove('hidden');subs[11].classList.add('active');if(!allRoles||!allRoles.length||!allDepartments||!allDepartments.length)await loadAdminData();renderWorkflowTemplates();})();}
  else if(tab==='grants'){var gr=document.getElementById('admin-grants');gr.classList.remove('hidden');subs[12].classList.add('active');renderGrantsPanel();}
}
...
async function saveMemberGrants(){
  ...
  var ids=[];
  document.querySelectorAll('#admin-grants-list input[type="checkbox"]:checked').forEach(function(cb){ids.push(cb.value)});
  var idsStr=ids.join(',');
  console.log('[saveMemberGrants] saving',{user_id:user_id,type:type,idsStr:idsStr,companyId:currentCompanyId});
  try{
    var r=await callAdmin('rpc','batch_grant_resources',{payload:{p_user_id:user_id,p_company_id:currentCompanyId,p_resource_type:type,p_resource_ids:idsStr,p_granted_by:currentUser?currentUser.id:null}});
    console.log('[saveMemberGrants] raw response:',JSON.stringify(r));
    var inner=r&&r.data;
    var err=r&&r.error;
    if(inner&&inner.error){err=inner.error;}
    if(err){showToast('授权失败: '+(err.message||JSON.stringify(err)));return}
    showToast('已授权 '+ids.length+' 项');
  }catch(e){console.error('[saveMemberGrants] exception:',e);showToast('授权失败: '+(e.message||e))}
}
...
function renderAdminUsers(){
  ...
  var h='<table class="user-table"><thead><tr><th>姓名</th><th>邮箱</th><th>公司</th><th>部门</th><th>岗位</th><th>状态</th><th>手机</th><th>角色</th><th>最后登录</th><th>操作</th></tr></thead><tbody>';
  for(var i=0;i<list.length;i++){
    var u=list[i];
    var deptName='-';
    ...
    var llText=ll?formatDateTime(ll):'从未登录';
    h+='<td style="font-size:12px;color:var(--text3)">'+escHtml(llText)+'</td>';
    h+='<td><button class="btn-sm btn-sm-primary" onclick="openUserEditForm(\''+u.user_id+'\')" style="font-size:11px;padding:3px 10px">编辑</button>'+((isSuperAdmin||currentUserRole==='admin')?' <button class="btn-sm btn-sm-danger" onclick="openResetPwdForm(\''+u.user_id+'\',\''+escHtml(u.email||u.display_name||'-')+'\')" style="font-size:11px;padding:3px 8px">重置密码</button>':'')+(isSuperAdmin?' <button class="btn-sm btn-sm-danger" onclick="deleteUser(\''+u.user_id+'\')" style="font-size:11px;padding:3px 8px;background:#e74c3c">删除</button>':'')+'</td></tr>';
  }
...
async function deleteUser(userId){
  if(!confirm('确定要删除该员工账号吗？此操作不可撤销，将同时清除其所有关联数据。'))return;
  try{
    var r=await callAdmin('auth_delete_user','',{userId:userId});
    if(r.error){alert('删除失败: '+(r.error.message||'未知错误'));return}
...
function openUserEditForm(userId){
  ...
  div.innerHTML='<div class="confirm-box" style="max-width:420px;max-height:80vh;overflow-y:auto"><p style="font-weight:600;margin-bottom:12px">编辑员工 - '+escHtml(u.display_name||u.email)+'</p>'+
    ...
    (isSuperAdmin||(currentUserRole==='admin'&&u.company_id===currentCompanyId)?'<label style="font-size:13px;display:block;margin-bottom:4px">状态</label><select id="uedit-status" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:14px;margin-bottom:10px"><option value="active"'+(u.status==='active'||!u.status?' selected':'')+'>在职</option><option value="inactive"'+(u.status==='inactive'?' selected':'')+'>停用</option><option value="leave"'+(u.status==='leave'?' selected':'')+'>离职</option></select>':'')+
...
async function saveUserEdit(userId){
  ...
  if(!isSuperAdmin&&currentUserRole==='admin'&&targetUser&&targetUser.company_id!==currentCompanyId){
    showToast('权限不足: 不能修改其他公司的成员');return;
  }
  var newStatus=data.status;
  if(!isSuperAdmin&&userId===currentUser.id&&newStatus==='leave'){
    showToast('不能将自己设为离职，请联系其他管理员操作');return;
  }
  if(newStatus==='leave'){data.company_id=null;data.department_id=null;}
  if(isSuperAdmin||currentUserRole==='admin'){
    var ur=await callAdmin('update','profiles',{payload:data,filters:[{col:'user_id',op:'eq',val:userId}],query:'*'});
...
async function saveDept(){
  ...
  if(isSuperAdmin||currentUserRole==='admin'){
    try{var dr;if(deptFormId){dr=await callAdmin('update','departments',{payload:payload,filters:[{col:'id',op:'eq',val:deptFormId}],query:'*'});}else{dr=await callAdmin('insert','departments',{payload:payload});}
    if(dr.error){alert('保存失败: '+dr.error.message);return}}
  ...
  await loadAdminData(true);renderAdminDepts();
}
...
async function deleteDept(id,name){
  ...
  var error=null;
  if(isSuperAdmin||currentUserRole==='admin'){
    try{var dr=await callAdmin('delete','departments',{filters:[{col:'id',op:'eq',val:id}],query:'*'});
    if(dr&&dr.error){var dj=dr.data.catch(function(){});error={message:dj&&dj.message||dr.statusText}}
...
async function doSetUserDeptCommit(userId,deptId){var tu=adminUserList.find(function(u){return u.user_id===userId;});if(!isSuperAdmin&&currentUserRole==='admin'&&tu&&tu.company_id!==currentCompanyId){showToast('权限不足');return;}var{error}=await sb.from('profiles').update({department_id:deptId}).eq('user_id',userId);if(error){alert('操作失败: '+error.message);return}for(var i=0;i<adminUserList.length;i++){if(adminUserList[i].user_id===userId){adminUserList[i].department_id=deptId;break;}}var m=document.getElementById('user-dept-modal');if(m)m.remove();if(typeof renderAdminEmployees==="function")renderAdminEmployees();}
...
async function saveNewUser(){
  ...
  var res=await callAdmin('upsert','profiles',{payload:body,query:'*'});
...
async function doResetPassword(){
  ...
  try{
    var r=await callAdmin('auth_reset_password','',{userId:_resetPwdUserId,password:pwd});
    if(r.error){showToast('重置失败: '+(r.error.message||'未知错误'));return}
    showToast('密码已重置');
    closeResetPwdForm();
...
function openQuoteForm(id){
  ...
  }catch(e){console.error('[DEBUG] openQuoteForm error:',e.message,e.stack);showToast('报价表单加载失败: '+e.message)}
}
...
async function saveQuote(){
  ...
  }catch(e){console.error('[DEBUG] saveQuote error:',e);showToast('保存报价失败: '+e.message)}
}

async function confirmDeleteQuote(){if(qeditId)confirmDialog('确定删除该报价？',async function(){await sb.from('quotations').delete().eq('id',qeditId);closeQuoteForm();loadQuotations();showToast('报价已删除')})}
...
function renderContracts(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(ct.project_id){var pn='';var prj=(allProjects||[]).find(function(x){return x.id===ct.project_id});if(prj)pn=prj.name;if(pn)h+='<div class="pc-project" style="font-size:12px;color:var(--primary);margin-top:2px">📋 '+escHtml(pn)+'</div>'}
...
function openContractForm(id){
  ...
  }catch(e){console.error('[DEBUG] openContractForm error:',e.message,e.stack);showToast('合同表单加载失败: '+e.message)}
}
...
async function saveContract(){
  ...
  if(fileEl&&fileEl.files&&fileEl.files.length>0){
    ...
    try{var fresp=await sb.storage.from('contract_files').upload(fpath.replace('contract_files/',''),fl,{cacheControl:'3600',upsert:true});
...
async function confirmDeleteContract(){if(ceditId)confirmDialog('确定删除该合同？',async function(){await sb.from('contracts').delete().eq('id',ceditId);closeContractForm();loadContracts();showToast('合同已删除')})}
...
function renderPayments(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(pn)h+='<div style="font-size:12px;color:var(--primary);margin-top:2px">📋 '+escHtml(pn)+'</div>';
...
function openPaymentForm(id){
  ...
  pprj.innerHTML='<option value="">选择项目...</option>';
  ...
  }catch(e){console.error('[DEBUG] openPaymentForm error:',e.message,e.stack);showToast('回款表单加载失败: '+e.message)}
}
...
async function confirmDeletePayment(){if(peditId)confirmDialog('确定删除该回款记录？',async function(){await sb.from('payments').delete().eq('id',peditId);closePaymentForm();loadPayments();showToast('回款记录已删除')})}
...
async function loadTickets(){
  ...
  if(error){grid.innerHTML='<div class="empty">加载失败: '+h(error.message)+'</div>';return}
...
function renderTickets(){
  ...
  var h='';for(var i=0;i<list.length;i++){var t=list[i];
    ...
    if(t.engineer_name)h+='<span>🔧 '+escHtml(t.engineer_name)+'</span>';
    h+='</div>';
...
function renderVisits(){
  ...
  var h='';for(var i=0;i<list.length;i++){var v=list[i];var cn='';if(v.client_id&&allClients){var cc=allClients.find(function(x){return x.id===v.client_id});cn=cc?cc.name:''}
    ...
    h+='<div class="lead-name">'+escHtml(v.title||'回访记录')+' <span style="font-size:11px;color:var(--text3)">'+escHtml(cn)+'</span></div>';
    h+='<div class="lead-meta"><span class="lead-status-badge '+(ss[v.status]||'')+'">'+(v.status==='planned'?'计划中':v.status==='completed'?'已完成':'已取消')+'</span>';
    h+='<span>'+escHtml(vm[v.visit_type]||v.visit_type)+'</span><span>'+(mth[v.visit_method]||'📋')+' '+escHtml(v.visit_method||'')+'</span>';
...
function openSalesVisitForm(id){
  svEditId=id||null;
...
function closeSalesVisit(){document.getElementById('visit-modal').classList.add('hidden');svEditId=null}
...
function openVisitTaskForm(){var n=document.getElementById('vf-title').value||'';var c=prompt('回访任务标题:',n);if(!c)return;
  ...
  sb.from('visit_tasks').insert([{company_id:currentCompanyId,title:c,task_type:'care',scheduled_date:date,status:'pending',created_at:new Date().toISOString()}]).then(function(r){
...
function renderWarranties(){
  ...
  var h='';for(var i=0;i<list.length;i++){var w=list[i];var cn='';if(w.client_id&&allClients){var cc=allClients.find(function(x){return x.id===w.client_id});cn=cc?cc.name:''}
    ...
    h+='<span>📅 '+escHtml(w.warranty_start||'')+' → '+escHtml(endLabel)+'</span>';
...
function renderMaintenancePlans(){
  ...
  var h='';for(var i=0;i<list.length;i++){var m=list[i];var cn='';if(m.client_id&&allClients){var cc=allClients.find(function(x){return x.id===m.client_id});cn=cc?cc.name:''}
    h+='<div class="lead-card">';
    h+='<div class="lead-name">🔧 '+escHtml(m.product_name||'维保计划')+' <span style="font-size:11px;color:var(--text3)">'+escHtml(cn)+'</span></div>';
    h+='<div class="lead-meta"><span class="lead-status-badge '+(sc[m.status]||'')+'">'+escHtml(ss[m.status]||m.status)+'</span>';
    h+='<span>'+escHtml(m.plan_type||'')+'</span><span>🔄 '+escHtml(m.schedule_interval||'')+'</span>';
...
function renderKB(){
  ...
  var h='';for(var i=0;i<list.length;i++){var k=list[i];
    ...
    if(k.related_products)h+='<span>📦 '+escHtml(k.related_products)+'</span>';
...
function renderScouting(){
  ...
  // Render table
  if(!filtered.length){
    tbody.innerHTML='<tr><td colspan="14" style="text-align:center;padding:24px;color:var(--text-secondary)">暂无匹配数据</td></tr>';
...
function refreshScoutingCompare(){
  ...
  if(cards){
    var total=filtered.length;var supSet=new Set();for(var vi=0;vi<filtered.length;vi++){if(filtered[vi].supplier_name)supSet.add(filtered[vi].supplier_name.trim().toLowerCase());}var supplierCount=supSet.size;
  ...
  // ====== Compare Table (Horizontal: suppliers as rows per product) ======
...
async function saveScouting(){
  ...
  var obj={
    ...
    status:document.getElementById('sc-status').value||'pending',
    updated_at:new Date().toISOString()
  };
  var r;
...
async function loadProducts(){
  ...
  var r;
  if(currentCompanyRole!=='admin'&&!isSuperAdmin){
    r=await callAdmin('select','products',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'updated_at.desc'},{col:'',op:'limit',val:'50000'}]});
    allProducts=(r&&r.data?r.data:[]);
  }else{
    r=await q;
    if(r.error){console.error(r.error);return}
    allProducts=r.data||[];
  }
  if(currentCompanyRole!=='admin'&&!isSuperAdmin){allProducts=allProducts.filter(function(p){return canAccess('products',p.id)})}
...
function openProductForm(){
  ...
  document.getElementById('pfm-commission-rate').value='';
  document.getElementById('pfm-moq').value='';
  document.getElementById('pfm-sample').value='';
  document.getElementById('pfm-channel').value='';
  document.getElementById('pfm-compliance').value='';
...
function editProduct(id){
  ...
  document.getElementById('pfm-commission-rate').value=p.commission_rate||'';
  var _sp={};try{_sp=typeof p.specs==='string'?JSON.parse(p.specs):(p.specs||{})}catch(e){_sp={}}
  document.getElementById('pfm-moq').value=_sp['MOQ']||_sp['moq']||'';
  document.getElementById('pfm-sample').value=_sp['有无样品']==='有'?'1':(_sp['有无样品']==='无'?'0':'');
  document.getElementById('pfm-channel').value=_sp['渠道']||_sp['供货渠道']||'';
  document.getElementById('pfm-compliance').value=_sp['合规要求']||'';
...
async function saveProduct(){
  ...
  var _moq=document.getElementById('pfm-moq').value.trim();if(_moq)specs['MOQ']=_moq;
  var _sample=document.getElementById('pfm-sample').value;if(_sample==='1')specs['有无样品']='有';else if(_sample==='0')specs['有无样品']='无';
  var _channel=document.getElementById('pfm-channel').value.trim();if(_channel)specs['渠道']=_channel;
  var _compliance=document.getElementById('pfm-compliance').value.trim();if(_compliance)specs['合规要求']=_compliance;
  var data={
    company_id:currentCompanyId,
    name:name,
    ...
    commission_rate:parseFloat(document.getElementById('pfm-commission-rate').value)||0,
  ...
  var r;
  if(productEditId){r=await sb.from('products').update(data).eq('id',productEditId)}else{r=await sb.from('products').insert(data).select('id')}
  if(r.error){showToast('保存失败:'+r.error.message);return}
  // 非管理员自动给自己授权
...
function renderCommission(){
  ...
  for(var i=0;i<allCommission.length;i++){
    ...
    h+='<tr><td>'+escHtml(uname)+'</td><td>'+c.calc_year+'-'+String(c.calc_month).padStart(2,'0')+'</td><td>¥'+fmtNum(c.actual_amount)+'</td><td>¥'+fmtNum(c.target_amount)+'</td><td>'+(parseFloat(c.target_completion_rate)||0)+'%</td><td style="font-weight:600;color:var(--primary)">¥'+fmtNum(c.commission_amount)+'</td><td>'+(statusLabels[c.status]||c.status)+'</td><td>';
...
async function loadSuppliers(){
  var r;
  if(currentCompanyRole!=='admin'&&!isSuperAdmin){
    r=await callAdmin('select','suppliers',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'updated_at.desc'},{col:'',op:'limit',val:'50000'}]});
    allSuppliers=(r&&r.data?r.data:[]);
  }else{
    var q=sb.from('suppliers').select('*').eq('company_id',currentCompanyId).order('updated_at',{ascending:false});
    var res=await q;
    allSuppliers=res.data||[];
  }
  if(currentCompanyRole!=='admin'&&!isSuperAdmin){allSuppliers=allSuppliers.filter(function(s){return canAccess('suppliers',s.id)})}
  var clientMap={};
...
function buildSupCategories(){
  ...
  for(var j=0;j<customSupCats.length;j++)cats.add(customSupCats[j]);
  allSupCategories=Array.from(cats).sort();
  var sel=document.getElementById('sup-cat-filter');
  var v=sel?sel.value:'';
  var h='<option value="">全部类别</option>';
  for(var k=0;k<allSupCategories.length;k++)h+='<option value="'+escHtml(allSupCategories[k])+'"'+(v===allSupCategories[k]?' selected':'')+'>'+escHtml(allSupCategories[k])+'</option>';
  if(sel)sel.innerHTML=h;
...
function renderSuppliers(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    h+='<div class="sup-card"><div class="sc-header"><div><div class="sc-name">'+escHtml(sp.name||'')+'</div><div class="sc-cat">'+(sp.category||'\u672a\u5206\u7c7b')+'</div>'+(sp._dir_name&&sp._dir_name!==sp.name?'<div class="sc-dir-link" style="font-size:11px;color:var(--primary)">\ud83d\udcc2 \u4f01\u4e1a\u540d\u5f55: '+escHtml(sp._dir_name)+'</div>':'')+'</div><span class="level-badge level-'+sp.cooperation_level+'">'+(sp.cooperation_level||'C')+'\u7ea7</span></div>';
...
function openSupplierForm(id){
  ...
  if(sp&&sp.company_directory_id&&allCompaniesMap){
    for(var dk4 in allCompaniesMap){if(allCompaniesMap[dk4].id===sp.company_directory_id){document.getElementById('sup-name').dataset.dirId=sp.company_directory_id;document.getElementById('sup-name').title='\u5173\u8054\u4f01\u4e1a\u540d\u5f55: '+allCompaniesMap[dk4].name;break}}
  }
  // Linked client
...
async function renderClient360Overview(c,orders){
  ...
  var html='<div style="display:flex;justify-content:flex-end;margin-bottom:12px"><button class="btn-sm btn-sm-primary" onclick="closeClient360();openForm(\''+c.id+'\')">✏️ 编辑客户</button></div>';
  // ── 项目信息
  var projs=[];
  try{projs=JSON.parse(c.projects||'[]');if(!Array.isArray(projs))projs=[]}catch(e){projs=[]}
  if(projs.length>0){
    html+='<div class="c360-card"><div class="c360-card-title">📦 项目信息</div>';
    for(var pj=0;pj<projs.length;pj++){
      ...
      html+='<div style="font-weight:600;font-size:14px;margin-bottom:6px">'+h(pr.project_name||'项目#'+(pj+1))+'</div>';
      var pfields=[['招标日期',pr.bidding_date||'-'],['中标人',pr.bidder_name||'-'],['中标人电话',pr.bidder_phone||'-'],['中标公司',pr.bidder_company||'-'],['中标金额',pr.bid_amount?formatMoney(pr.bid_amount):'-']];
  ...
  // ── 基本信息
  html+='<div class="c360-card"><div class="c360-card-title">📋 基本信息</div>';
  var rows=[
...
function renderClient360Contacts(c){
  ...
  if(contacts.length===0)html='<div class="empty" style="padding:40px">暂无联系人</div>';
    ...
    for(var i=0;i<contacts.length;i++){
      ...
      html+='<div class="c360-contact-item"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div class="c360-contact-name">'+h(co.name||'未命名')+dm+'</div><button onclick="event.stopPropagation();transferFrom360(\''+co.id+'\')" style="font-size:11px;color:var(--primary);border:1px solid var(--primary);border-radius:12px;background:transparent;padding:2px 10px;cursor:pointer;flex-shrink:0">转移</button></div>';
...
function renderLeads(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(l.project_name)html+='<div class="lead-project" style="font-size:12px;color:var(--primary);margin-bottom:4px">📋 '+h(l.project_name)+'</div>';
    ...
    html+='<button class="btn-lead-primary" onclick="event.stopPropagation();openLeadForm(\''+l.id+'\')">编辑</button>';
...
function renderPoolLeads(){
  ...
  if(statsBar)statsBar.innerHTML='<div class="stat-card"><div class="stat-num">'+total+'</div><div class="stat-label">公海线索</div></div><div class="stat-card"><div class="stat-num">'+list.length+'</div><div class="stat-label">'+(filter!=='all'||q?'筛选结果':'当前显示')+'</div></div>';
  ...
  for(var i=0;i<list.length;i++){
    ...
    if(l.source)html+='<span>来源: '+h(l.source)+'</span>';
    if(l.industry)html+='<span>行业: '+h(l.industry)+'</span>';
    html+='</div>';
    html+='<div class="lead-actions">';
    html+='<button class="btn-lead-primary" onclick="event.stopPropagation();openLeadForm(\''+l.id+'\')">编辑</button>';
    html+='<button class="btn-lead-primary" onclick="event.stopPropagation();assignLead(\''+l.id+'\')">分配</button>';
    var li=_leadProjectCache.length;
    _leadProjectCache.push({id:l.id,projectName:l.project_name||l.name||'',companyName:l.name||'',clientId:l.client_id||''});
    html+='<button class="btn-lead-success" onclick="event.stopPropagation();openProjectFormFromLeadIdx('+li+')">转项目</button>';
    html+='<button class="btn-lead-danger" onclick="event.stopPropagation();deleteLead(\''+l.id+'\')">删除</button>';
...
function selectLeadCompany(idx){
  ...
  onLeadTypeChange();
  // 自动填充联系人（从 allContacts 表按 client_id 查找）
...
// === Lead Contact picker — from selected client's contacts only ===
...
function onLeadContactPicker(){
  ...
  for(var i=0;i<contacts.length;i++){
    ...
    html+='<div class="name-suggestion" style="cursor:pointer;padding:8px 12px;border-bottom:1px solid var(--border)" onmousedown="selectLeadContact('+i+')"><div class="ns-name">'+escHtml(ct.name||'')+'</div><div class="ns-detail">📞 '+escHtml(ph)+(em?' · 📧 '+escHtml(em):'')+(ct.position?' · '+escHtml(ct.position):'')+'</div></div>';
  }
...
function openLeadForm(id){
  ...
  if(id){
    var l=(allLeads||[]).concat(poolLeads||[]).find(function(x){return x.id==id||String(x.id)===String(id)});
    if(l){
...
function syncLeadFromClient(clientId){
  ...
  if(!c)return;
  // 同步信用代码：优先从 company_info JSONB，兼容 credit_code 独立列
  var cc='';
  ...
  // 同步联系人
  try{var cts=allContacts.filter(function(ct){return ct.client_id===clientId||String(ct.client_id)===String(clientId)});
...
async function saveLead(){
  ...
  if(!leadEditId){
    ...
    if(error){console.error('[DEBUG saveLead] error:',error.message,'details:',error.details,'hint:',error.hint,'code:',error.code);showToast('创建失败: '+error.message);return}
  }else{
...
async function doAssignLead(){
  ...
  await loadPoolLeads();await loadLeads();showToast('已分配');
}
...
async function convertLeadToClient(id){
  ...
  if(le||!leads){showToast('线索未找到: '+(le?le.message:'数据为空'));return}
  var l=leads;
  ...
  showToast(existing&&existing.length>0?'线索已关联到已有客户「'+existing[0].name+'」':'已转化为客户');
}
...
async function importLeads(){
  ...
  if(imported>0){await loadPoolLeads();showToast('已导入 '+imported+' 条线索')}
  else showToast('导入失败');
}
...
async function recycleStale(){
  ...
  if(recycled>0){await loadPoolLeads();await loadLeads();showToast('已回收 '+recycled+' 条线索')}
...
async function loadFollowups(){
  ...
  for(var i=0;i<data.length;i++){
    ...
    h+='<div class="schedule-item" style="border-left-color:#8B5CF6"><div class="sch-title"><span style="background:#EDE9FE;color:#7C3AED;padding:1px 6px;border-radius:4px;font-size:11px">'+escHtml(f.type||'其他')+'</span> '+(cl2?escHtml(cl2.name):'未知客户')+'</div><div style="font-size:13px;margin:4px 0">'+escHtml(f.content||'')+'</div>'+(f.outcome?'<div style="font-size:12px;color:#10B981">'+escHtml(f.outcome)+'</div>':'')+(f.next_step?'<div class="sch-meta">下一步: '+escHtml(f.next_step)+(f.next_date?' ('+f.next_date+')':'')+'</div>':'')+'<div class="sch-meta">'+new Date(f.created_at).toLocaleString('zh-CN')+'<button class="btn-sm" style="font-size:10px;padding:1px 6px;color:var(--danger);margin-left:8px" onclick="deleteEngagement(\''+f.id+'\')\">删除</button></div></div>';
  }
  list.innerHTML=h;
}
...
function switchCollabTab(tab){  // Hide all non-mo collab panels when showing mo panels
  if(tab.startsWith("mo-")){
    var regularPanels=document.querySelectorAll("#collab-view .collab-panel");
    for(var rp=0;rp<regularPanels.length;rp++)regularPanels[rp].classList.add("hidden");
  } else {
    // Hide mo panels when showing regular panels
    var moPs=document.querySelectorAll("#collab-view .mo-panel");
    for(var mp2=0;mp2<moPs.length;mp2++)moPs[mp2].classList.add("hidden");
  }

  // [INTEGRATED] Handle my-office panels (overview + docs only)
  var moPanels=document.querySelectorAll("#collab-view .mo-panel");
  for(var mp=0;mp<moPanels.length;mp++)moPanels[mp].classList.toggle("hidden",moPanels[mp].id!==tab);
  // Init on demand
  if(tab==="mo-overview"&&typeof renderMyOfficeOverview==="function")renderMyOfficeOverview();
  ...
  var panels={schedule:'collab-schedule-panel',task:'collab-task-panel',approval:'collab-approval-panel',comment:'collab-comment-panel',followup:'collab-followup-panel',msg:'collab-msg-panel'};
  for(var k in panels){
  ...
  else if(tab==='followup')loadFollowups();
  else if(tab==='msg')loadMessages();
}

// === 1. Schedules ===
async function loadSchedules(){
  console.log('[DEBUG loadSchedules] entered, currentCompanyId:',currentCompanyId);
  if(!currentCompanyId){console.log('[DEBUG loadSchedules] NO currentCompanyId, returning');return;}
  var today=new Date().toISOString().slice(0,10);
  console.log('[DEBUG loadSchedules] querying Supabase for today>=',today);
  var {data}=await sb.from('schedules').select('*').eq('company_id',currentCompanyId).gte('start_time',today).order('start_time');
  console.log('[DEBUG loadSchedules] Supabase returned, data length:',data?data.length:'null');
  var list=document.getElementById('schedule-list');
  console.log('[DEBUG loadSchedules] schedule-list element found:',!!list);
  ...
  for(var i=0;i<data.length;i++){
    ...
    h+='<div class="schedule-item" style="border-left-color:'+(s.color||'#4F6EF7')+'"><div class="sch-title">'+escHtml(s.title)+' <span style="font-size:11px;color:var(--text3)">'+time+'</span></div>'+(s.description?'<div style="font-size:13px;color:var(--text2)">'+escHtml(s.description)+'</div>':'')+(s.location?'<div class="sch-meta">📍 '+escHtml(s.location)+'</div>':'')+'<div style="margin-top:6px;display:flex;gap:6px"><button class="btn-sm" style="font-size:11px;padding:2px 8px" onclick="editSchedule(\''+s.id+'\')\">编辑</button><button class="btn-sm" style="font-size:11px;padding:2px 8px;color:var(--danger)" onclick="deleteSchedule(\''+s.id+'\')\">删除</button></div></div>';
  }
  list.innerHTML=h;
}
...
async function loadTasks(){
  ...
  for(var i=0;i<data.length;i++){
    ...
    h+='<div class="task-item task-priority-'+prio+'"><div class="task-title">'+escHtml(t.title)+' <span class="status-badge status-'+(t.status==='in_progress'?'progress':t.status)+'">'+statusLabel+'</span></div>'+(t.description?'<div style="font-size:13px;color:var(--text2)">'+escHtml(t.description)+'</div>':'')+(t.due_date?'<div class="task-meta">📅 截止: '+t.due_date+'</div>':'')+'<div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap"><button class="btn-sm" style="font-size:11px;padding:2px 8px" onclick="editTask(\''+t.id+'\')\">编辑</button><button class="btn-sm" style="font-size:11px;padding:2px 8px;color:var(--danger)" onclick="deleteTask(\''+t.id+'\')\">删除</button></div></div>';
  }
  list.innerHTML=h;
}
...
async function loadComments(){
  ...
  if(!commentClientsList.length){
    ...
    for(var i=0;i<allClients.length;i++){var cl=allClients[i];sel.innerHTML+='<option value="'+cl.id+'">'+escHtml(cl.name)+'</option>';commentClientsList.push(cl)}
  }
  ...
  for(var i=0;i<data.length;i++){
    ...
    h+='<div class="comment-item"><div class="cmt-title">'+escHtml(cm.content)+'</div><div class="cmt-meta">客户: '+(cl?escHtml(cl.name):'未知')+' · '+new Date(cm.created_at).toLocaleString('zh-CN')+'<button class="btn-sm" style="font-size:10px;padding:1px 6px;color:var(--danger);margin-left:8px" onclick="deleteComment(\''+cm.id+'\')\">删除</button></div></div>';
  }
  list.innerHTML=h;
}
...
async function renderAdminEmployees(){
  ...
  if(q)list=list.filter(function(u){return (u.display_name||'').toLowerCase().indexOf(q)>=0||(u.email||'').toLowerCase().indexOf(q)>=0||(u.position||'').toLowerCase().indexOf(q)>=0;});
  var h='<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="border-bottom:2px solid var(--border)"><th style="padding:10px 8px;text-align:left">姓名</th><th style="padding:10px 8px;text-align:left">邮箱</th><th style="padding:10px 8px;text-align:left">公司</th><th style="padding:10px 8px;text-align:left">手机</th><th style="padding:10px 8px;text-align:left">岗位</th><th style="padding:10px 8px;text-align:left">角色</th><th style="padding:10px 8px;text-align:left">部门</th><th style="padding:10px 8px;text-align:left">状态</th><th style="padding:10px 8px;text-align:left">最后登录</th><th style="padding:10px 8px;text-align:center">操作</th></tr></thead><tbody>';
  for(var j=0;j<list.length;j++){
    var u=list[j];var dn=u.display_name||'';var em=u.email||'';var rn=u.role||'';var did=u.department_id||u.dept_id||'';
    ...
    h+='<tr style="border-bottom:1px solid var(--border)"><td style="padding:8px">'+escHtml(dn)+'</td><td style="padding:8px">'+escHtml(em)+'</td><td style="padding:8px">'+escHtml(companyName)+'</td><td style="padding:8px">'+escHtml(u.phone||'-')+'</td><td style="padding:8px">'+escHtml(u.position||'-')+'</td><td style="padding:8px">'+escHtml(rn)+'</td><td style="padding:8px">'+escHtml(deptName)+'</td><td style="padding:8px">'+statusLabel+'</td><td style="padding:8px;font-size:12px;color:var(--text3)">'+escHtml(u.last_login_at?formatDateTime(u.last_login_at):'从未登录')+'</td><td style="padding:8px;text-align:center"><a href="javascript:void(0)" onclick="openUserEditForm(\''+u.user_id+'\')" style="color:var(--primary);font-size:13px">编辑</a></td></tr>';
  }
  h+='</tbody></table>';
...
async function sendInvitationByEmail(){
  ...
  if(!email){document.getElementById('invite-err').textContent='请输入邮箱地址';document.getElementById('invite-err').style.display='block';return}
  if(!/^[^@]+@[^@]+$/.test(email)){document.getElementById('invite-err').textContent='邮箱格式不正确';document.getElementById('invite-err').style.display='block';return}
  try{
    var check=await callAdmin('select','invitations',{query:'*',filters:[{col:'to_email',op:'eq',val:email},{col:'from_company_id',op:'eq',val:currentCompanyId},{col:'status',op:'eq',val:'pending'}]});
    if(check&&check.data){var existing=check.data;if(existing&&existing.length>0){document.getElementById('invite-err').textContent='该邮箱已有待处理的邀请';document.getElementById('invite-err').style.display='block';return}}
    var userCheck=await callAdmin('select','profiles',{query:'user_id,company_id',filters:[{col:'email',op:'eq',val:email}]});
    var userData=null;if(userCheck&&userCheck.data){var ua=userCheck.data;if(ua&&ua.length>0)userData=ua[0]}
    var r=await callAdmin('insert','invitations',{payload:{from_company_id:currentCompanyId,to_email:email,message:message||null}});
    if(r&&r.error){var j=r.error.catch(function(){});document.getElementById('invite-err').textContent='发送失败: '+(j&&j.message||r.statusText);document.getElementById('invite-err').style.display='block';return}
    closeInviteModal();
    var msg=userData&&userData.company_id?'该用户当前在职，但已发送邀请（需先离职才能接受）。':'邀请已发送，用户将在通知面板看到邀请。';
    if(!userData)msg='邀请已发送。该邮箱尚未注册，用户注册后将自动看到邀请。';
    showToast(msg);
  }catch(e){document.getElementById('invite-err').textContent='发送失败: '+e.message;document.getElementById('invite-err').style.display='block'}
}

// === Employee side: load my invitations ===
...
async function loadFreeAgents(){
  try{
    var r=await callAdmin('select','profiles',{query:'user_id,email,display_name,phone,status,invited_company_id,invited_at',filters:[{col:'status',op:'eq',val:'leave'},{col:'company_id',op:'is',val:null},{col:'',op:'order',val:'invited_at.asc.nullslast'}]});
    if(r&&r.data)allFreeAgents=r.data;
    else allFreeAgents=[];
  }catch(e){allFreeAgents=[];console.error('loadFreeAgents',e)}
}
function renderFreeAgents(){
  var el=document.getElementById('admin-fa-list');
  var q=(document.getElementById('fa-search').value||'').trim().toLowerCase();
  var list=allFreeAgents;
  if(q)list=list.filter(function(u){return(u.email||'').toLowerCase().indexOf(q)>=0||(u.display_name||'').toLowerCase().indexOf(q)>=0||(u.phone||'').toLowerCase().indexOf(q)>=0});
  if(list.length===0){el.innerHTML='<div style="text-align:center;padding:40px;color:var(--muted)">暂无自由员工</div>';return}
  var h='<table class="data-table"><thead><tr><th>姓名</th><th>邮箱</th><th>电话</th><th>状态</th><th>操作</th></tr></thead><tbody>';
  for(var i=0;i<list.length;i++){
    var u=list[i];
    var badge='离职';
    if(u.invited_company_id)badge='已邀请';
    h+='<tr><td>'+escHtml(u.display_name||'-')+'</td><td>'+escHtml(u.email||'-')+'</td><td>'+escHtml(u.phone||'-')+'</td><td><span class="badge '+(u.invited_company_id?'badge-info':'badge-warn')+'">'+badge+'</span></td><td>';
    if(!u.invited_company_id){
      h+='<button class="btn-save" style="padding:4px 12px;font-size:12px" onclick="inviteFreeAgent(\''+u.user_id+'\',\''+escHtml(u.email||'')+'\')">邀请入职</button>';
    }else{
      h+='<span style="font-size:12px;color:var(--muted)">等待接受</span>';
    }
    h+='</td></tr>';
  }
  h+='</tbody></table>';
...
async function inviteFreeAgent(uid,email){
  if(!confirm('确认邀请 '+email+' 入职本公司？'))return;
  try{
    var r=await callAdmin('update','profiles',{payload:{invited_company_id:currentCompanyId,invited_at:new Date().toISOString()},filters:[{col:'user_id',op:'eq',val:uid}],query:'*'});
    if(r&&r.error){var j=r.error.catch(function(){});showToast('邀请失败: '+(j&&j.message||r.statusText));return}
    showToast('邀请已发送');
    await loadFreeAgents();renderFreeAgents();
  }catch(e){showToast('邀请失败: '+e.message)}
}
...
function openResignForm(){
  ...
  document.getElementById('resign-date').value=today.toISOString().split('T')[0];
  document.getElementById('resign-reason').value='';
  document.getElementById('resign-form-err').style.display='none';
  document.getElementById('resign-form-modal').style.display='flex';
}
function closeResignForm(){
  document.getElementById('resign-form-modal').style.display='none';
}
async function submitResignation(){
  var dateVal=document.getElementById('resign-date').value;
  var reason=document.getElementById('resign-reason').value.trim();
  if(!dateVal){document.getElementById('resign-form-err').textContent='请选择预期离职日期';document.getElementById('resign-form-err').style.display='block';return}
  var d=new Date(dateVal);var today=new Date();today.setHours(0,0,0,0);
  if(d<today){document.getElementById('resign-form-err').textContent='离职日期不能是过去';document.getElementById('resign-form-err').style.display='block';return}
  try{
    var r=await callAdmin('update','profiles',{payload:{resignation_status:'pending',resignation_date:dateVal,resignation_reason:reason||null,resignation_requested_at:new Date().toISOString()},filters:[{col:'user_id',op:'eq',val:currentUser.id}],query:'*'});
    if(r&&r.error){var j=r.error.catch(function(){});document.getElementById('resign-form-err').textContent='提交失败: '+(j&&j.message||r.statusText);document.getElementById('resign-form-err').style.display='block';return}
    closeResignForm();showToast('离职申请已提交，等待管理员审批');
    document.getElementById('my-resign-btn').style.display='none';
  }catch(e){document.getElementById('resign-form-err').textContent='提交失败: '+e.message;document.getElementById('resign-form-err').style.display='block'}
}
var allResignations=[];
async function loadResignations(){
  try{
    var r=await callAdmin('select','profiles',{query:'user_id,email,display_name,phone,position,department_id,resignation_status,resignation_date,resignation_reason,resignation_requested_at',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'resignation_status',op:'not.is',val:null},{col:'',op:'order',val:'resignation_requested_at.desc'}]});
    if(r&&r.data)allResignations=r.data;
    else allResignations=[];
  }catch(e){allResignations=[];console.error('loadResignations',e)}
}
function renderResignations(){
  var el=document.getElementById('admin-resign-list');
  var filter=document.getElementById('resign-filter').value;
  var list=allResignations;
  if(filter!=='all')list=list.filter(function(r){return r.resignation_status===filter});
  if(list.length===0){el.innerHTML='<div style="text-align:center;padding:40px;color:var(--muted)">暂无离职申请</div>';return}
  var h='<table class="data-table"><thead><tr><th>姓名</th><th>邮箱</th><th>岗位</th><th>离职日期</th><th>原因</th><th>状态</th><th>操作</th></tr></thead><tbody>';
  for(var i=0;i<list.length;i++){
    var r=list[i];
    var statusText={'pending':'待审批','approved':'已通过','rejected':'已拒绝'}[r.resignation_status]||r.resignation_status;
    var statusCls={'pending':'badge-warn','approved':'badge-success','rejected':'badge-danger'}[r.resignation_status]||'';
    h+='<tr><td>'+escHtml(r.display_name||'-')+'</td><td>'+escHtml(r.email||'-')+'</td><td>'+escHtml(r.position||'-')+'</td><td>'+escHtml(r.resignation_date||'-')+'</td><td>'+escHtml((r.resignation_reason||'').substring(0,30))+'</td><td><span class="badge '+statusCls+'">'+statusText+'</span></td><td>';
    if(r.resignation_status==='pending'){
      h+='<button class="btn-save" style="padding:4px 12px;font-size:12px;margin-right:4px" onclick="approveResignation(\''+r.user_id+'\')">通过</button>';
      h+='<button class="btn-cancel" style="padding:4px 12px;font-size:12px" onclick="rejectResignation(\''+r.user_id+'\')">拒绝</button>';
    }
    h+='</td></tr>';
  }
  h+='</tbody></table>';
...
async function approveResignation(uid){
  if(!confirm('确认通过该离职申请？'))return;
  try{
    var r=await callAdmin('update','profiles',{payload:{resignation_status:'approved'},filters:[{col:'user_id',op:'eq',val:uid}],query:'*'});
    if(r&&r.error){var j=r.error.catch(function(){});showToast('操作失败: '+(j&&j.message||r.statusText));return}
    showToast('已通过离职申请，到期后将自动解除');
    await loadResignations();renderResignations();
  }catch(e){showToast('操作失败: '+e.message)}
}
async function rejectResignation(uid){
  if(!confirm('确认拒绝该离职申请？'))return;
  try{
    var r=await callAdmin('update','profiles',{payload:{resignation_status:'rejected'},filters:[{col:'user_id',op:'eq',val:uid}],query:'*'});
    if(r&&r.error){var j=r.error.catch(function(){});showToast('操作失败: '+(j&&j.message||r.statusText));return}
    showToast('已拒绝离职申请');
    await loadResignations();renderResignations();
  }catch(e){showToast('操作失败: '+e.message)}
...
/* ===== OPERATION LOGS ===== */
var allOpLogs=[];var logsError=null;
async function loadOperationLogs(){
  try{
    var resp=await callAdmin('select','operation_logs',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'created_at.desc'},{col:'',op:'limit',val:500}]});
    if(resp&&resp.data){allOpLogs=resp.data;logsError=null;}
    else{allOpLogs=[];var t=await resp.text();logsError='HTTP '+resp.status+(' '+t.substring(0,120));console.error('loadOperationLogs:',resp.status,t.substring(0,200));}
  }catch(e){allOpLogs=[];logsError=e.message;console.error('loadOperationLogs:',e);}
}
function renderOperationLogs(){
  ...
  h+='</tbody></table>';
  if(list.length===0){
    if(logsError)h='<div style="text-align:center;padding:40px;color:var(--text3)"><p>&#x26A0;&#xFE0F; 日志加载失败</p><p style="font-size:12px;margin-top:8px;color:var(--text3)">'+escHtml(logsError)+'</p><p style="font-size:11px;margin-top:12px;color:var(--primary);cursor:pointer" onclick="navigator.clipboard.writeText(\'operation_logs.sql\').then(function(){showToast(\'已复制文件名\')})">📋 请执行 operation_logs.sql → 点此复制文件名</p></div>';
    else h='<div style="text-align:center;padding:40px;color:var(--text3)">暂无操作记录</div>';
  }
  el.innerHTML=h;
}
...
async function writeOpLog(action,entityType,entityId,entityName,detail){
  try{
    ...
    await callAdmin('rpc','write_op_log',{payload:body});
  }catch(e){}
}
...
async function renderCustomFields(){
  ...
  var resp=await callAdmin('select','custom_field_defs',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'entity_type',op:'eq',val:et},{col:'',op:'order',val:'sort_order.asc'}]});
  if(resp&&resp.data)allCustomFields=resp.data;else allCustomFields=[];
...
async function saveField(){
  ...
  var resp;
  if(fieldEditId){
    resp=await callAdmin('update','custom_field_defs',{payload:body,filters:[{col:'id',op:'eq',val:fieldEditId}],query:'*'});
  }else{
    body.sort_order=allCustomFields.length;body.created_at=new Date().toISOString();
    resp=await callAdmin('insert','custom_field_defs',{payload:body});
  }
  if(resp&&resp.data){showToast('字段已保存');closeFieldForm();renderCustomFields();await writeOpLog(fieldEditId?'update':'create','custom_field',fieldEditId||'',label);}
  else{showToast('保存失败')}
}
async function deleteField(id){
  if(!confirm('确认删除此字段？'))return;
  var resp=await callAdmin('delete','custom_field_defs',{filters:[{col:'id',op:'eq',val:id}]});
  if(resp&&resp.data){showToast('已删除');renderCustomFields();}else{showToast('删除失败')}
}
...
async function renderCustomForms(){
  ...
  var resp=await callAdmin('select','custom_forms',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'created_at.desc'}]});
  if(resp&&resp.data)allCustomForms=resp.data;else allCustomForms=[];
  var h='';
...
function openFormBuilder(){
  ...
  callAdmin('insert','custom_forms',{payload:{company_id:currentCompanyId,form_name:name,entity_type:et,fields:[],created_at:new Date().toISOString()}}).then(function(r){if(r.data){showToast('Form created');renderCustomForms();}}).catch(function(){showToast('Create failed');});
}
async function deleteForm(id){if(!confirm('确认删除？'))return;await callAdmin('delete','custom_forms',{filters:[{col:'id',op:'eq',val:id}]});renderCustomForms();showToast('已删除');}
...
async function renderSalesStages(){
  ...
  var resp=await callAdmin('select','sales_stages_def',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'sort_order.asc'}]});
  if(resp&&resp.data)allSalesStages=resp.data;else allSalesStages=[];
  var h='';
...
async function saveStage(){
  ...
  var resp;
  if(stageEditId){
    resp=await callAdmin('update','sales_stages_def',{payload:body,filters:[{col:'id',op:'eq',val:stageEditId}],query:'*'});
  }else{
    body.sort_order=allSalesStages.length+1;body.created_at=new Date().toISOString();
    resp=await callAdmin('insert','sales_stages_def',{payload:body});
  }
  if(resp&&resp.data){showToast('阶段已保存');closeStageForm();renderSalesStages();}else{showToast('保存失败')}
}
async function deleteStage(id){if(!confirm('确认删除？'))return;await callAdmin('delete','sales_stages_def',{filters:[{col:'id',op:'eq',val:id}]});renderSalesStages();showToast('已删除');}

/* Custom Tags */
var allCustomTags=[],tagEditId=null;
async function renderCustomTags(){
  var el=document.getElementById('cfg-tag-list');if(!el)return;
  var resp=await callAdmin('select','custom_tags',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'sort_order.asc'}]});
  if(resp&&resp.data)allCustomTags=resp.data;else allCustomTags=[];
  var h='';
...
async function saveTag(){
  ...
  var resp;
  if(tagEditId){
    resp=await callAdmin('update','custom_tags',{payload:body,filters:[{col:'id',op:'eq',val:tagEditId}],query:'*'});
  }else{
    body.sort_order=allCustomTags.length;body.created_at=new Date().toISOString();
    resp=await callAdmin('insert','custom_tags',{payload:body});
  }
  if(resp&&resp.data){showToast('标签已保存');closeTagForm();renderCustomTags();}else{showToast('保存失败')}
}
async function deleteTag(id){if(!confirm('确认删除？'))return;await callAdmin('delete','custom_tags',{filters:[{col:'id',op:'eq',val:id}]});renderCustomTags();}
...
async function renderNumberingRules(){
  ...
  var resp=await callAdmin('select','numbering_rules',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'created_at.desc'}]});
  if(resp&&resp.data)allNumRules=resp.data;else allNumRules=[];
...
async function saveNumberRule(){
  ...
  var resp;
  if(numEditId){
    resp=await callAdmin('update','numbering_rules',{payload:body,filters:[{col:'id',op:'eq',val:numEditId}],query:'*'});
  }else{
    body.current_seq=1;body.created_at=new Date().toISOString();
    resp=await callAdmin('insert','numbering_rules',{payload:body});
  }
  if(resp&&resp.data){showToast('规则已保存');closeNumberForm();renderNumberingRules();}else{showToast('保存失败')}
}
async function deleteNumRule(id){if(!confirm('确认删除？'))return;await callAdmin('delete','numbering_rules',{filters:[{col:'id',op:'eq',val:id}]});renderNumberingRules();}
...
async function toggleRoleExport(roleName,roleId,checked){
  var resp=await callAdmin('update','roles',{payload:{func_perms:{allow_export:checked}},filters:[{col:'id',op:'eq',val:roleId}],query:'*'});
  if(resp&&resp.data)showToast('导出权限已更新');else showToast('更新失败');
}
async function doBackup(){
  ...
  for(var i=0;i<types.length;i++){
    var t=types[i];
    var resp=await callAdmin('select',(t==='products'?'products':t),{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'limit',val:'10000'}]});
    if(resp&&resp.data){var data=resp.data;allData[t]=data;totalCount+=data.length;}
  }
  ...
  // Log backup
  try{
    await callAdmin('insert','backup_logs',{payload:{company_id:currentCompanyId,user_id:currentUser?currentUser.id:null,backup_type:'manual',entity_types:types,record_count:totalCount,file_name:'backup_'+currentCompanyId+'.json',file_size:json.length,created_at:new Date().toISOString()}});
  }catch(e){}
...
async function loadBackupHistory(){
  ...
  var resp=await callAdmin('select','backup_logs',{query:'*',filters:[{col:'company_id',op:'eq',val:currentCompanyId},{col:'',op:'order',val:'created_at.desc'},{col:'',op:'limit',val:20}]});
  if(resp&&resp.error){el.innerHTML='加载失败';return}
  var logs=resp.data;
...
async function renderWorkflowTemplates(){
  console.log('renderWorkflowTemplates: start');
  var list=document.getElementById('workflow-template-list');
  if(!list){console.log('renderWorkflowTemplates: list element not found');return}
  try{
    var url=SUPABASE_URL+'/rest/v1/workflow_templates?order=id';
    console.log('renderWorkflowTemplates: fetching',url);
    var r=await callAdmin('select','workflow_templates',{query:'*',filters:[{col:'',op:'order',val:'id'}]});
    if(r&&r.error){console.log('renderWorkflowTemplates: fetch failed',r.error);list.innerHTML='<p style="color:var(--text2)">加载失败: '+escHtml(r.error.message||'未知错误')+'</p>';return}
    var data=r.data;console.log('renderWorkflowTemplates: got',data.length,'templates');
    allTemplates=data;
    var h='';
    if(!data||!data.length){h='<p style="color:var(--text2);text-align:center;padding:20px">暂无模板，点击上方按钮新建</p>'}
    else{
      for(var i=0;i<data.length;i++){
        var t=data[i];
        var stepCount=0;
        try{var s=typeof t.steps==='string'?JSON.parse(t.steps):t.steps;stepCount=Array.isArray(s)?s.length:0}catch(e){}
        var isGlobal=t.company_id===null?'全局':'公司';
        h+='<div class="card" style="padding:12px 16px;display:flex;justify-content:space-between;align-items:center">'+
          '<div><strong>'+escHtml(t.name)+'</strong> <span style="font-size:11px;color:var(--text2);margin-left:8px">'+isGlobal+' · '+stepCount+'步</span>'+
          (t.description?'<br><span style="font-size:12px;color:var(--text2)">'+escHtml(t.description)+'</span>':'')+
          '</div>'+
          '<div style="display:flex;gap:6px">'+
          '<button class="btn-sm" onclick="openWtEdit('+t.id+')">编辑</button>'+
          '<button class="btn-sm" style="background:#f44336;color:#fff" onclick="deleteWt('+t.id+')">删除</button>'+
          '</div></div>';
      }
    }
    list.innerHTML=h;
  }catch(e){console.error('renderWorkflowTemplates: exception',e);list.innerHTML='<p style="color:var(--text2)">加载异常: '+escHtml(e.message)+'</p>'}
}
...
function openWtEdit(id){
  var t=allTemplates.find(function(x){return x.id===id});
  if(!t)return;
  wtEditId=id;
  var steps=t.steps;
  if(typeof steps!=='string')steps=JSON.stringify(steps);
  showWtModal('编辑流程模板',steps);
}
function showWtModal(title,stepsJson){
...
function wfShowProps(idx){
  ...
  if(!allDepartments||!allDepartments.length){
    sb.from('departments').select('*').eq('company_id',currentCompanyId).order('name').then(function(d){if(!d.error){allDepartments=d.data||[];setTimeout(function(){wfShowProps(idx)},0);}});
  }
  if(!allRoles||!allRoles.length){
    sb.from('roles').select('*').eq('company_id',currentCompanyId).order('name').then(function(d){if(!d.error){allRoles=d.data||[];setTimeout(function(){wfShowProps(idx)},0);}});
  }
  ...
  // Custom params
  var params=s.params||[];
  h+='<div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px"><label style="font-weight:600;margin-bottom:6px">⚙️ 自定义参数</label>';
  h+='<div id="wf-params-'+idx+'">';
  for(var pi=0;pi<params.length;pi++){h+=wfBuildParamRow(idx,pi,params[pi]);}
  h+='</div>';
  h+='<button onclick="wfAddParam('+idx+')" style="margin-top:6px;font-size:11px;padding:4px 10px;background:var(--accent);color:#fff;border:none;border-radius:4px;cursor:pointer">+ 添加参数</button></div>';
  // ── 记录当前的展开状态，方便重建后恢复 ──
  var expanded=window.__wfExpanded||{};
  ...
  var apprOpen=expanded['appr_'+idx]?'block':'none';
  h+='<div onclick="var n=this.nextElementSibling;var d=n.style.display===\'none\'?\'block\':\'none\';n.style.display=d;window.__wfExpanded=window.__wfExpanded||{};window.__wfExpanded[\'appr_'+idx+'\']=d===\'block\'" style="cursor:pointer;font-weight:600;font-size:12px;padding:4px 0;display:flex;align-items:center;gap:4px">🔒 审批设置 <span style="font-size:10px;color:var(--text3)">'+(appr.enabled?'已启用':'未启用')+'</span></div>';
  h+='<div style="padding:6px 0;display:'+apprOpen+'">';
  h+='<label style="display:flex;align-items:center;gap:6px;font-size:12px"><input type="checkbox" '+(appr.enabled?'checked':'')+' onchange="wfSetApproval('+idx+',\'enabled\',this.checked)"> 启用审批</label>';
  h+='<label>审批人角色</label><select onchange="wfSetApproval('+idx+',\'approverRole\',this.value)"><option value="">无</option>';
  ...
  h+='</select>';
  h+='<label>驳回行为</label><select onchange="wfSetApproval('+idx+',\'onReject\',this.value)"><option value="stay"'+(appr.onReject==='stay'?' selected':'')+'>停留在当前步骤</option></select>';
  h+='</div></div>';
  // ── 负责人设置 ──
  ...
  var asgOpen=expanded['asg_'+idx]?'block':'none';
  h+='<div onclick="var n=this.nextElementSibling;var d=n.style.display===\'none\'?\'block\':\'none\';n.style.display=d;window.__wfExpanded=window.__wfExpanded||{};window.__wfExpanded[\'asg_'+idx+'\']=d===\'block\'" style="cursor:pointer;font-weight:600;font-size:12px;padding:4px 0;display:flex;align-items:center;gap:4px">👤 负责人设置 <span style="font-size:10px;color:var(--text3)">'+(asg.enabled?'已启用':'未启用')+'</span></div>';
  h+='<div style="padding:6px 0;display:'+asgOpen+'">';
  h+='<label style="display:flex;align-items:center;gap:6px;font-size:12px"><input type="checkbox" '+(asg.enabled?'checked':'')+' onchange="wfSetAssignee('+idx+',\'enabled\',this.checked)"> 指定负责人</label>';
  h+='<label>授权部门</label><select onchange="wfSetAssignee('+idx+',\'value\',this.value)"><option value="">无</option>';
  for(var di=0;di<(allDepartments||[]).length;di++){h+='<option value="'+escHtml(allDepartments[di].id)+'"'+(String(asg.value||'')===String(allDepartments[di].id||'')?' selected':'')+'>'+escHtml(allDepartments[di].name)+'</option>';}
  h+='</select>';
  h+='<label>鎸囧畾鍛樺伐</label><select id="wf-staff-dd-'+idx+'" onchange="wfSetAssignee('+idx+',\'staffId\',this.value)"><option value="">--</option>';
  ...
  var permOpen=expanded['perm_'+idx]?'block':'none';
  h+='<div style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px">';
  h+='<div onclick="var n=this.nextElementSibling;var d=n.style.display===\'none\'?\'block\':\'none\';n.style.display=d;window.__wfExpanded=window.__wfExpanded||{};window.__wfExpanded[\'perm_'+idx+'\']=d===\'block\'" style="cursor:pointer;font-weight:600;font-size:12px;padding:4px 0">✏️ 编辑权限</div>';
  h+='<div style="padding:6px 0;display:'+permOpen+'">';
  h+='<label>可编辑字段（逗号分隔）</label><input value="'+escHtml((perm.editFields||[]).join(','))+'" onchange="wfSetPermField('+idx+',\'editFields\',this.value)" placeholder="空=全部可编辑">';
  var ebRoles=['assignee','approver','admin'];
  h+='<label>谁可编辑</label><div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px">';
...
function wfSetApproval(idx,key,val){
  try{
    ...
    setTimeout(function(){wfShowProps(idx)},0);
...
function wfBuildParamRow(nodeIdx,paramIdx,p){
  ...
  return '<div style="display:flex;gap:4px;align-items:center;margin-top:4px">'+
    '<input value="'+k+'" placeholder="参数名" onchange="wfSetParam('+nodeIdx+','+paramIdx+',\'key\',this.value)" style="flex:1;padding:4px 6px;font-size:11px">'+
    '<input value="'+v+'" placeholder="参数值" onchange="wfSetParam('+nodeIdx+','+paramIdx+',\'value\',this.value)" style="flex:2;padding:4px 6px;font-size:11px">'+
    '<button onclick="wfRemoveParam('+nodeIdx+','+paramIdx+')" style="background:none;border:none;color:#f44336;font-size:14px;cursor:pointer;padding:0 4px">&times;</button>'+
    '</div>';
}
function wfAddParam(nodeIdx){
  if(!wfEditorSteps[nodeIdx].params)wfEditorSteps[nodeIdx].params=[];
  wfEditorSteps[nodeIdx].params.push({key:'',value:''});
  wfShowProps(nodeIdx);
}
function wfSetParam(nodeIdx,paramIdx,field,value){
  if(!wfEditorSteps[nodeIdx].params)return;
  wfEditorSteps[nodeIdx].params[paramIdx][field]=value;
}
function wfRemoveParam(nodeIdx,paramIdx){
  wfEditorSteps[nodeIdx].params.splice(paramIdx,1);
  wfShowProps(nodeIdx);
}
...
async function saveWtVisual(){
  ...
  try{
    ...
    if(wtEditId){var r=await callAdmin('update','workflow_templates',{payload:body,filters:[{col:'id',op:'eq',val:wtEditId}],query:'*'});}else{var r=await callAdmin('insert','workflow_templates',{payload:body});};
    if(r&&r.error){var t=await r.text();alert('保存失败: '+t);btn.disabled=false;btn.textContent='保存';return}
...
async function deleteWt(id){
  var t=allTemplates.find(function(x){return x.id===id});
  if(!t)return;
  if(!confirm('确定删除模板 "'+t.name+'"？'))return;
  try{
    var url=SUPABASE_URL+'/rest/v1/workflow_templates?id=eq.'+id;
    var r=await callAdmin('delete','workflow_templates',{filters:[{col:'id',op:'eq',val:id}]});
    if(r&&r.error){var e=await r.text();alert('删除失败: '+e);return}
    await renderWorkflowTemplates();
    showToast('模板已删除');
...
// Stub for defunct freeze buttons (always hidden, prevent ReferenceError)
function toggleLeadFrozen(){showToast('冻结功能暂未开放')}
function editForm(id){showToast('表单编辑功能暂未开放')}


// ====== Project Management ======
var allProjects=[],allStages=[],allBiddings=[],allDeliveries=[],allPayments=[],allTemplates=[],curProjectId=null,curProjectName='',projectStateFilter='doing';
var pStageEditId=null,pBiddingEditId=null,pDeliveryEditId=null,pPaymentEditId=null;

// ---- Workflow Data ----
var curWorkflow={}, curWorkflowState={}, curStep=0;
var WORKFLOW_STEPS=[
  ...
  {seq:6,phase:'内部赋能',key:'training',name:'学习培训',icon:'📖',panel:'training',tip:'完成培训后自动跳转步骤7'},
...
// Build lookup
var WFLookup={};
(function(){var st0=WORKFLOW_STEPS;for(var i=0;i<st0.length;i++){WFLookup[st0[i].seq]=st0[i]}})()

function safeGetSteps(){
  try{return getTemplateSteps(curProject?curProject.template_id:null);}
  catch(e){return WORKFLOW_STEPS;}
}

function initWorkflow(templateId){
  var steps;
  if(templateId){steps=getTemplateSteps(templateId)}
  else{try{steps=safeGetSteps()}catch(e){steps=WORKFLOW_STEPS}}
  curWorkflow={};curWorkflowState={};curStep=1;
  for(var i=0;i<steps.length;i++){
    var seq=steps[i].seq;
    curWorkflow[seq]={done:false,note:'',data:{}};
    // Initialize workflow_state from template step's approval/assignee
    var initStatus='pending';
    if(seq===1)initStatus='active';
    var initSt={status:initStatus,assignedTo:null,approvedBy:null,approvedAt:null};
    // If step 1 has assignee, set assignedTo (resolved later when user selects)
    if(steps[i].assignee&&steps[i].assignee.enabled){
      // assignedTo is set later when project is saved (user picks assignee)
    }
    curWorkflowState[seq]=initSt;
  }
}
initWorkflow();
...
function getTemplateSteps(templateId){
  var steps;
  if(templateId&&allTemplates&&allTemplates.length){
    var t=allTemplates.find(function(x){return x.id===templateId});
    if(t){
      try{if(typeof t.steps==='string')steps=JSON.parse(t.steps);else if(Array.isArray(t.steps))steps=t.steps;}catch(e){}
    }
  }
  // Fallback to WORKFLOW_STEPS if no template steps found
  if(!steps)steps=WORKFLOW_STEPS;
  // Always sync WFLookup with the steps we're returning
  WFLookup={};
  for(var i=0;i<steps.length;i++){WFLookup[steps[i].seq]=steps[i];}
  console.log('[getTemplateSteps] templateId=',templateId,'steps count=',steps.length,'WFLookup keys=',Object.keys(WFLookup));
  return steps;
}
...
function loadWorkflow(){
  ...
  try{curWorkflowState=JSON.parse(proj.workflow_state||'{}')}catch(e){curWorkflowState={}}
  if(!curWorkflow||Object.keys(curWorkflow).length===0)initWorkflow(proj?proj.template_id:null);
  if(!curWorkflowState||Object.keys(curWorkflowState).length===0){
    console.log('[loadWorkflow] initializing workflow_state from template');
    wfInitStateFromTemplate(proj?proj.template_id:null);
  }
  renderWorkbench();
}
function wfInitStateFromTemplate(templateId){
  var steps;
  if(templateId){steps=getTemplateSteps(templateId)}
  else{try{steps=safeGetSteps()}catch(e){steps=WORKFLOW_STEPS}}
  curWorkflowState={};
  for(var i=0;i<steps.length;i++){
    ...
    var initStatus='pending';
    if(seq===1)initStatus='active';
    curWorkflowState[seq]={status:initStatus,assignedTo:null,approvedBy:null,approvedAt:null};
  }
}

async function saveWorkflow(){
  if(!curProjectId)return;
  var payload={workflow:JSON.stringify(curWorkflow),current_step:curStep,workflow_state:JSON.stringify(curWorkflowState)};
  ...
  var proj=allProjects.find(function(p){return p.id===curProjectId});
  if(proj){proj.workflow=JSON.stringify(curWorkflow);proj.current_step=curStep;proj.workflow_state=JSON.stringify(curWorkflowState)}
}
...
function toggleWbSidebar(){
  var l=document.getElementById('wb-left');var o=document.getElementById('wb-mobile-overlay');
  if(!l||!o)return;
  var isOpen=l.classList.contains('open');
  if(isOpen){l.classList.remove('open');o.classList.remove('open')}
  else{l.classList.add('open');o.classList.add('open')}
}
...
function renderWorkflowNav(){
  ...
  for(var i=0;i<safeGetSteps().length;i++){
    var s=safeGetSteps()[i];
...
function updateProgress(){
  var doneCount=0;
  for(var i=0;i<safeGetSteps().length;i++){if(stepDone(safeGetSteps()[i].seq))doneCount++}
  var pct=Math.round(doneCount/safeGetSteps().length*100);
  var bar=document.getElementById('wb-progress-bar');if(bar)bar.style.width=pct+'%';
  var txt=document.getElementById('wb-progress-text');if(txt)txt.textContent=doneCount+'/'+safeGetSteps().length;
...
function clickWorkflowStep(seq){
  ...
  // Permission check: admin always has access
  var hasPerm=wfCanEditStep(seq);
  if(!hasPerm){
    showToast('⚠️ 你没有编辑当前步骤的权限，仅可查看');
  }

  // Mobile: close sidebar after selecting step
  if(window.innerWidth<=768){toggleWbSidebar()}

  // Decision node: show popup
  if(step.decision && seq===curStep && hasPerm){
...
function decideYes(seq){
  ...
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'active'};
  curWorkflowState[seq].status='done';
  // Jump to yes target
  ...
  curStep=nextSeq;
  // Activate next step's state
  wfActivateStep(nextSeq);
  renderWorkbench();
  saveWorkflow();
}
...
function markStepDone(){
  ...
  if(step&&step.approval&&step.approval.enabled){
    // Need approval - check if already submitted
    var wst=curWorkflowState[curStep]||{};
    if(wst.status==='pending_approval'){showToast('⚠️ 已提交审批，请等待审批人处理');return}
    if(wst.status==='done'){showToast('✓ 审批已通过');return}
    // Submit for approval
    wfSubmitApproval(curStep);
    return;
  }
  // No approval needed - mark directly
  wfDoMarkStepDone(curStep);
}
function wfDoMarkStepDone(seq){
  curWorkflow[seq]=curWorkflow[seq]||{done:false,note:'',data:{}};
  curWorkflow[seq].done=true;
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'active'};
  curWorkflowState[seq].status='done';
  ...
  if(next<=46){curStep=next;wfActivateStep(next)}
  renderWorkbench();
  saveWorkflow();
  showToast('✓ 已完成：'+(WFLookup[seq]?WFLookup[seq].name:''));
}
function wfSubmitApproval(seq){
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'active'};
  curWorkflowState[seq].status='pending_approval';
  curWorkflowState[seq].submittedAt=new Date().toISOString();
  saveWorkflow();
  renderWorkbench();
  showStepPanel(seq);
  showToast('📨 已提交审批，等待审批人处理');
  console.log('[wfSubmitApproval] step',seq,'submitted for approval');
}
function wfApprove(seq){
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'pending_approval'};
  curWorkflowState[seq].approvedBy=currentUser.id;
  curWorkflowState[seq].approvedAt=new Date().toISOString();
  curWorkflowState[seq].status='approved';
  // Now mark done
  wfDoMarkStepDone(seq);
  showToast('✓ 审批通过');
  console.log('[wfApprove] step',seq,'approved by',currentUser.id);
}
function wfReject(seq){
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'pending_approval'};
  curWorkflowState[seq].approvedBy=currentUser.id;
  curWorkflowState[seq].approvedAt=new Date().toISOString();
  curWorkflowState[seq].status='active'; // back to active
  saveWorkflow();
  renderWorkbench();
  showStepPanel(seq);
  showToast('↩️ 审批驳回，请修改后重新提交');
  console.log('[wfReject] step',seq,'rejected by',currentUser.id);
}
function wfActivateStep(seq){
  curWorkflowState[seq]=curWorkflowState[seq]||{status:'pending',assignedTo:null,approvedBy:null,approvedAt:null};
  curWorkflowState[seq].status='active';
  // Check if step has assignee configured
  var step=WFLookup[seq];
  if(step&&step.assignee&&step.assignee.enabled&&step.assignee.value){
    // Find a user with that role
    var assignee=wfFindUserByDept(step.assignee.value);
    if(assignee){
      curWorkflowState[seq].assignedTo=assignee.user_id;
      console.log('[wfActivateStep] assigned step',seq,'to',assignee.display_name,'(role:',step.assignee.value,')');
    }
  }
}
function wfCanEditStep(seq){
  // Admin/superAdmin always has access
  if(isSuperAdmin||currentUserRole==='admin')return true;
  var step=WFLookup[seq];
  if(!step)return true;
  var perm=step.permissions||{};
  var editableBy=perm.editableBy||['admin'];
  // Check if currentUser is assigned to this step
  var wst=curWorkflowState[seq]||{};
  var isAssignee=wst.assignedTo===currentUser.id;
  // Check if currentUser is the approver for this step
  var isApprover=false;
  if(step.approval&&step.approval.enabled&&step.approval.approverRole){
    isApprover=(currentUserRole===step.approval.approverRole);
  }
  // Check against editableBy list
  if(editableBy.indexOf('admin')>=0){return true}
  if(editableBy.indexOf('assignee')>=0&&isAssignee)return true;
  if(editableBy.indexOf('approver')>=0&&isApprover)return true;
  return false;
}
function wfFindUserByDept(deptId){
...
function showStepPanel(seq){
  console.log('[showStepPanel] seq=',seq,'WFLookup=',Object.keys(WFLookup));
  var step=WFLookup[seq];
  if(!step){console.log('[showStepPanel] step not found for seq',seq);return;}
  console.log('[showStepPanel] step=',step.name,'panel=',step.panel);
  // Hide all panels
  var panels=['wb-panel-basic','wb-panel-editor','wb-panel-competitor','wb-panel-decision-chain','wb-panel-training',
  ...
  console.log('[showStepPanel] panelEl for wb-panel-'+panelName+'=',!!panelEl);
  if(panelEl){
    console.log('[showStepPanel] panelEl display=',getComputedStyle(panelEl).display,'hidden class=',panelEl.classList.contains('hidden'));
    console.log('[showStepPanel] panelEl offsetHeight=',panelEl.offsetHeight,'offsetWidth=',panelEl.offsetWidth);
    console.log('[showStepPanel] panelEl innerHTML length=',panelEl.innerHTML.length);
    // Force visible styles
    panelEl.style.display='block';
    panelEl.style.visibility='visible';
    panelEl.style.opacity='1';
    panelEl.style.background='#fff';
    panelEl.style.border='2px solid red';
    panelEl.style.padding='20px';
  }
  var rightEl=document.getElementById('wb-right');
  if(rightEl){console.log('[showStepPanel] wb-right children count=',rightEl.children.length,'first child visible=',rightEl.children[0]?!rightEl.children[0].classList.contains('hidden'):'n/a');}
  ...
  // Render approval status & action buttons
  wfRenderActionButtons(seq,step);
  // Mobile: convert 2-col grids to single column
  if(window.innerWidth<=768){adaptPanelForMobile()}
}

function adaptPanelForMobile(){
  // Convert inline 2-col grids to single column in visible panel
  var right=document.getElementById('wb-right');if(!right)return;
  var grids=right.querySelectorAll('div[style*="grid-template-columns"]');
  for(var i=0;i<grids.length;i++){
    var g=grids[i];
    if(!g.classList.contains('hidden')||g.offsetParent!==null){
...
function onLeadSearchInput(){
  ...
  if(allLeads&&allLeads.length){
    ...
    for(var i=0;i<allLeads.length;i++){
      var l=allLeads[i];
      if((l.company_name||'').toLowerCase().indexOf(lowerQ)>=0||(l.contact_person||'').toLowerCase().indexOf(lowerQ)>=0||(l.name||'').toLowerCase().indexOf(lowerQ)>=0){
        results.push(l);
  ...
  for(var i=0;i<results.length;i++){
    var r=results[i];
    var rName=r.company_name||r.name||'未命名';
    var rSource=r.source||r.lead_source||'';
    html+='<div class="name-suggestion-item" onclick="addLinkedLead(\''+(r.id||'').replace(/'/g,"\\'")+'\',\''+rName.replace(/'/g,"\\'")+'\',\''+rSource.replace(/'/g,"\\'")+'\')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px">';
    html+=h(rName)+' <span style="color:var(--text3);font-size:11px">'+h(r.contact_person||'')+'</span>';
    html+='</div>';
  }
  sug.innerHTML=html;
...
// ── Approval / Permission action buttons rendering ──
function wfRenderActionButtons(seq,step){
  var container=document.getElementById('wb-action-buttons');
  if(!container){return}
  var wst=curWorkflowState[seq]||{};
  var status=wst.status||'active';
  var isDone=curWorkflow[seq]?curWorkflow[seq].done:false;
  var hasPerm=wfCanEditStep(seq);
  var isApprover=wfIsApprover(seq);
  var html='';

  // Show current status badge
  var statusLabel='';
  if(isDone){statusLabel='✅ 已完成'}
  else if(status==='pending_approval'){statusLabel='⏳ 等待审批'}
  else if(status==='approved'){statusLabel='✅ 审批通过'}
  else{statusLabel='📌 进行中'}
  html+='<span style="font-size:12px;color:var(--text2);margin-right:8px">'+statusLabel+'</span>';

  // Show assignee info
  if(wst.assignedTo){
    var assigneeName=wst.assignedTo;
    for(var ui=0;ui<(allUsers||[]).length;ui++){if(allUsers[ui].user_id===wst.assignedTo){assigneeName=allUsers[ui].display_name||allUsers[ui].email||wst.assignedTo;break}}
    html+='<span style="font-size:11px;color:var(--text3);margin-right:8px">👤 '+h(assigneeName)+'</span>';
  }

  // Action buttons based on status
  if(!isDone&&hasPerm){
    if(status==='pending_approval'){
      // Pending approval - show approver buttons if current user is approver
      if(isApprover){
        html+='<button class="btn-save" onclick="wfApprove('+seq+')">✅ 通过</button>';
        html+='<button class="btn-save" style="background:#f44336" onclick="wfReject('+seq+')">↩️ 驳回</button>';
      }else{
        html+='<span style="font-size:11px;color:var(--warning)">已提交审批，等待处理...</span>';
      }
    }else{
      // Active - show submit/save and mark-done buttons
      html+='<button class="btn-sm btn-sm-primary" onclick="saveStepNote()">💾 保存备注</button>';
      if(step&&step.approval&&step.approval.enabled){
        html+='<button class="btn-save" onclick="markStepDone()">📨 提交审批</button>';
      }else{
        html+='<button class="btn-save" onclick="markStepDone()">✓ 标记完成</button>';
      }
    }
  }else if(!isDone&&!hasPerm){
    html+='<span style="font-size:11px;color:var(--text3)">🔒 仅可查看</span>';
  }
  html+='<span style="font-size:11px;color:var(--text3);margin-left:8px" id="wb-save-status"></span>';
  container.innerHTML=html;
}
function wfIsApprover(seq){
  if(isSuperAdmin)return true;
  var step=WFLookup[seq];if(!step)return false;
  if(step.approval&&step.approval.enabled&&step.approval.approverRole){
    return currentUserRole===step.approval.approverRole;
  }
  return false;
}
...
function loadPaymentsTo(listId){
  var list=document.getElementById(listId);if(!list)return;
  if(!allPayments.length){list.innerHTML='<div style="text-align:center;padding:20px;color:var(--text2)">暂无收款记录，点右上角 + 新建</div>';loadProjectPayments();return}
...
function renderPaymentListTo(listId){
  ...
  for(var i=0;i<allPayments.length;i++){
    var p=allPayments[i];
    s+='<div class="delivery-item" style="cursor:pointer" onclick="wfOpenPaymentForm(\''+p.id+'\')">';
...
async function loadProjects(){
  ...
  try {
    ...
    if(error){list.innerHTML='<div style="text-align:center;padding:40px;color:#e74c3c">加载失败: '+h(error.message)+'。请确认 projects 表是否已在 Supabase 中创建（执行 project_management.sql）</div>';return}
    allProjects=data||[];
    renderProjects();
  } catch(e) {
    list.innerHTML='<div style="text-align:center;padding:40px;color:#e74c3c">加载异常: '+h(e.message)+'</div>';
    console.error(e);
  }
}
...
function renderProjects(){
  ...
  if(projectStateFilter){filtered=allProjects.filter(function(p){return (p.current_state||'doing')===projectStateFilter})}
  if(!filtered.length){list.innerHTML='<div style="text-align:center;padding:40px;color:var(--text2)">'+({todo:'无待办项目',doing:'无在办项目',done:'无办结项目'}[projectStateFilter]||'无项目')+'</div>';return}
  var s='';
  var statusMap={planning:'规划中',in_progress:'进行中',completed:'已完成',suspended:'已暂停'};
  var stateMap={todo:'⏳ 待办',doing:'🔄 在办',done:'✅ 办结'};
  for(var i=0;i<filtered.length;i++){
    var p=filtered[i];
    var client=allClients.find(function(c){return c.id===p.client_id});
    var cn=client?client.name:'未关联';
    var cs=p.current_state||'doing';
    ...
    s+='<div class="project-card" onclick="renderProjectDetail(\''+p.id+'\')">';
    s+='<div class="project-card-header"><span class="project-card-name">'+h(p.name)+'</span><span class="project-card-status status-'+p.status+'">'+(statusMap[p.status]||p.status)+'</span><span style="font-size:11px;color:var(--text2);margin-left:4px">'+(stateMap[cs]||cs)+'</span></div>';
...
function filterProjectsByState(state){
  ...
  document.querySelectorAll('#project-list-panel .pd-status-tab').forEach(function(b){b.classList.toggle('active',b.dataset.status===state)});
  renderProjects();
}
...
async function loadStages(){
  ...
  try{
    ...
    if(error){list.innerHTML='<div style="padding:20px;color:#e74c3c">'+h(error.message)+'</div>';return}
    allStages=data||[];
    renderStages(list);
...
function openProjectFormFromLeadIdx(idx){
...
async function openProjectFormFromLead(leadId,projectName,companyName,clientId){
  oppLeadId=leadId||null;
  oppEditId=null;
  var title=document.getElementById('opp-form-title');
  if(title)title.textContent='新建商机';
  ...
  pfContactsData=[];renderPfContacts();
  ...
  (async function(){
    ...
    if(!client&&clientId){
      try{var {data}=await sb.from('clients').select('id,name,industry,address,type,credit_code,company_info').eq('id',clientId).maybeSingle();if(data)client=data}catch(e){}
    }
    if(client){
      ...
      var cc=client.credit_code||'';if(!cc){try{var ci=typeof client.company_info==='string'?JSON.parse(client.company_info):(client.company_info||{});cc=ci.credit_code||'';}catch(e){}}if(cc)document.getElementById('pf-credit-code').value=cc;
      ...
      // Load contacts from contacts table and populate pfContactsData
      console.log('[openProjectFormFromLead] loading contacts for clientId=',clientId);
      try{
        var {data:cts,error:ctErr}=await sb.from('contacts').select('name,title,phone,email').eq('client_id',clientId);
        if(ctErr){console.error('[openProjectFormFromLead] contacts query error',JSON.stringify(ctErr));return}
        console.log('[openProjectFormFromLead] contacts loaded',{count:cts?cts.length:0,data:cts});
        if(cts&&cts.length){
          for(var ci=0;ci<cts.length;ci++){
            var ct=cts[ci];
            var ph=ct.phone||'';if(typeof ph==='string'){try{var arr=JSON.parse(ph);ph=Array.isArray(arr)?arr:[ph];}catch(e){}}
            pfContactsData.push({name:ct.name||'',title:ct.title||'',phone:ph||'',email:ct.email||''});
          }
          renderPfContacts();
        }
      }catch(e){}
  ...
  if(!allClients||!allClients.length){
    (async function(){
      try{var {data}=await sb.from('clients').select('id,name,industry,address,type,credit_code').eq('company_id',currentCompanyId);if(data)allClients=data;}catch(e){}
    })();
  }
  // Populate template dropdown
  var templateSel=document.getElementById('pf-template-id');
...
function openProjectForm(id,leadId){
  oppEditId=id||null;
  var title=document.getElementById('opp-form-title');
  ...
  if(id){
    ...
    if(p){
      ...
      // Restore contacts
      pfContactsData=[];
      if(p.contacts_json){try{pfContactsData=JSON.parse(p.contacts_json)}catch(e){}}else if(p.contact_phone){try{var phones=JSON.parse(p.contact_phone);var names=(p.contact_name||'').split(', ');var titles=(p.contact_title||'').split(', ');for(var ci=0;ci<Math.max(phones.length,names.length,titles.length);ci++)pfContactsData.push({name:names[ci]||'',title:titles[ci]||'',phone:phones[ci]||'',email:''})}catch(e){}}
      renderPfContacts();
    }
  }
  // Populate template dropdown
...
function onCompSearchInput(){
  ...
  var s='';
  for(var i=0;i<matches.length;i++){
    s+='<div class="name-suggestion-item" onclick="selectCompClient('+JSON.stringify(matches[i].id)+','+JSON.stringify(matches[i].name)+')">'+h(matches[i].name)+'</div>';
  }
  sug.innerHTML=s;sug.classList.remove('hidden');
}
...
function completeTraining(){
  ...
  curWorkflowState[6]=curWorkflowState[6]||{status:'active'};
  curWorkflowState[6].status='done';
  curStep=7;
  wfActivateStep(7);
  saveWorkflow();
  renderWorkbench();
  showToast('✓ 培训完成，已跳转步骤7');
  console.log('[completeTraining] advanced to step 7');
}
...
function wfOpenStageForm(id){showToast('阶段管理')}
...
function wfOpenPaymentForm(id){if(curProjectId)showToast('收款表单')}
function wfDeleteStage(id){if(confirm('确定删除？')){sb.from('project_stages').delete().eq('id',id).then(function(r){if(!r.error){loadStages();showToast('已删除')}else showToast(r.error.message)})}}
async function saveProject(){
  var name=document.getElementById('pf-name').value.trim();
  if(!name){showToast('请输入商机名称');return}
  var conList=[];
  for(var ci=0;ci<pfContactsData.length;ci++){
    var ct=pfContactsData[ci];
    if(ct.name||ct.phone)conList.push({name:ct.name||'',title:ct.title||'',phone:ct.phone||'',email:ct.email||''});
  }
  var proj={
    name:name,
    ...
    contact_name:conList.length>0?conList.map(function(x){return x.name}).join(', '):'',
    contact_title:conList.length>0?conList.map(function(x){return x.title}).join(', '):'',
    contact_phone:conList.length>0?JSON.stringify(conList.map(function(x){return x.phone})):'[]',
    contacts_json:conList.length>0?JSON.stringify(conList):null,
    ...
    status:document.getElementById('pf-status').value,
    conclusion:document.getElementById('pf-conclusion').value,
  ...
  if(!oppEditId){
    ...
    proj.workflow='{}';proj.current_step=1;proj.template_id=document.getElementById('pf-template-id')?(parseInt(document.getElementById('pf-template-id').value)||null):null;
    // Initialize workflow_state from template
    if(proj.template_id){
      var tmplSteps=getTemplateSteps(proj.template_id);
      var wfStateInit={};
      for(var wsi=0;wsi<tmplSteps.length;wsi++){
        var wfSeq=tmplSteps[wsi].seq;
        wfStateInit[wfSeq]={status:wfSeq===1?'active':'pending',assignedTo:null,approvedBy:null,approvedAt:null};
      }
      proj.workflow_state=JSON.stringify(wfStateInit);
    }
  ...
  var _leadId = oppLeadId;
  closeOppForm();
  await loadProjects();
  // If created from lead, mark lead as converted
  if(_leadId&&!oppEditId){
    try{await sb.from('lead_pool').update({opportunity_id:proj.bus_id,status:'converted'}).eq('id',_leadId);console.log('[saveProject] lead',_leadId,'converted')}catch(e){console.error('[saveProject] lead convert error',e)}
  }
  await loadLeads();
...
function addPfContact(){pfContactsData.push({name:'',title:'',phone:'',email:''});renderPfContacts();}
function rmPfContact(idx){pfContactsData.splice(idx,1);renderPfContacts();}
function upPfContact(idx,field,val){pfContactsData[idx][field]=val;}
function renderPfContacts(){
  var el=document.getElementById('pf-contacts-container');
  if(!el)return;
  if(!pfContactsData.length){el.innerHTML='<div style="font-size:12px;color:var(--text3);padding:4px 0">暂无联系人</div>';return}
  var h='';
  for(var ci=0;ci<pfContactsData.length;ci++){
    var ct=pfContactsData[ci];
    h+='<div style="display:flex;gap:6px;align-items:center;margin-bottom:6px;flex-wrap:wrap">';
    h+='<input value="'+escHtml(ct.name||'')+'" oninput="upPfContact('+ci+',\'name\',this.value)" placeholder="姓名" style="flex:1;min-width:80px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;font-size:13px">';
    h+='<select onchange="upPfContact('+ci+',\'title\',this.value)" style="min-width:80px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;font-size:13px"><option value="">职位</option><option value="采购"'+(ct.title==='采购'?' selected':'')+'>采购</option><option value="总经理"'+(ct.title==='总经理'?' selected':'')+'>总经理</option><option value="技术负责人"'+(ct.title==='技术负责人'?' selected':'')+'>技术负责人</option><option value="行政"'+(ct.title==='行政'?' selected':'')+'>行政</option><option value="IT"'+(ct.title==='IT'?' selected':'')+'>IT</option><option value="财务"'+(ct.title==='财务'?' selected':'')+'>财务</option><option value="其他"'+(ct.title==='其他'?' selected':'')+'>其他</option></select>';
    h+='<input value="'+escHtml(ct.phone||'')+'" oninput="upPfContact('+ci+',\'phone\',this.value)" placeholder="电话" style="flex:1;min-width:100px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;font-size:13px">';
    h+='<button type="button" onclick="rmPfContact('+ci+')" style="padding:4px 8px;background:var(--danger);color:#fff;border:none;border-radius:6px;font-size:12px;cursor:pointer">✕</button>';
    h+='</div>';
  }
  el.innerHTML=h;
}

var _leadProjectCache=[];
...
// saveStage removed (duplicate empty stub overrides real impl at L9273)
async function saveBidding(){}
async function saveDelivery(){}
// savePayment removed (duplicate empty stub overrides real impl at L4684)
...
async function deletePayment(id){if(confirm('确定删除？')){var {error}=await sb.from('project_payments').delete().eq('id',id);if(error){showToast(error.message);return}loadProjectPayments();showToast('已删除')}}

// ===== My Office =====
var _moCurrentTab='overview';
function switchMyOfficeTab(tab){
  // Legacy stub - tabs merged into collab-view
  switchCollabTab(tab==='overview'?'mo-overview':tab==='docs'?'mo-docs':tab);
}async function renderMyOfficeOverview(){
  console.log("[DEBUG mo-overview] START currentCompanyId:"+currentCompanyId+" currentUser:"+(currentUser?currentUser.id:"null"));
  var el=document.getElementById('mo-overview-content');
  if(!currentCompanyId){el.innerHTML='<div class="empty">请先选择公司</div>';return}
  var myName=currentUser&&currentUser.display_name?currentUser.display_name:(currentUser&&currentUser.email?currentUser.email:'同事');
  var today=new Date();
  var dateStr=today.getFullYear()+'年'+(today.getMonth()+1)+'月'+today.getDate()+'日';
  var weekDays=['日','一','二','三','四','五','六'];
  var weekDay=weekDays[today.getDay()];
  var hour=today.getHours();
  var greet=hour<9?'早上好':hour<12?'上午好':hour<14?'中午好':hour<18?'下午好':'晚上好';
  
  var h='';
  h+='<div class="mo-greeting">'+greet+'，'+escHtml(myName)+' 👋</div>';
  h+='<div class="mo-date">📅 '+dateStr+' 星期'+weekDay+'</div>';
  
  // Quick actions
  h+='<div class="mo-quick-actions">';
  h+='<div class="mo-quick-btn" onclick="switchMyOfficeTab(\'todo\');openMoTaskForm()"><span class="icon">✅</span>新建任务</div>';
  h+='<div class="mo-quick-btn" onclick="switchMyOfficeTab(\'approvals\');openMoApprovalForm()"><span class="icon">📋</span>发起审批</div>';
  h+='<div class="mo-quick-btn" onclick="switchMyOfficeTab(\'docs\');openMoDocForm()"><span class="icon">📄</span>写文档</div>';
  h+='<div class="mo-quick-btn" onclick="switchTab(\'collab\');setTimeout(function(){switchCollabTab(\'schedule\')},200)"><span class="icon">📅</span>查看日程</div>';
  h+='</div>';
  
  // Stats row
  h+='<div class="mo-overview-grid" style="margin-bottom:12px">';
  h+='<div class="mo-stat-card" onclick="switchMyOfficeTab(\'todo\')"><div class="num" id="mo-stat-todo">-</div><div class="label">待办任务</div></div>';
  h+='<div class="mo-stat-card" onclick="switchMyOfficeTab(\'approvals\')"><div class="num" id="mo-stat-pending-approvals">-</div><div class="label">待处理审批</div></div>';
  h+='<div class="mo-stat-card" onclick="switchMyOfficeTab(\'messages\')"><div class="num" id="mo-stat-unread">-</div><div class="label">未读消息</div></div>';
  h+='<div class="mo-stat-card" onclick="switchMyOfficeTab(\'todo\')"><div class="num" id="mo-stat-overdue">-</div><div class="label">已逾期</div></div>';
  h+='</div>';
  
  // Today's todos summary + schedules
  h+='<div class="mo-overview-grid" style="margin-bottom:12px">';
  h+='<div class="mo-section"><div class="mo-section-title">📝 今日待办 (前5条)</div><div id="mo-overview-todos">加载中...</div></div>';
  h+='<div class="mo-section"><div class="mo-section-title">📅 日程安排</div><div id="mo-overview-schedules">加载中...</div></div>';
  h+='</div>';
  
  // Announcements
  h+='<div class="mo-section"><div class="mo-section-title">📢 公司公告</div><div id="mo-overview-announcements">加载中...</div></div>';
  
  el.innerHTML=h;
  
  // Load data
  if(typeof loadTasks!=='undefined'){
    console.log('[DEBUG mo-overview] querying tasks...');
    var {data:tasks}=await sb.from('tasks').select('*').eq('company_id',currentCompanyId).in('status',['todo','in_progress']).order('due_date',{ascending:true,nullsFirst:false});
    console.log('[DEBUG mo-overview] querying approvals...');
    var {data:approvals}=await sb.from('approval_requests').select('*').eq('company_id',currentCompanyId).eq('status','pending').order('created_at',{ascending:false});
    var pendingCount=0;
    if(approvals)for(var a=0;a<approvals.length;a++){if(approvals[a].requestor_id!==currentUser.id)pendingCount++;}
    // Get unread messages count from dept_messages
    var unreadCount=0;
    try{var {count:uc}=await sb.from('dept_messages').select('*',{count:'exact',head:true}).eq('company_id',currentCompanyId).order('created_at',{ascending:false}).limit(1);/* we count recent messages as "unread" if they exist */if(uc>0)unreadCount=1;}catch(e){}
    
    document.getElementById('mo-stat-todo').textContent=tasks?tasks.length:0;
    document.getElementById('mo-stat-pending-approvals').textContent=pendingCount;
    document.getElementById('mo-stat-unread').textContent=unreadCount;
    
    var overdueCount=0;
    var todayStr=today.toISOString().slice(0,10);
    if(tasks){for(var t=0;t<tasks.length;t++){if(tasks[t].due_date&&tasks[t].due_date<todayStr)overdueCount++;}}
    document.getElementById('mo-stat-overdue').textContent=overdueCount;
    ...
    if(tasks&&tasks.length){
      ...
      for(var i=0;i<top5.length;i++){
        ...
        var overdue=tk.due_date&&tk.due_date<todayStr;
        todoHtml+='<div class="mo-task-item priority-'+prio+'">';
        todoHtml+='<div class="mo-task-title">'+escHtml(tk.title)+'</div>';
        todoHtml+='<div class="mo-task-meta">';
        if(tk.due_date)todoHtml+='<span style="color:'+(overdue?'var(--danger)':'var(--text2)')+'">📅 '+tk.due_date+(overdue?' ⚠️':'')+'</span>';
        todoHtml+='<span>'+({todo:'待办',in_progress:'进行中'}[tk.status]||tk.status)+'</span>';
        todoHtml+='</div></div>';
      }
    }else{todoHtml='<div style="color:var(--text3);font-size:13px;padding:8px 0">暂无待办 ✅</div>'}
    document.getElementById('mo-overview-todos').innerHTML=todoHtml;
  }
  
  // Load schedules for today + tomorrow
  try{
    var todayISO=today.toISOString().slice(0,10);
    var tomorrow=new Date(today);tomorrow.setDate(today.getDate()+1);
    var tomorrowISO=tomorrow.toISOString().slice(0,10);
    console.log('[DEBUG mo-overview] querying schedules...');
    var {data:schedules}=await sb.from('schedules').select('*').eq('company_id',currentCompanyId).gte('start_time',todayISO).lte('start_time',tomorrowISO+'T23:59:59Z').order('start_time');
    var schHtml='';
    if(schedules&&schedules.length){
      for(var s=0;s<schedules.length;s++){
        var sc=schedules[s];
        var st=new Date(sc.start_time);
        var timeStr=sc.all_day?'全天':st.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'});
        schHtml+='<div class="mo-timeline-item">';
        schHtml+='<div class="mo-tl-dot"></div>';
        schHtml+='<div class="mo-tl-time">'+timeStr+'</div>';
        schHtml+='<div class="mo-tl-content">'+escHtml(sc.title)+(sc.location?' 📍'+escHtml(sc.location):'')+'</div>';
        schHtml+='</div>';
      }
    }else{schHtml='<div style="color:var(--text3);font-size:13px;padding:8px 0">暂无日程</div>'}
    document.getElementById('mo-overview-schedules').innerHTML=schHtml;
  }catch(e){document.getElementById('mo-overview-schedules').innerHTML='<div style="color:var(--text3);font-size:13px">加载失败</div>'}
  
  // Load announcements
  try{
    console.log('[DEBUG mo-overview] querying announcements...');
    var {data:anns}=await sb.from('announcements').select('*').eq('company_id',currentCompanyId).order('created_at',{ascending:false}).limit(3);
    var annHtml='';
    if(anns&&anns.length){
      for(var an=0;an<anns.length;an++){
        var ann=anns[an];
        annHtml+='<div class="mo-announcement'+(ann.priority==='urgent'?' urgent':'')+'">';
        annHtml+='<div class="mo-ann-title">'+escHtml(ann.title)+'</div>';
        annHtml+='<div class="mo-ann-meta">'+new Date(ann.created_at).toLocaleDateString('zh-CN')+' · '+(ann.priority==='urgent'?'🔥 紧急':ann.priority==='normal'?'普通':'低')+'</div>';
        annHtml+='<div class="mo-ann-body">'+(ann.content.length>80?escHtml(ann.content.slice(0,80))+'...':escHtml(ann.content))+'</div>';
        annHtml+='</div>';
      }
    }else{annHtml='<div style="color:var(--text3);font-size:13px;padding:8px 0">暂无公告</div>'}
    document.getElementById('mo-overview-announcements').innerHTML=annHtml;
  }catch(e){document.getElementById('mo-overview-announcements').innerHTML='<div style="color:var(--text3);font-size:13px">暂未开放</div>'}
}

/* ===== Todo Panel ===== */
var _moTaskFilter='all';
async function renderMyOfficeTodo(filter){
  // My todos now handled by collab task panel
  switchCollabTab("task");
}function openMoTaskForm(id){
  var t=id?allTasks.find(function(x){return x.id===id}):null;
  var h='<div class="modal-overlay" id="mo-task-modal"><div class="modal-sheet"><h3>'+(t?'编辑任务':'新建任务')+'</h3>';
  h+='<div class="form-group"><label>标题 *</label><input id="mo-ta-title" value="'+(t?escAttr(t.title):'')+'"></div>';
  h+='<div class="form-group"><label>描述</label><textarea id="mo-ta-desc" rows="2">'+(t?escHtml(t.description||''):'')+'</textarea></div>';
  h+='<div class="form-row"><div class="form-group"><label>优先级</label><select id="mo-ta-priority"><option value="urgent"'+(t&&t.priority==='urgent'?' selected':'')+'>紧急</option><option value="high"'+(t&&t.priority==='high'?' selected':'')+'>高</option><option value="normal"'+(t&&t.priority==='normal'?' selected':'')+'>普通</option><option value="low"'+(t&&t.priority==='low'?' selected':'')+'>低</option></select></div><div class="form-group"><label>截止日期</label><input type="date" id="mo-ta-due" value="'+(t&&t.due_date?t.due_date:'')+'"></div></div>';
  h+='<div class="form-row"><div class="form-group"><label>状态</label><select id="mo-ta-status"><option value="todo"'+(t&&t.status==='todo'?' selected':'')+'>待办</option><option value="in_progress"'+(t&&t.status==='in_progress'?' selected':'')+'>进行中</option><option value="done"'+(t&&t.status==='done'?' selected':'')+'>已完成</option></select></div></div>';
  h+='<div class="modal-actions"><button class="btn-cancel" onclick="this.closest(\'.modal-overlay\').remove()">取消</button><button class="btn-save" onclick="saveMoTask(\''+(t?t.id:'')+'\')">保存</button></div></div></div>';
  document.body.insertAdjacentHTML('beforeend',h);
}

async function saveMoTask(id){
  var title=document.getElementById('mo-ta-title').value.trim();
  if(!title){showToast('请输入标题');return}
  var desc=document.getElementById('mo-ta-desc').value.trim();
  var priority=document.getElementById('mo-ta-priority').value;
  var due=document.getElementById('mo-ta-due').value||null;
  var status=document.getElementById('mo-ta-status').value;
  var obj={title:title,description:desc||null,priority:priority,due_date:due,status:status,assigned_to:currentUser.id,assigned_by:currentUser.id,company_id:currentCompanyId};
  if(status==='done')obj.completed_at=new Date().toISOString();
  var err;
  if(id){var r=await sb.from('tasks').update(obj).eq('id',id);err=r.error}
  else{var r2=await sb.from('tasks').insert(obj);err=r2.error}
  var modalEl=document.getElementById('mo-task-modal');if(modalEl)modalEl.remove();
  if(err){showToast('保存失败: '+err.message);return}
  renderMyOfficeTodo(_moTaskFilter||'all');showToast('已保存');
  if(_moCurrentTab==='overview')renderMyOfficeOverview();
}

async function toggleMoTaskDone(taskId,cb){
  var newStatus=cb.checked?'done':'todo';
  var obj={status:newStatus};
  if(newStatus==='done')obj.completed_at=new Date().toISOString();
  else obj.completed_at=null;
  var {error}=await sb.from('tasks').update(obj).eq('id',taskId);
  if(error){showToast('操作失败: '+error.message);cb.checked=!cb.checked;return}
  showToast(cb.checked?'已完成':'已恢复待办');
  if(_moCurrentTab==='overview')renderMyOfficeOverview();
  else renderMyOfficeTodo(_moTaskFilter||'all');
}

async function deleteMoTask(id){
  if(!confirm('确定删除该任务?'))return;
  await sb.from('tasks').delete().eq('id',id);
  renderMyOfficeTodo(_moTaskFilter||'all');showToast('已删除');
  if(_moCurrentTab==='overview')renderMyOfficeOverview();
}

/* ===== Approvals Panel ===== */
async function renderMyOfficeApprovals(){
  // My approvals now handled by collab approval panel
  console.log("renderMyOfficeApprovals: redirecting to collab approval tab");
  switchCollabTab("approval");
}function openMoApprovalForm(){
  var types=['报价审批','合同审批','折扣审批','出差申请','报销申请','客户转移','投标材料审批','公章使用申请','项目立项审批','交付验收审批','自定义'];
  var h='<div class="modal-overlay" id="mo-approval-modal"><div class="modal-sheet"><h3>发起审批</h3>';
  h+='<div class="form-group"><label>审批类型</label><select id="mo-ap-type">';
  for(var i=0;i<types.length;i++)h+='<option value="'+types[i]+'">'+types[i]+'</option>';
  h+='</select></div>';
  h+='<div class="form-group"><label>标题 *</label><input id="mo-ap-title" placeholder="审批标题"></div>';
  h+='<div class="form-group"><label>内容说明</label><textarea id="mo-ap-content" rows="3" placeholder="请输入审批内容和理由..."></textarea></div>';
  h+='<div class="form-group"><label>审批人</label><select id="mo-ap-approver"><option value="">任意审批人</option>'+(()=>{var o='';for(var k=0;k<allUsers.length;k++){var uu=allUsers[k];if(uu.user_id!==currentUser.id)o+='<option value="'+uu.user_id+'">'+(uu.display_name||uu.email||uu.user_id)+'</option>'}return o})()+'</select></div>';
  h+='<div class="form-group"><label>紧急程度</label><select id="mo-ap-urgency"><option value="normal">普通</option><option value="urgent">紧急</option></select></div>';
  h+='<div class="modal-actions"><button class="btn-cancel" onclick="this.closest(\'.modal-overlay\').remove()">取消</button><button class="btn-save" onclick="saveMoApproval()">提交</button></div></div></div>';
  document.body.insertAdjacentHTML('beforeend',h);
}

async function saveMoApproval(){
  var type=document.getElementById('mo-ap-type').value;
  var title=document.getElementById('mo-ap-title').value.trim();
  var content=document.getElementById('mo-ap-content').value.trim();
  if(!title){showToast('请输入标题');return}
  var approver=document.getElementById('mo-ap-approver').value||null;
  var priority=document.getElementById('mo-ap-urgency').value;
  var {error}=await sb.from('approval_requests').insert({company_id:currentCompanyId,requestor_id:currentUser.id,title:title,type:type,content:{detail:content||'',priority:priority},status:'pending',total_steps:1});
  var modalEl=document.getElementById('mo-approval-modal');if(modalEl)modalEl.remove();
  if(error){showToast('提交失败: '+error.message);return}
  renderMyOfficeApprovals();showToast('已提交');
}

async function moApproveRequest(id,action){
  var remark=prompt(action==='approved'?'通过备注 (可选):':'驳回原因:');
  var newStatus=action==='approved'?'approved':'rejected';
  var {error}=await sb.from('approval_requests').update({status:newStatus,updated_at:new Date().toISOString()}).eq('id',id);
  if(error){showToast('操作失败: '+error.message);return}
  try{await sb.from('approval_actions').insert({company_id:currentCompanyId,request_id:id,approver_id:currentUser.id,action:action,step:1,remark:remark||null})}catch(e){}
  renderMyOfficeApprovals();showToast(action==='approved'?'已通过':'已驳回');
  if(_moCurrentTab==='overview')renderMyOfficeOverview();
}

/* ===== Messages Panel ===== */
async function loadMoMessages(){
  // Messages now handled by collab msg panel
  switchCollabTab("msg");
}async function sendMoMessage(){
  var input=document.getElementById('mo-msg-input');
  var content=input.value.trim();
  if(!content){showToast('请输入消息');return}
  if(!currentCompanyId){showToast('请先选择公司');return}
  var deptFilter=document.getElementById('mo-msg-dept-filter').value;
  var {error}=await sb.from('dept_messages').insert({company_id:currentCompanyId,sender_id:currentUser.id,content:content,dept_id:deptFilter||null,created_at:new Date().toISOString()});
  if(error){showToast('发送失败: '+error.message);return}
  input.value='';
  loadMoMessages();
}

async function loadMoAnnouncements(){
  var el=document.getElementById('mo-announcements-list');
  if(!currentCompanyId){el.innerHTML='<div class="empty">请先选择公司</div>';return}
  var {data}=await sb.from('announcements').select('*').eq('company_id',currentCompanyId).order('created_at',{ascending:false}).limit(10);
  if(!data||!data.length){el.innerHTML='<div style="color:var(--text3);font-size:13px;padding:8px 0">暂无公告</div>';return}
  var h='';
  for(var i=0;i<data.length;i++){
    var ann=data[i];
    h+='<div class="mo-announcement'+(ann.priority==='urgent'?' urgent':'')+'">';
    h+='<div class="mo-ann-title">'+escHtml(ann.title)+'</div>';
    h+='<div class="mo-ann-meta">'+new Date(ann.created_at).toLocaleDateString('zh-CN')+' · '+(ann.priority==='urgent'?'🔥 紧急':ann.priority==='normal'?'普通':'低')+'</div>';
    h+='<div class="mo-ann-body">'+escHtml(ann.content)+'</div>';
    h+='</div>';
  }
  el.innerHTML=h;
}
...
/* ===== Docs Panel ===== */
var _moDocFilter='all';
async function renderMyOfficeDocs(filter){
  console.log("[DEBUG mo-docs] START filter:"+filter+" currentCompanyId:"+currentCompanyId+" currentUser:"+(currentUser?currentUser.id:"null"));
  _moDocFilter=filter;
  var el=document.getElementById('mo-docs-list');
  if(!currentCompanyId){el.innerHTML='<div class="empty">请先选择公司</div>';return}
  var fb=document.querySelectorAll('#mo-doc-filters button');
  for(var i=0;i<fb.length;i++){fb[i].classList.remove('active');if(fb[i].getAttribute('data-cat')===filter)fb[i].classList.add('active')}
  console.log('[DEBUG mo-docs] querying my_documents...');
  var q=sb.from('my_documents').select('*').eq('user_id',currentUser.id).order('is_pinned',{ascending:false}).order('updated_at',{ascending:false});
  if(filter!=='all')q=q.eq('category',filter);
  var {data}=await q;
  if(!data||!data.length){el.innerHTML='<div class="empty" style="padding:20px">暂无文档</div>';return}
  var h='';
  for(var i=0;i<data.length;i++){
    var d=data[i];
    var tagsHtml='';
    if(d.tags&&d.tags.length){for(var t=0;t<d.tags.length;t++)tagsHtml+='<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:var(--primary-light);color:var(--primary);margin-right:4px">'+escHtml(d.tags[t])+'</span>'}
    h+='<div class="mo-doc-card'+(d.is_pinned?' pinned':'')+'">';
    h+='<div class="title">'+(d.is_pinned?'📌 ':'')+escHtml(d.title)+'</div>';
    if(d.content)h+='<div style="font-size:12px;color:var(--text2);margin:6px 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(d.content.length>100?escHtml(d.content.slice(0,100))+'...':escHtml(d.content))+'</div>';
    h+='<div class="meta">';
    h+='<span>📁 '+escHtml(d.category||'general')+'</span>';
    h+='<span>🕐 '+new Date(d.updated_at).toLocaleDateString('zh-CN')+'</span>';
    if(tagsHtml)h+=tagsHtml;
    h+='</div>';
    h+='<div class="actions">';
    if(d.file_url)h+='<a href="'+escAttr(d.file_url)+'" target="_blank" class="btn-sm" style="font-size:11px;padding:2px 8px;text-decoration:none;display:inline-block">📎 '+(escHtml(d.file_name||d.file_url.split('/').pop()||'文件'))+'</a>';
    h+='<button class="btn-sm" style="font-size:11px;padding:2px 8px" onclick="openMoDocForm(\''+d.id+'\')">编辑</button>';
    h+='<button class="btn-sm" style="font-size:11px;padding:2px 8px;color:var(--danger)" onclick="deleteMoDoc(\''+d.id+'\')">删除</button>';
    h+='</div></div>';
  }
  el.innerHTML=h;
}

function openMoDocForm(id){
  var d=id?allMyDocs.find(function(x){return x.id===id}):null;
  var h='<div class="modal-overlay" id="mo-doc-modal"><div class="modal-sheet"><h3>'+(d?'编辑文档':'新建文档')+'</h3>';
  h+='<div class="form-group"><label>标题 *</label><input id="mo-doc-title" value="'+(d?escAttr(d.title):'')+'"></div>';
  h+='<div class="form-group"><label>内容</label><textarea id="mo-doc-content" rows="5" placeholder="文档内容...">'+(d?escHtml(d.content||''):'')+'</textarea></div>';
  h+='<div class="form-row"><div class="form-group"><label>分类</label><select id="mo-doc-category"><option value="general"'+(d&&d.category==='general'?' selected':'')+'>通用</option><option value="work"'+(d&&d.category==='work'?' selected':'')+'>工作</option><option value="template"'+(d&&d.category==='template'?' selected':'')+'>模板</option><option value="personal"'+(d&&d.category==='personal'?' selected':'')+'>个人</option></select></div><div class="form-group"><label>附件</label><input type="file" id="mo-doc-file" style="padding:6px;display:block;width:100%;font-size:13px;border:1px solid var(--border);border-radius:6px">'+(d&&d.file_url?'<div style="font-size:11px;color:var(--text2);margin-top:4px">当前: '+escHtml(d.file_url.split('/').pop()||d.file_name||d.file_url)+'</div>':'')+'</div></div>';
  h+='<div class="form-group"><label>标签 (逗号分隔)</label><input id="mo-doc-tags" value="'+(d&&d.tags?d.tags.join(','):'')+'" placeholder="如: 合同, 模板"></div>';
  h+='<div class="form-group"><label><input type="checkbox" id="mo-doc-pinned" '+(d&&d.is_pinned?'checked':'')+'> 置顶</label></div>';
  h+='<div class="modal-actions"><button class="btn-cancel" onclick="this.closest(\'.modal-overlay\').remove()">取消</button><button class="btn-save" onclick="saveMoDoc(\''+(d?d.id:'')+'\')">保存</button></div></div></div>';
  document.body.insertAdjacentHTML('beforeend',h);
}

async function saveMoDoc(id){
  var title=document.getElementById('mo-doc-title').value.trim();
  if(!title){showToast('请输入标题');return}
  var content=document.getElementById('mo-doc-content').value.trim();
  var category=document.getElementById('mo-doc-category').value;
  var tagsRaw=document.getElementById('mo-doc-tags').value.trim();
  var tags=tagsRaw?tagsRaw.split(',').map(function(s){return s.trim()}).filter(Boolean):[];
  var isPinned=document.getElementById('mo-doc-pinned').checked;
  // File upload
  var fileInput=document.getElementById('mo-doc-file');
  var fileUrl=null;var fileName=null;
  if(fileInput&&fileInput.files&&fileInput.files[0]){
    var f=fileInput.files[0];
    var bucket='mo-documents';
    var filePath=currentUser.id+'/'+Date.now()+'_'+f.name.replace(/[^a-zA-Z0-9._-]/g,'_');
    try{
      var upRes=await sb.storage.from(bucket).upload(filePath,f,{cacheControl:'3600',upsert:false});
      if(upRes.error){showToast('文件上传失败: '+upRes.error.message);return}
      var urlRes=sb.storage.from(bucket).getPublicUrl(filePath);
      fileUrl=urlRes.data.publicUrl;
      fileName=f.name;
    }catch(e){showToast('文件上传异常: '+e.message);return}
  }
  var obj={title:title,content:content||null,category:category,file_url:fileUrl,tags:tags.length?tags:null,is_pinned:isPinned,updated_at:new Date().toISOString()};
  if(fileName)obj.file_name=fileName;
  var err;
  if(id){var r=await sb.from('my_documents').update(obj).eq('id',id);err=r.error}
  else{obj.user_id=currentUser.id;obj.company_id=currentCompanyId;var r2=await sb.from('my_documents').insert(obj);err=r2.error}
  var modalEl=document.getElementById('mo-doc-modal');if(modalEl)modalEl.remove();
  if(err){showToast('保存失败: '+err.message);return}
  renderMyOfficeDocs(_moDocFilter||'all');showToast('已保存');
}

async function deleteMoDoc(id){
  if(!confirm('确定删除该文档?'))return;
  await sb.from('my_documents').delete().eq('id',id);
  renderMyOfficeDocs(_moDocFilter||'all');showToast('已删除');
}

var allMyDocs=[];

// ═══════════════════════════════════════════
//  Analytics Module — 经营分析
// ═══════════════════════════════════════════
var _anCurrentTab='overview';

function switchAnalyticsTab(tab){
  console.log('[Analytics] switching to',tab);
  _anCurrentTab=tab;
  var btns=document.querySelectorAll('.analytics-tabs button');
  for(var i=0;i<btns.length;i++)btns[i].classList.remove('active');
  var panels=document.querySelectorAll('.an-panel');
  for(var j=0;j<panels.length;j++)panels[j].classList.add('hidden');
  var panel=document.getElementById('an-'+tab);
  if(panel)panel.classList.remove('hidden');
  for(var k=0;k<btns.length;k++){
    if(btns[k].getAttribute('onclick')&&btns[k].getAttribute('onclick').indexOf("'"+tab+"'")>=0){
  ...
  switch(tab){
    ...
    case 'sales-funnel':renderAnSalesFunnel();break;
    case 'collection':renderCollectionAnalysis();break;
    case 'profit':renderProfitAnalysis();break;
    case 'client-health':renderClientHealth();break;
  }
}
...
function _getMonthsAgo(n){
  var d=new Date();
  for(var i=0;i<n;i++){d.setMonth(d.getMonth()-1)}
  return{year:d.getFullYear(),month:d.getMonth()+1,
    start:new Date(d.getFullYear(),d.getMonth(),1).toISOString(),
    end:new Date(d.getFullYear(),d.getMonth()+1,0,23,59,59).toISOString()};
}
...
function _getLast12Months(){
  ...
  for(var i=11;i>=0;i--){
    ...
    months.push({label:(m.getMonth()+1)+'月',year:m.getFullYear(),month:m.getMonth()+1,
      start:new Date(m.getFullYear(),m.getMonth(),1).toISOString(),
      end:new Date(m.getFullYear(),m.getMonth()+1,0,23,59,59).toISOString()});
  }
  return months;
}

function _getOrderAmount(o){return o._amount||parseFloat(o.amount)||0}
function _getPaymentAmount(p){return p._amount||parseFloat(p.amount)||0}

function renderKpiCards(containerId,metrics){
  var el=document.getElementById(containerId);if(!el)return;
  var s='';
  for(var i=0;i<metrics.length;i++){
    var m=metrics[i],ch='';
    if(m.change!==undefined){ch='<div class="kpi-change '+(m.change>=0?'up':'down')+'">'+(m.change>=0?'↑':'↓')+Math.abs(m.change)+'%</div>'}
    s+='<div class="an-kpi-card"><div class="kpi-label">'+m.icon+' '+m.label+'</div><div class="kpi-val">'+m.value+'</div>'+ch+'</div>';
  }
  el.innerHTML=s;
}

async function renderAnalyticsOverview(){
  console.log('[Analytics] rendering overview');
  await loadOrders();await loadPayments();
  var tm=_getThisMonth();
  var monthOrders=allOrders.filter(function(o){return o.created_at&&o.created_at>=tm.start&&o.created_at<=tm.end&&o.stage!=='已取消'});
  var monthTotal=0;for(var i=0;i<monthOrders.length;i++)monthTotal+=_getOrderAmount(monthOrders[i]);
  var monthPay=allPayments.filter(function(p){return p.payment_date&&p.payment_date>=tm.start&&p.payment_date<=tm.end});
  var monthCol=0;for(var j=0;j<monthPay.length;j++)monthCol+=_getPaymentAmount(monthPay[j]);
  var allTotal=0;for(var k=0;k<allOrders.length;k++)if(allOrders[k].stage!=='已取消')allTotal+=_getOrderAmount(allOrders[k]);
  var allCol=0;for(var l=0;l<allPayments.length;l++)allCol+=_getPaymentAmount(allPayments[l]);
  var rate=allTotal>0?Math.round(allCol/allTotal*100):0;
  var clientSet={};for(var m=0;m<allOrders.length;m++)if(allOrders[m].client_id)clientSet[allOrders[m].client_id]=1;
  var active=Object.keys(clientSet).length;
  var lm=_getMonthsAgo(1);
  var lmOrders=allOrders.filter(function(o){return o.created_at&&o.created_at>=lm.start&&o.created_at<=lm.end&&o.stage!=='已取消'});
  var lmTotal=0;for(var n=0;n<lmOrders.length;n++)lmTotal+=_getOrderAmount(lmOrders[n]);
  var ch=lmTotal>0?Math.round((monthTotal-lmTotal)/lmTotal*100):0;
  renderKpiCards('an-ov-kpi',[
    {icon:'📦',label:'本月订单额',value:'¥'+formatMoney(monthTotal),change:ch},
    {icon:'💵',label:'本月回款额',value:'¥'+formatMoney(monthCol)},
    {icon:'📈',label:'回款率',value:rate+'%'},
    {icon:'👥',label:'活跃客户数',value:active}
  ]);
  drawRevenueTrend('an-ov-revenue-trend');
  drawCollectionChart('an-ov-collect-bar');
  drawClientPie('an-ov-rev-pie');
  renderTopClients(10);
  renderOverduePayments(5);
}

function drawRevenueTrend(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),revData=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],sum=0;
    for(var oi=0;oi<allOrders.length;oi++){
      var o=allOrders[oi];if(o.created_at&&o.created_at>=m.start&&o.created_at<=m.end&&o.stage!=='已取消')sum+=_getOrderAmount(o);
    }
    revData.push(sum);
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'line',
    data:{labels:months.map(function(m){return m.label}),datasets:[{label:'收入',data:revData,borderColor:'#4F6EF7',backgroundColor:'rgba(79,110,247,0.08)',fill:true,tension:0.35,pointRadius:4,pointBackgroundColor:'#4F6EF7'}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{ticks:{callback:function(v){return v>=10000?'¥'+(v/10000).toFixed(1)+'万':'¥'+v}}}}}});
}

function drawCollectionChart(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),colArr=[],recArr=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],col=0,rec=0;
    for(var oi=0;oi<allOrders.length;oi++){var o=allOrders[oi];if(o.created_at&&o.created_at>=m.start&&o.created_at<=m.end&&o.stage!=='已取消')rec+=_getOrderAmount(o)}
    for(var pi=0;pi<allPayments.length;pi++){var p=allPayments[pi];if(p.payment_date&&p.payment_date>=m.start&&p.payment_date<=m.end)col+=_getPaymentAmount(p)}
    colArr.push(col);recArr.push(rec);
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'bar',
    data:{labels:months.map(function(m){return m.label}),datasets:[{label:'已回款',data:colArr,backgroundColor:'#10B981'},{label:'应收',data:recArr,backgroundColor:'#94A3B8'}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}},scales:{y:{ticks:{callback:function(v){return v>=10000?'¥'+(v/10000).toFixed(1)+'万':'¥'+v}}}}}});
}

function drawClientPie(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var typeMap={};
  for(var i=0;i<allOrders.length;i++){
    var o=allOrders[i];if(o.stage==='已取消')continue;
    var t=o.project_name||o.stage||'其他';if(!typeMap[t])typeMap[t]=0;typeMap[t]+=_getOrderAmount(o)
  }
  var labels=[],vals=[];for(var k in typeMap){labels.push(k);vals.push(typeMap[k])}
  if(!labels.length){labels=['暂无数据'];vals=[1]}
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:labels,datasets:[{data:vals,backgroundColor:['#4F6EF7','#10B981','#F59E0B','#EF4444','#8B5CF6','#EC4899','#06B6D4','#84CC16','#6366F1','#F97316','#14B8A6','#78716C']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:11}}}}}});
}

function renderTopClients(limit){
  var tbody=document.getElementById('an-ov-top-clients');if(!tbody)return;
  var cm={};
  for(var i=0;i<allOrders.length;i++){
    var o=allOrders[i];if(o.stage==='已取消')continue;var cid=o.client_id||'u';
    if(!cm[cid])cm[cid]={total:0,count:0,latest:''};
    cm[cid].total+=_getOrderAmount(o);cm[cid].count++;
    if(o.created_at&&(!cm[cid].latest||o.created_at>cm[cid].latest))cm[cid].latest=o.created_at
  }
  var arr=[];for(var k2 in cm)arr.push({cid:k2,data:cm[k2]});
  arr.sort(function(a,b){return b.data.total-a.data.total});arr=arr.slice(0,limit);
  if(!arr.length){tbody.innerHTML='<tr><td colspan="4" class="tc">暂无数据</td></tr>';return}
  var h2='';
  for(var j=0;j<arr.length;j++){
    var a=arr[j],nm='客户#'+a.cid;var cl=allClients.find(function(x){return x.id===a.cid});if(cl)nm=cl.name;
    var dt='-';if(a.data.latest)dt=a.data.latest.substring(0,10);
    h2+='<tr><td>'+h(nm)+'</td><td style="font-weight:600">¥'+formatMoney(a.data.total)+'</td><td>'+a.data.count+'</td><td>'+dt+'</td></tr>';
  }
  tbody.innerHTML=h2;
}

function renderOverduePayments(limit){
  var tbody=document.getElementById('an-ov-overdue');if(!tbody)return;
  var now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  var pm={};
  for(var i=0;i<allPayments.length;i++){var p=allPayments[i];if(!pm[p.order_id])pm[p.order_id]=0;pm[p.order_id]+=_getPaymentAmount(p)}
  var ovd=[];
  for(var j=0;j<allOrders.length;j++){
    var o=allOrders[j];if(!o.expected_date||o.stage==='已取消'||o.stage==='已完成')continue;
    var ed=new Date(o.expected_date);if(isNaN(ed.getTime()))continue;
    var dO=Math.floor((today-ed)/86400000);if(dO<=0)continue;
    var ttl=_getOrderAmount(o),paid=pm[o.id]||0,owed=ttl-paid;if(owed<=0)continue;
    ovd.push({order:o,total:ttl,paid:paid,owed:owed,daysOver:dO})
  }
  ovd.sort(function(a,b){return b.daysOver-a.daysOver});ovd=ovd.slice(0,limit);
  if(!ovd.length){tbody.innerHTML='<tr><td colspan="7" class="tc">🎉 暂无逾期款项</td></tr>';return}
  var h3='';
  for(var k=0;k<ovd.length;k++){
    var it=ovd[k],o=it.order,cn='-';var cl=allClients.find(function(x){return x.id===o.client_id});if(cl)cn=cl.name;
    var badge='',rc='';
    if(it.daysOver>60){badge='🔴 严重';rc=' class="an-danger-row"'}
    else if(it.daysOver>30){badge='🟡 关注';rc=' class="an-alert-row"'}
    else{badge='ℹ️ 提醒'}
    h3+='<tr'+rc+'><td>'+h(cn)+'</td><td>'+h(o.order_number||'-')+'</td><td>¥'+formatMoney(it.total)+'</td><td>¥'+formatMoney(it.paid)+'</td><td>'+(o.expected_date||'').substring(0,10)+'</td><td>'+it.daysOver+'天</td><td>'+badge+'</td></tr>';
  }
  tbody.innerHTML=h3;
}

function calcFunnelData(){
  var period=document.getElementById('an-funnel-period')?document.getElementById('an-funnel-period').value:'this_month';
  var now=new Date(),start,end;
  if(period==='this_month'){start=new Date(now.getFullYear(),now.getMonth(),1);end=new Date(now.getFullYear(),now.getMonth()+1,0,23,59,59)}
  else if(period==='this_quarter'){var q=Math.floor(now.getMonth()/3);start=new Date(now.getFullYear(),q*3,1);end=new Date(now.getFullYear(),q*3+3,0,23,59,59)}
  else{start=new Date(now.getFullYear(),0,1);end=new Date(now.getFullYear(),11,31,23,59,59)}
  var si=start.toISOString(),ei=end.toISOString();
  var prevStart,prevEnd;
  if(period==='this_month'){prevStart=new Date(now.getFullYear(),now.getMonth()-1,1);prevEnd=new Date(now.getFullYear(),now.getMonth(),0,23,59,59)}
  else if(period==='this_quarter'){var pq=Math.floor(now.getMonth()/3)-1;prevStart=new Date(now.getFullYear(),pq*3,1);prevEnd=new Date(now.getFullYear(),pq*3+3,0,23,59,59)}
  else{prevStart=new Date(now.getFullYear()-1,0,1);prevEnd=new Date(now.getFullYear()-1,11,31,23,59,59)}
  var psi=prevStart.toISOString(),pei=prevEnd.toISOString();
  var stages=['线索','商机','报价','谈判中','已签约','执行中','已完成'];
  var sm={};
  for(var i=0;i<stages.length;i++){
    var sn=stages[i];
    var curr=allOrders.filter(function(o){if(o.stage==='已取消')return false;if(sn==='线索')return o.stage==='线索'&&o.created_at>=si&&o.created_at<=ei;if(sn==='商机')return o.stage==='商机'&&o.created_at>=si&&o.created_at<=ei;return o.stage===sn});
    var prev=allOrders.filter(function(o){if(o.stage==='已取消')return false;if(sn==='线索')return o.stage==='线索'&&o.created_at>=psi&&o.created_at<=pei;if(sn==='商机')return o.stage==='商机'&&o.created_at>=psi&&o.created_at<=pei;return o.stage===sn});
    var ca=0;for(var j=0;j<curr.length;j++)ca+=_getOrderAmount(curr[j]);
    var pa=0;for(var k=0;k<prev.length;k++)pa+=_getOrderAmount(prev[k]);
    sm[sn]={count:curr.length,amount:ca,prevCount:prev.length,prevAmount:pa}
  }
  return{stages:stages,stageMap:sm,period:period};
}

function renderAnSalesFunnel(){
  console.log('[Analytics] rendering sales funnel');
  var fd=calcFunnelData();
  var fv=document.getElementById('an-funnel-visual');
  var colors=['#4F6EF7','#6366F1','#818CF8','#A78BFA','#10B981','#34D399','#6EE7B7'];
  var maxCnt=1;for(var i=0;i<fd.stages.length;i++){var c=fd.stageMap[fd.stages[i]];if(c.count>maxCnt)maxCnt=c.count}
  if(fv){
    var h4='';
    for(var j=0;j<fd.stages.length;j++){
      var sn=fd.stages[j],s=fd.stageMap[sn];var pct=maxCnt>0?Math.max(15,Math.round(s.count/maxCnt*100)):15;
      h4+='<div class="an-funnel-step" style="width:'+pct+'%;background:'+colors[j]+'">'+sn+' ('+s.count+'个)<div class="an-funnel-detail"><span>¥'+formatMoney(s.amount)+'</span></div></div>';
    }
    fv.innerHTML=h4;
  }
  drawFunnelChart('an-funnel-conv-chart',fd);
  var tbody=document.getElementById('an-funnel-detail');
  if(tbody){
    var h5='',pc=fd.stageMap[fd.stages[0]].count||1;
    for(var k=0;k<fd.stages.length;k++){var sn2=fd.stages[k],s2=fd.stageMap[sn2];var cv=pc>0?Math.round(s2.count/pc*100):0;h5+='<tr><td>'+h(sn2)+'</td><td>'+s2.count+'</td><td style="font-weight:600">¥'+formatMoney(s2.amount)+'</td><td>'+cv+'%</td><td>-</td></tr>';pc=s2.count||1}
    tbody.innerHTML=h5;
  }
  var ce=document.getElementById('an-funnel-compare');
  if(ce){var ts=fd.stageMap['线索']||fd.stageMap[fd.stages[0]];ce.textContent='(vs 上期: '+ts.prevCount+'线索)'}
}

function drawFunnelChart(canvasId,fd){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  if(!fd)fd=calcFunnelData();
  var labels=[],vals=[];
  for(var i=0;i<fd.stages.length;i++){
    var sm=fd.stageMap[fd.stages[i]];
    if(i>0){var prev=fd.stageMap[fd.stages[i-1]];labels.push(fd.stages[i]);vals.push(prev.count>0?Math.round(sm.count/prev.count*100):0)}
    else{labels.push(fd.stages[i]+'→');vals.push(100)}
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'bar',data:{labels:labels,datasets:[{label:'转化率%',data:vals,backgroundColor:'#4F6EF7',borderRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false}},scales:{x:{max:100,ticks:{callback:function(v){return v+'%'}}}}}});
}

async function renderCollectionAnalysis(){
  console.log('[Analytics] rendering collection');
  await loadPayments();await loadOrders();
  var now=new Date(),tm=_getThisMonth();
  var mp=allPayments.filter(function(p){return p.payment_date&&p.payment_date>=tm.start&&p.payment_date<=tm.end});
  var mc=0;for(var i=0;i<mp.length;i++)mc+=_getPaymentAmount(mp[i]);
  var allT=0;for(var j=0;j<allOrders.length;j++)if(allOrders[j].stage!=='已取消')allT+=_getOrderAmount(allOrders[j]);
  var allC=0;for(var k=0;k<allPayments.length;k++)allC+=_getPaymentAmount(allPayments[k]);
  var ar=allT-allC,rate=allT>0?Math.round(allC/allT*100):0;
  var today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),payMap={};
  for(var l=0;l<allPayments.length;l++){var pp=allPayments[l];if(!payMap[pp.order_id])payMap[pp.order_id]=0;payMap[pp.order_id]+=_getPaymentAmount(pp)}
  var odAmt=0;
  for(var m=0;m<allOrders.length;m++){
    var oo=allOrders[m];if(!oo.expected_date||oo.stage==='已取消'||oo.stage==='已完成')continue;
    var ed=new Date(oo.expected_date);if(isNaN(ed.getTime()))continue;
    var dO=Math.floor((today-ed)/86400000);if(dO<=0)continue;
    var ttl=_getOrderAmount(oo),paid=payMap[oo.id]||0;if(ttl-paid>0)odAmt+=ttl-paid
  }
  renderKpiCards('an-col-kpi',[
    {icon:'💵',label:'本月回款额',value:'¥'+formatMoney(mc)},
    {icon:'📋',label:'应收账款',value:'¥'+formatMoney(ar)},
    {icon:'📈',label:'回款率',value:rate+'%'},
    {icon:'⚠️',label:'逾期金额',value:'¥'+formatMoney(odAmt)}
  ]);
  drawAgingPie('an-col-aging-pie');
  drawCollectionClientBar('an-col-client-bar');
  drawCollectionMonthlyTrend('an-col-monthly-trend');
  renderOverdueDetail('an-col-overdue');
}

function getAgingBuckets(){
  var now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  var buckets={d030:0,d3160:0,d6190:0,d90p:0},pm={};
  for(var i=0;i<allPayments.length;i++){var p=allPayments[i];if(!pm[p.order_id])pm[p.order_id]=0;pm[p.order_id]+=_getPaymentAmount(p)}
  for(var j=0;j<allOrders.length;j++){
    var o=allOrders[j];if(o.stage==='已取消'||o.stage==='已完成')continue;
    var ttl=_getOrderAmount(o),paid=pm[o.id]||0,bal=ttl-paid;if(bal<=0)continue;
    var ed=o.expected_date?new Date(o.expected_date):null;if(!ed||isNaN(ed.getTime())){buckets.d90p+=bal;continue}
    var dO=Math.floor((today-ed)/86400000);
    if(dO<=30)buckets.d030+=bal;else if(dO<=60)buckets.d3160+=bal;else if(dO<=90)buckets.d6190+=bal;else buckets.d90p+=bal
  }
  return buckets;
}

function drawAgingPie(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var b=getAgingBuckets();
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:['0-30天','31-60天','61-90天','90天+'],datasets:[{data:[b.d030,b.d3160,b.d6190,b.d90p],backgroundColor:['#10B981','#F59E0B','#EF4444','#B91C1C']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:11}}}}}});
}

function drawCollectionClientBar(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var cm={};
  for(var i=0;i<allPayments.length;i++){var p=allPayments[i],cid=p.client_id||'u';if(!cm[cid])cm[cid]=0;cm[cid]+=_getPaymentAmount(p)}
  var arr=[];for(var k in cm)arr.push({cid:k,amount:cm[k]});
  arr.sort(function(a,b){return b.amount-a.amount});arr=arr.slice(0,10);
  var labels=[],vals=[];
  for(var j=0;j<arr.length;j++){var cl=allClients.find(function(x){return x.id===arr[j].cid});labels.push(cl?cl.name.substring(0,6):arr[j].cid);vals.push(arr[j].amount)}
  if(!labels.length){labels=['暂无'];vals=[0]}
  _chartInstances[canvasId]=new Chart(ctx,{type:'bar',
    data:{labels:labels,datasets:[{label:'回款额',data:vals,backgroundColor:'#10B981',borderRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,indexAxis:'y',plugins:{legend:{display:false}},scales:{x:{ticks:{callback:function(v){return v>=10000?'¥'+(v/10000).toFixed(1)+'万':'¥'+v}}}}}});
}

function drawCollectionMonthlyTrend(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),colArr=[],arArr=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],col=0,rec=0;
    for(var pi=0;pi<allPayments.length;pi++){var p=allPayments[pi];if(p.payment_date&&p.payment_date>=m.start&&p.payment_date<=m.end)col+=_getPaymentAmount(p)}
    for(var oi=0;oi<allOrders.length;oi++){var o=allOrders[oi];if(o.created_at&&o.created_at>=m.start&&o.created_at<=m.end&&o.stage!=='已取消')rec+=_getOrderAmount(o)}
    colArr.push(col);arArr.push(rec);
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'line',
    data:{labels:months.map(function(m){return m.label}),datasets:[
      {label:'回款额',data:colArr,borderColor:'#10B981',backgroundColor:'rgba(16,185,129,0.1)',fill:true,tension:0.35},
      {label:'应收额',data:arArr,borderColor:'#F59E0B',backgroundColor:'rgba(245,158,11,0.08)',fill:true,tension:0.35}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}},scales:{y:{ticks:{callback:function(v){return v>=10000?'¥'+(v/10000).toFixed(1)+'万':'¥'+v}}}}}});
}

function renderOverdueDetail(tbodyId){
  var tbody=document.getElementById(tbodyId);if(!tbody)return;
  var now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),pm={};
  for(var i=0;i<allPayments.length;i++){var p=allPayments[i];if(!pm[p.order_id])pm[p.order_id]=0;pm[p.order_id]+=_getPaymentAmount(p)}
  var ovd=[];
  for(var j=0;j<allOrders.length;j++){
    var o=allOrders[j];if(!o.expected_date||o.stage==='已取消'||o.stage==='已完成')continue;
    var ed=new Date(o.expected_date);if(isNaN(ed.getTime()))continue;
    var dO=Math.floor((today-ed)/86400000);if(dO<=0)continue;
    var ttl=_getOrderAmount(o),paid=pm[o.id]||0;if(ttl-paid<=0)continue;
    ovd.push({order:o,total:ttl,paid:paid,owed:ttl-paid,daysOver:dO})
  }
  ovd.sort(function(a,b){return b.daysOver-a.daysOver});
  if(!ovd.length){tbody.innerHTML='<tr><td colspan="7" class="tc">🎉 暂无逾期款项</td></tr>';return}
  var h2='';
  for(var k=0;k<ovd.length;k++){
    var it=ovd[k],o=it.order,cn='-';var cl=allClients.find(function(x){return x.id===o.client_id});if(cl)cn=cl.name;
    var badge='',rc='';
    if(it.daysOver>60){badge='<span style="color:#c62828;font-weight:700">🔴 严重</span>';rc=' class="an-danger-row"'}
    else if(it.daysOver>30){badge='<span style="color:#f57f17;font-weight:700">🟡 关注</span>';rc=' class="an-alert-row"'}
    else{badge='<span style="color:var(--text2)">ℹ️ 提醒</span>'}
    h2+='<tr'+rc+'><td>'+h(cn)+'</td><td>'+h(o.order_number||'-')+'</td><td>¥'+formatMoney(it.total)+'</td><td>¥'+formatMoney(it.paid)+'</td><td>'+(o.expected_date||'').substring(0,10)+'</td><td>'+it.daysOver+'天</td><td>'+badge+'</td></tr>';
  }
  tbody.innerHTML=h2;
}

function renderProfitAnalysis(){
  console.log('[Analytics] rendering profit');
  var allRev=0;for(var i=0;i<allOrders.length;i++)if(allOrders[i].stage!=='已取消')allRev+=_getOrderAmount(allOrders[i]);
  var estCost=allRev*0.65,estProfit=allRev-estCost,estMargin=allRev>0?Math.round(estProfit/allRev*100):0;
  renderKpiCards('an-prof-kpi',[
    {icon:'💰',label:'总收入',value:'¥'+formatMoney(allRev)},
    {icon:'📉',label:'估算成本',value:'¥'+formatMoney(estCost)},
    {icon:'📊',label:'估算利润',value:'¥'+formatMoney(estProfit)},
    {icon:'📈',label:'毛利率',value:estMargin+'%'}
  ]);
  drawProfitTrend('an-prof-trend');
  drawMarginTrend('an-prof-margin');
  drawProfitTypePie('an-prof-type-pie');
  renderProfitProjRank();
  drawCostPie('an-prof-cost-pie');
}

function drawProfitTrend(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),revArr=[],costArr=[],profArr=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],rev=0;
    for(var oi=0;oi<allOrders.length;oi++){var o=allOrders[oi];if(o.created_at&&o.created_at>=m.start&&o.created_at<=m.end&&o.stage!=='已取消')rev+=_getOrderAmount(o)}
    var cost=rev*0.65;revArr.push(rev);costArr.push(Math.round(cost));profArr.push(Math.round(rev-cost));
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'line',
    data:{labels:months.map(function(m){return m.label}),datasets:[
      {label:'收入',data:revArr,borderColor:'#4F6EF7',tension:0.35,fill:false},
      {label:'成本(估)',data:costArr,borderColor:'#EF4444',tension:0.35,fill:false},
      {label:'利润(估)',data:profArr,borderColor:'#10B981',tension:0.35,fill:true,backgroundColor:'rgba(16,185,129,0.1)'}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}},scales:{y:{ticks:{callback:function(v){return v>=10000?'¥'+(v/10000).toFixed(1)+'万':'¥'+v}}}}}});
}

function drawMarginTrend(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),margArr=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],rev=0;
    for(var oi=0;oi<allOrders.length;oi++){var o=allOrders[oi];if(o.created_at&&o.created_at>=m.start&&o.created_at<=m.end&&o.stage!=='已取消')rev+=_getOrderAmount(o)}
    var cost=rev*0.65;margArr.push(rev>0?Math.round((rev-cost)/rev*100):0);
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'line',
    data:{labels:months.map(function(m){return m.label}),datasets:[{label:'毛利率%',data:margArr,borderColor:'#8B5CF6',backgroundColor:'rgba(139,92,246,0.08)',fill:true,tension:0.35,pointRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{max:100,ticks:{callback:function(v){return v+'%'}}}}}});
}

function drawProfitTypePie(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var tm={};
  for(var i=0;i<allOrders.length;i++){
    var o=allOrders[i];if(o.stage==='已取消')continue;var t=o.project_name||o.stage||'其他';
    if(!tm[t])tm[t]=0;tm[t]+=_getOrderAmount(o)*0.35
  }
  var labels=[],vals=[];for(var k in tm){labels.push(k);vals.push(Math.round(tm[k]))}
  if(!labels.length){labels=['暂无'];vals=[1]}
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:labels,datasets:[{data:vals,backgroundColor:['#10B981','#4F6EF7','#F59E0B','#8B5CF6','#EF4444','#EC4899','#06B6D4','#84CC16']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{font:{size:11}}}}}});
}

function renderProfitProjRank(){
  var tbody=document.getElementById('an-prof-proj-rank');if(!tbody)return;
  var pm={};
  for(var i=0;i<allOrders.length;i++){var o=allOrders[i];if(o.stage==='已取消')continue;var pn=o.project_name||'未分类';if(!pm[pn])pm[pn]=0;pm[pn]+=_getOrderAmount(o)}
  var arr=[];for(var k in pm)arr.push({name:k,rev:pm[k]});
  arr.sort(function(a,b){return b.rev-a.rev});
  if(!arr.length){tbody.innerHTML='<tr><td colspan="5" class="tc">暂无数据</td></tr>';return}
  var h2='';
  for(var j=0;j<arr.length;j++){
    var a=arr[j],cost=Math.round(a.rev*0.65),profit=a.rev-cost,margin=a.rev>0?Math.round(profit/a.rev*100):0;
    var mc=margin>=30?'color:#10B981':margin>=10?'color:#F59E0B':'color:#EF4444';
    h2+='<tr><td>'+h(a.name)+'</td><td>¥'+formatMoney(a.rev)+'</td><td>¥'+formatMoney(cost)+'</td><td style="font-weight:600;'+mc+'">¥'+formatMoney(profit)+'</td><td style="font-weight:600;'+mc+'">'+margin+'%</td></tr>';
  }
  tbody.innerHTML=h2;
}

function drawCostPie(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var allRev=0;for(var i=0;i<allOrders.length;i++)if(allOrders[i].stage!=='已取消')allRev+=_getOrderAmount(allOrders[i]);
  var totalCost=allRev*0.65;
  var items=[{label:'人力成本',pct:40,val:Math.round(totalCost*0.4)},{label:'采购成本',pct:30,val:Math.round(totalCost*0.3)},{label:'运营费用',pct:15,val:Math.round(totalCost*0.15)},{label:'管理费用',pct:10,val:Math.round(totalCost*0.1)},{label:'其他',pct:5,val:Math.round(totalCost*0.05)}];
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:items.map(function(x){return x.label}),datasets:[{data:items.map(function(x){return x.val}),backgroundColor:['#EF4444','#F59E0B','#8B5CF6','#06B6D4','#94A3B8']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'},tooltip:{callbacks:{label:function(ctxi){return ctxi.label+': ¥'+formatMoney(ctxi.parsed)+' ('+items[ctxi.dataIndex].pct+'%)'}}}}}});
}
...
function getClientScores(){
  ...
  for(var i=0;i<allOrders.length;i++){
    var o=allOrders[i];if(o.stage==='已取消')continue;var cid=o.client_id||'u';
    if(!cm[cid])cm[cid]={total:0,count:0,latest:'',first:o.created_at||''};
    cm[cid].total+=_getOrderAmount(o);cm[cid].count++;
    if(!cm[cid].latest||o.created_at>cm[cid].latest)cm[cid].latest=o.created_at;
    if(o.created_at&&(!cm[cid].first||o.created_at<cm[cid].first))cm[cid].first=o.created_at
  }
  var allTotal=0;for(var k in cm)allTotal+=cm[k].total;
  var allCount=0;for(var k2 in cm)allCount+=cm[k2].count;
  var avgRev=allCount>0?Math.round(allTotal/allCount):0;
  var results=[];
  for(var cid in cm){
    var c=cm[cid],recency=0;if(c.latest){var ld=new Date(c.latest);recency=Math.floor((today-ld)/86400000)}
    var score=0;
    if(c.count>=10)score+=3;else if(c.count>=5)score+=2;else if(c.count>=2)score+=1;
    if(c.total>=avgRev*3)score+=3;else if(c.total>=avgRev)score+=2;else if(c.total>0)score+=1;
    if(recency<=30)score+=3;else if(recency<=90)score+=2;else if(recency<=180)score+=1;
    var tier='',tl='';
    if(score>=8){tier='healthy';tl='🟢 健康'}
    else if(score>=5){tier='attention';tl='🟡 关注'}
    else if(score>=2){tier='risk';tl='🔴 风险'}
    else{tier='lost';tl='⚫ 流失'}
    results.push({cid:cid,total:c.total,count:c.count,recency:recency,score:score,tier:tier,tierLabel:tl,latest:c.latest,first:c.first})
  }
...
function renderClientHealth(){
  console.log('[Analytics] rendering client health');
  var scores=getClientScores();
  var h=0,a=0,r=0,l=0;
  for(var i=0;i<scores.length;i++){var s=scores[i];if(s.tier==='healthy')h++;else if(s.tier==='attention')a++;else if(s.tier==='risk')r++;else l++}
  renderKpiCards('an-ch-kpi',[
    {icon:'🟢',label:'健康客户',value:h},
    {icon:'🟡',label:'关注客户',value:a},
    {icon:'🔴',label:'风险客户',value:r},
    {icon:'⚫',label:'流失客户',value:l}
  ]);
  drawClientTierPie('an-ch-tier-pie',scores);
  drawClientActiveBar('an-ch-active-bar',scores);
  drawNewClientTrend('an-ch-new-trend');
  drawValueScatter('an-ch-value-scatter',scores);
  drawRepurchaseChart('an-ch-repurchase',scores);
  renderChurnList('an-ch-churn-list',scores);
}

function drawClientTierPie(canvasId,scores){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var counts=[0,0,0,0];
  for(var i=0;i<scores.length;i++){var s=scores[i];if(s.tier==='healthy')counts[0]++;else if(s.tier==='attention')counts[1]++;else if(s.tier==='risk')counts[2]++;else counts[3]++}
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:['健康','关注','风险','流失'],datasets:[{data:counts,backgroundColor:['#10B981','#F59E0B','#EF4444','#94A3B8']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});
}

function drawClientActiveBar(canvasId,scores){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var b={d030:0,d3160:0,d6190:0,d91:0};
  for(var i=0;i<scores.length;i++){var r=scores[i].recency;if(r<=30)b.d030++;else if(r<=60)b.d3160++;else if(r<=90)b.d6190++;else b.d91++}
  _chartInstances[canvasId]=new Chart(ctx,{type:'bar',
    data:{labels:['活跃(≤30天)','较活跃(31-60)','沉默(61-90)','流失(>90天)'],datasets:[{data:[b.d030,b.d3160,b.d6190,b.d91],backgroundColor:['#10B981','#4F6EF7','#F59E0B','#EF4444'],borderRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
}

function drawNewClientTrend(canvasId){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var months=_getLast12Months(),newArr=[];
  for(var ri=0;ri<months.length;ri++){
    var m=months[ri],cnt=0;
    for(var ci=0;ci<allClients.length;ci++){var c=allClients[ci];if(c.created_at&&c.created_at>=m.start&&c.created_at<=m.end)cnt++}
    newArr.push(cnt);
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'line',
    data:{labels:months.map(function(m){return m.label}),datasets:[{label:'新增客户',data:newArr,borderColor:'#10B981',backgroundColor:'rgba(16,185,129,0.08)',fill:true,tension:0.35,pointRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
}

function drawValueScatter(canvasId,scores){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var pts=[];
  for(var i=0;i<scores.length;i++){var s=scores[i];if(s.count===0&&s.total===0)continue;pts.push({x:s.count,y:s.total/10000,tier:s.tier})}
  var tc={healthy:'#10B981',attention:'#F59E0B',risk:'#EF4444',lost:'#94A3B8'},ds=[];
  for(var t in tc){
    var fp=pts.filter(function(p){return p.tier===t});
    ds.push({label:t==='healthy'?'健康':t==='attention'?'关注':t==='risk'?'风险':'流失',data:fp,backgroundColor:tc[t],pointRadius:6});
  }
  _chartInstances[canvasId]=new Chart(ctx,{type:'scatter',data:{datasets:ds},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{position:'bottom'}},
      scales:{x:{title:{display:true,text:'订单数(频次)'}},y:{title:{display:true,text:'订单额(万)'}}}}});
}

function drawRepurchaseChart(canvasId,scores){
  _destroyChart(canvasId);var ctx=document.getElementById(canvasId);if(!ctx)return;
  var once=0,rep=0;
  for(var i=0;i<scores.length;i++){if(scores[i].count>=2)rep++;else once++}
  _chartInstances[canvasId]=new Chart(ctx,{type:'pie',
    data:{labels:['复购客户(≥2单)','单次客户'],datasets:[{data:[rep,once],backgroundColor:['#10B981','#94A3B8']}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});
}

function renderChurnList(tbodyId,scores){
  var tbody=document.getElementById(tbodyId);if(!tbody)return;
  var now=new Date(),threeMAgo=new Date(now.getFullYear(),now.getMonth()-3,now.getDate()).toISOString();
  var items=[];
  for(var ci=0;ci<allClients.length;ci++){
    var cl=allClients[ci],sc=scores.find(function(s){return s.cid===cl.id});
    if(!sc||sc.count===0)continue;
    if(!sc.latest||sc.latest<threeMAgo)items.push({client:cl,score:sc})
  }
  if(!items.length){tbody.innerHTML='<tr><td colspan="6" class="tc">🎉 无流失风险客户</td></tr>';return}
  var h2='';
  for(var j=0;j<items.length;j++){
    var it=items[j],cl=it.client,s=it.score,daysAgo=s.latest?Math.floor((now-new Date(s.latest))/86400000):999;
    var bc=s.tier==='risk'?'an-badge-risk':s.tier==='attention'?'an-badge-attention':'an-badge-lost';
    h2+='<tr><td>'+h(cl.name)+'</td><td>'+(h(cl.client_type||'-'))+'</td><td>'+(s.latest?s.latest.substring(0,10):'-')+'</td><td>'+daysAgo+'天</td><td>'+s.count+'单</td><td><span class="an-badge '+bc+'">'+s.tierLabel+'</span></td></tr>';
  }
  tbody.innerHTML=h2;
}
...
function switchFWTab(tab){
  var dbg=document.getElementById("fw-debug");if(dbg){dbg.style.display="block";dbg.innerHTML="▶ switchFWTab: "+h(tab);}
...
async function loadFWOverview(){
  ...
  if(dbg)dbg.innerHTML+="<br>✔ checks: "+(checks?(checks.length+" rows"):"null");
  ...
  if(dbg)dbg.innerHTML+="<br>✔ visits: "+(visits?(visits.length+" rows"):"null");
  ...
  if(dbg)dbg.innerHTML+="<br>✔ weekVisits: "+(weekVisits?(weekVisits.length+" rows"):"null");
    // Team today
  ...
  if(dbg)dbg.innerHTML+="<br>✔ teamChecks: "+(teamChecks?(teamChecks.length+" rows"):"null");
    ...
    if(dbg)dbg.innerHTML+="<br>▶ Rendering KPI cards...";kpi.innerHTML='<div class="fw-kpi-card"><div class="kpi-label">📍 今日签到</div><div class="kpi-val">'+(hasIn?'✅':hasOut?'☀️':'—')+'</div></div>'
  ...
  if(dbg)dbg.innerHTML+="<br>🔥 ERROR: "+h(e.message||e)+"<br>"+(e.stack||"").substring(0,200);console.error('loadFWOverview error:',e);if(kpi)kpi.innerHTML='<div style="color:var(--danger);font-size:13px">加载失败</div>'}
}
...
function fwResetBtn(type){var btnIn=document.getElementById('fw-btn-in');var btnOut=document.getElementById('fw-btn-out');if(type==='in'){btnIn.disabled=false;btnIn.textContent='📍 上班签到'}else{btnOut.disabled=false;btnOut.textContent='🏠 下班签退'}}
async function doCheckIn(type){
  console.log('[doCheckIn] type=',type,'navigator.geolocation=',!!(navigator&&navigator.geolocation));
  ...
  if(!btnEl){console.error('[doCheckIn] btnEl not found for type=',type);showToast('错误：找不到按钮');return;}
  ...
  var t=setTimeout(function(){if(!settled){console.log('[doCheckIn] timeout, saving without location');resetBtn();showToast('\u26A0\uFE0F 定位超时(8秒)，无定位签到...');doSaveCheckIn(null,null,type,note,addr);}},8000);
  if(!navigator||!navigator.geolocation){console.log('[doCheckIn] geolocation not supported');clearTimeout(t);resetBtn();showToast('\u26A0\uFE0F 浏览器不支持定位');doSaveCheckIn(null,null,type,note,addr);return;}
  navigator.geolocation.getCurrentPosition(function(p){console.log('[doCheckIn] position success',p.coords);if(settled)return;var lat=p.coords.latitude,lng=p.coords.longitude;if(isNaN(lat)||isNaN(lng)){console.log('[doCheckIn] invalid coords');clearTimeout(t);resetBtn();showToast('\u26A0\uFE0F 坐标无效，无定位签到...');doSaveCheckIn(null,null,type,note,addr);return;}clearTimeout(t);resetBtn();showLocationConfirm(lat,lng,type==='in'?'\u{1F4CD} 上班签到 - 确认位置':'\u{1F3E0} 下班签退 - 确认位置',function(clat,clng){doSaveCheckIn(clat,clng,type,note,addr);});},function(err){console.log('[doCheckIn] position error',err.code,err.message);if(settled)return;clearTimeout(t);resetBtn();showToast(err.code===1?'\u26A0\uFE0F 定位被拒绝，无定位签到...':'\u26A0\uFE0F GPS不可用，无定位签到...');doSaveCheckIn(null,null,type,note,addr);},{timeout:7000,enableHighAccuracy:true,maximumAge:60000});
}
...
async function doSaveCheckIn(lat,lng,type,note,addr){
  ...
  try{
    ...
    if(r.error){console.error('[doSaveCheckIn] supabase error:',JSON.stringify(r.error));showToast(label+'失败: '+(r.error.message||r.error.details||JSON.stringify(r.error)));return}
  ...
  }catch(e){btnEl.disabled=false;btnEl.textContent=type==='in'?'\u{1F4CD} 上班签到':'\u{1F3E0} 下班签退';console.error('[doSaveCheckIn] exception:',e);showToast(label+'异常: '+(e.message||e))}
}

function openFWVisitForm(id){
  ...
  document.getElementById('fw-vm-client-list').classList.add('hidden');
...
function onFWClientInput(){
  ...
  if(!q||!allClients.length){list.classList.add('hidden');list.innerHTML='';return}
  var matches=allClients.filter(function(c){return (c.name||'').toLowerCase().indexOf(q)>=0}).slice(0,8);
  if(!matches.length){list.classList.add('hidden');list.innerHTML='';return}
  fwClientSuggList=matches;fwClientSuggIdx=0;
  var h='';
  for(var i=0;i<matches.length;i++){
    h+='<div class="name-suggestion-item'+(i===0?' active':'')+'" onmousedown="selectFWClient(\''+matches[i].id+'\')">'+escHtml(matches[i].name)+'</div>';
  }
  list.innerHTML=h;list.classList.remove('hidden');
}
function onFWClientKey(e){
  ...
  if(e.key==='ArrowDown'){e.preventDefault();fwClientSuggIdx=Math.min((fwClientSuggIdx||-1)+1,items.length-1)}
  else if(e.key==='ArrowUp'){e.preventDefault();fwClientSuggIdx=Math.max((fwClientSuggIdx||0)-1,0)}
...
function selectFWClient(id,name){
  fwVisitClientId=id;
  var list=document.getElementById('fw-vm-client-list');if(list){list.classList.add('hidden');list.innerHTML=''}
  fwClientSuggIdx=-1;fwClientSuggList=[];
  var cl=allClients.find(function(x){return x.id===id});
  document.getElementById('fw-vm-client').value=cl?cl.name:(name||'');
}

var fwLocationConfirmCallback=null;
var fwPendingLat=null,fwPendingLng=null;
...
async function saveFVisit(){
  ...
  // Build data object (will fill lat/lng after confirm)
  var data={
  ...
  if(navigator.geolocation){
    try{
      ...
      console.log('[GPS] saveFVisit requesting, mobile='+isMobile);
      var pos=await new Promise(function(res,rej){
        navigator.geolocation.getCurrentPosition(res,rej,{timeout:10000,enableHighAccuracy:isMobile,maximumAge:60000});
      });
      var lat=pos.coords.latitude,lng=pos.coords.longitude;
      console.log('[GPS] saveFVisit got coords:',lat,lng,'accuracy:',pos.coords.accuracy);
      gotLocation=true;
      // Step 2: Show confirm modal
      showLocationConfirm(lat,lng,'🏢 拜访记录 - 确认位置',async function(confirmedLat,confirmedLng){
        data.latitude=confirmedLat;data.longitude=confirmedLng;
        await doSaveFVisit(data);
      });
    }catch(g){
      console.warn('[GPS] saveFVisit failed:',g.message||g);
      if(g.code===1)showToast('⚠️ 定位被拒绝，请在浏览器设置中允许定位权限');
      else if(g.code===2)showToast('⚠️ 定位不可用，请稍后重试');
      else if(g.code===3)showToast('⚠️ 定位超时（10秒）');
    }
  }
  if(!gotLocation){
...
async function doSaveFVisit(data){
  try{
    var r;
...
function closeVisitForm(){
  var m=document.getElementById('fw-visit-modal');
  if(m){m.classList.add('hidden');m.style.display='';}
  fwVisitEditId=null;
  document.getElementById('fw-vm-client').value='';
  document.getElementById('fw-vm-purpose').value='';
  document.getElementById('fw-vm-date').value='';
  document.getElementById('fw-vm-notes').value='';
  document.getElementById('fw-vm-result').value='';
  document.getElementById('fw-vm-title').textContent='新增拜访记录';
}

// Location confirmation modal
function closeLocationConfirm(){
  document.getElementById('fw-location-confirm').classList.add('hidden');
  document.getElementById('fw-location-confirm').style.display='';
  fwPendingLat=null;fwPendingLng=null;fwLocationConfirmCallback=null;
  if(window._fwPendingType)fwResetBtn(window._fwPendingType);
  window._fwPendingType=null;
  if(window._fwLcMap){window._fwLcMap.clearOverlays();window._fwLcMap=null}
}
function confirmLocation(){
  if(!fwLocationConfirmCallback) return;
  var cb=fwLocationConfirmCallback;
  var lat=fwPendingLat,lng=fwPendingLng;
  window._fwPendingType=null;
...
function showLocationConfirm(lat,lng,titleText,callback){
  ...
  document.getElementById('fw-lc-title').textContent=titleText||'📍 确认定位';
  document.getElementById('fw-lc-info').innerHTML='<b>纬度:</b> '+lat.toFixed(6)+'&nbsp;&nbsp;<b>经度:</b> '+lng.toFixed(6);
  modal.classList.remove('hidden');
  modal.style.display='flex';modal.style.position='fixed';modal.style.top='0';modal.style.left='0';modal.style.width='100vw';modal.style.height='100vh';modal.style.background='rgba(0,0,0,.5)';modal.style.zIndex='9999';modal.style.alignItems='center';modal.style.justifyContent='center';
  var mcont=document.getElementById('fw-lc-map');
  mcont.innerHTML='<div id="fw-lc-bmap" style="width:100%;height:100%"></div>';
  setTimeout(function(){
    if(!window.BMap||!window.BMap.Map){mcont.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--text3)">地图加载失败</div>';return}
    if(window._fwLcMap){window._fwLcMap.clearOverlays();window._fwLcMap=null}
    ...
    window._gps2bd.translate([new BMap.Point(lng,lat)],1,5,function(r){
      ...
      map.addOverlay(new BMap.Circle(pt,50,{strokeColor:'#3B82F6',strokeWeight:2,strokeOpacity:0.8,fillColor:'#3B82F6',fillOpacity:0.15}));
    });
  },200);
}
async function loadFWVisits(){
  ...
  try{
    ...
    if(error){list.innerHTML='<div style="color:var(--danger);font-size:13px">加载失败: '+h(error.message)+'</div>';return}
    ...
    list.innerHTML=html;
  }catch(e){list.innerHTML='<div style="color:var(--danger);font-size:13px">加载失败: '+h(e.message||e)+'</div>';console.error('loadFWVisits:',e)}
}

function openFVisit(id){
  fwVisitEditId=id||null;
  fwVisitClientId=null;
  var modal=document.getElementById('fw-visit-modal');
...
async function loadFWTrack(){
  ...
  try{
    ...
    var mcont=document.getElementById('fw-map-container');
  if(points===0){
    mcont.innerHTML='<div style="text-align:center;padding:40px"><div style="font-size:48px;margin-bottom:12px">🗺️</div><div style="font-size:14px;color:var(--text2)">暂无GPS定位数据<br><span style="font-size:12px;color:var(--text3)">请使用支持GPS的设备签到以生成轨迹</span></div></div>';
    if(window._fwMap){window._fwMap.clearOverlays();window._fwMap=null}
  } else {
    // Render real map with check-in points
    mcont.innerHTML='<div id="fw-baidu-map" style="width:100%;height:100%"></div>';
    setTimeout(function(){
...
var curDetailState='todo';
var curDetailBlocks=[];
var curDetailAssignments=[];
var curDetailProject=null;
...
async function renderProjectDetail(pid){
  ...
  var v=document.getElementById('project-detail-view');
  ...
  var r=await sb.from('projects').select('*').eq('id',pid).single();
  if(r.error||!r.data){alert('项目不存在');closeProjectDetail();return}
  curDetailProject=r.data;
  document.getElementById('pd-pid-badge').textContent='#'+pid;
  document.getElementById('pd-title').textContent=r.data.name||'未命名项目';
  document.getElementById('pd-owner-info').textContent='负责人: '+(r.data.owner_email||'未指派');

  var ra=await sb.from('project_assignments').select('*').eq('project_id',pid);
  curDetailAssignments=ra.data||[];

  var perm='none';
  if(typeof isSuperAdmin!=='undefined'&&isSuperAdmin)perm='admin';
  else if(typeof currentUserRole!=='undefined'&&currentUserRole==='admin')perm='admin';
  else if(curDetailProject.owner_id===currentUser.id)perm='owner';
  else{
    var my=curDetailAssignments.find(function(a){return a.user_id===currentUser.id});
    if(my)perm=my.project_role;
  }
  var permMap={admin:'全部(管理员)',owner:'全部(项目所有者)',edit:'编辑权限',view:'只读',approve:'审批权限',none:'无权限'};
  document.getElementById('pd-perm-info').textContent='权限: '+(permMap[perm]||perm);

  if(typeof loadTemplates==='function')await loadTemplates();
  var tpl=null;
  if(curDetailProject.template_id&&typeof allTemplates!=='undefined'){
    tpl=allTemplates.find(function(t){return t.id==curDetailProject.template_id});
  }
  if(!tpl&&typeof allTemplates!=='undefined'&&allTemplates.length>0)tpl=allTemplates[0];

  curDetailBlocks=[];
  if(tpl&&tpl.steps){
    var steps=Array.isArray(tpl.steps)?tpl.steps:[];
    steps.forEach(function(st,i){
      curDetailBlocks.push({
        key:st.key||('step_'+i),
        label:st.label||st.name||('步骤 '+(i+1)),
        data:curDetailProject['step_'+i]||{},
        visible_to:st.block_visible_to||[],
        editable_to:st.block_editable_to||[],
        department:st.department||'',
        panel:st.panel||null,
        seq:i  // 使用索引而不是 st.seq，确保和 project_step_assignees 的 step_key 匹配
      });
    });
  }
  if(curDetailBlocks.length===0){
    curDetailBlocks=[{key:'basic',label:'基本信息',data:curDetailProject,visible_to:[],editable_to:[]}];
  }

  var rs=await sb.from('project_block_state').select('*').eq('project_id',pid);
  var blockStates=rs.data||[];
  curDetailBlocks.forEach(function(b){
    var st=blockStates.find(function(x){return x.block_key===b.key});
    b.state=st?st.status:'pending';
  });

  if(curDetailProject.current_state){
    var stateMap={todo:'待办',doing:'在办',done:'办结',archived:'归档'};
    document.getElementById('pd-owner-info').textContent='负责人: '+(curDetailProject.owner_email||'-')+' | 状态: '+(stateMap[curDetailProject.current_state]||curDetailProject.current_state);
  }

  switchDetailTab('todo');
  }catch(e){
    console.error('renderProjectDetail error:',e);
    showToast('加载项目详情失败: '+(e.message||e));
    closeProjectDetail();
  }
}
...
function switchDetailTab(status){
  curDetailState=status;
  document.querySelectorAll('.pd-status-tab').forEach(function(el){
...
function renderDetailBlocks(){
  // Async wrapper
  renderDetailBlocksAsync().catch(function(e){console.error('renderDetailBlocks error:',e);});
}
async function renderDetailBlocksAsync(){
  var c=document.getElementById('pd-blocks-container');
  var perm=getCurrentPerm();

  // 加载字段配置和数据
  var allFields=[];
  var allValues=[];
  try{
    var r1=await sb.from('project_step_fields').select('*').eq('project_id',curDetailId).order('step_key,sort_order');
    allFields=r1.data||[];
    var r2=await sb.from('project_step_field_values').select('*').eq('project_id',curDetailId);
    allValues=r2.data||[];
  }catch(e){console.warn('load step fields failed:',e);}

  var visible=curDetailBlocks.filter(function(b){
    if(curDetailState==='todo')return b.state==='pending'||b.state==='in_progress';
    if(curDetailState==='doing')return b.state==='in_progress';
    if(curDetailState==='done')return b.state==='done'||b.state==='skipped';
    return true;
  });

  if(visible.length===0){
    c.innerHTML='<div class="pd-no-access">该状态下没有可见的区块</div>';
    return;
  }

  var html='';
  visible.forEach(function(b){
    if(!canViewBlock(b,perm))return;
    var canEdit=canEditBlock(b,perm);
    var collapsed=b.state==='done'?'collapsed':'';
    var statusLabel={pending:'待办',in_progress:'进行中',done:'已完成',skipped:'已跳过'}[b.state]||b.state;

    html+='<div class="pd-block '+collapsed+'" data-key="'+b.key+'" data-seq="'+b.seq+'" draggable="true" ondragstart="pdBlockDragStart(event)" ondragover="pdBlockDragOver(event)" ondrop="pdBlockDrop(event)" ondragend="pdBlockDragEnd(event)">';
    html+='<div class="pd-block-header" onclick="if(event.target.closest(\'.pd-block-edit-btn\'))return;this.parentElement.classList.toggle(\'collapsed\')" style="cursor:grab">';
    html+='<div class="pd-block-title"><span class="pd-drag-handle" style="margin-right:6px;color:var(--text3)">⠿</span><span class="pd-seq-badge">#'+b.seq+'</span>'+escHtml(b.label);
    if(b.department)html+=' <span class="pd-dept-tag">'+escHtml(b.department)+'</span>';
    html+='</div>';
    html+='<div style="display:flex;align-items:center;gap:8px">';
    // 管理员的编辑入口
    if(isAdmin()){
      html+='<button class="pd-block-edit-btn" onclick="event.stopPropagation();pdEditStep(\''+b.key+'\')" style="font-size:11px;padding:2px 8px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text2);cursor:pointer" title="编辑步骤属性">⚙</button>';
    }
    html+='<span class="pd-block-status '+b.state+'">'+statusLabel+'</span>';
    html+='<span class="pd-block-toggle">▼</span>';
    html+='</div></div>';

    html+='<div class="pd-block-body">';
    // Check if data has actual fields to render
    var dataKeys=b.data&&typeof b.data==='object'&&!Array.isArray(b.data)?Object.keys(b.data).filter(function(k){return !k.startsWith('_')}):[];
    if(dataKeys.length>0){
      dataKeys.forEach(function(k){
        var v=b.data[k];
        html+='<div class="pd-block-row">';
        html+='<div class="pd-block-label">'+escHtml(k)+'</div>';
        if(canEdit){
          html+='<input class="pd-block-input" data-block="'+b.key+'" data-field="'+escHtml(k)+'" value="'+escHtml(String(v==null?'':v))+'">';
        }else{
          html+='<div class="pd-block-value">'+escHtml(String(v==null?'-':v))+'</div>';
        }
        html+='</div>';
      });
    }else if(b.panel){
      // 从数据库获取该步骤的字段配置和数据
      var stepKey='step_'+b.seq;
      console.log('[DEBUG] render panel:',{key:b.key,seq:b.seq,stepKey:stepKey,allFieldsCount:allFields.length,matchedFields:allFields.filter(function(f){return f.step_key===stepKey;}).length});
      var stepFields=allFields.filter(function(f){return f.step_key===stepKey;});
      var stepValues=allValues.filter(function(v){return v.step_key===stepKey;});
      html+=renderPanelContent(b.panel,b.seq,canEdit,stepFields,stepValues);
    }else{
      html+='<div class="pd-block-value" style="color:var(--text3);padding:20px;text-align:center">暂无数据</div>';
    }

    if(b.visible_to&&b.visible_to.length>0){
      html+='<div class="pd-block-views">可见: ';
      b.visible_to.forEach(function(d){html+='<span class="pd-dept-tag">'+escHtml(d)+'</span>'});
      html+='</div>';
    }
    html+='</div></div>';
  });
  ...
  if(!html){
    c.innerHTML='<div class="pd-no-access">当前权限下无可查看的区块</div>';
    return;
  }
  c.innerHTML=html;
}

function renderPanelContent(panel,seq,canEdit,stepFields,stepValues){
  var html='';
  var fields=stepFields||[];
  var values=stepValues||[];
  var valueMap={};
  values.forEach(function(v){valueMap[v.field_key]=v.field_value;});
  
  if(fields.length===0){
    html+='<div style="padding:20px;text-align:center;color:var(--text3);font-size:13px">暂无配置字段，管理员可在"配置成员→字段配置"中添加</div>';
    return html;
  }

  // 按 sort_order 排序
  fields.sort(function(a,b){return (a.sort_order||99)-(b.sort_order||99);});

  // 检查是否有串行字段
  var hasSerial=fields.some(function(f){return f.collaboration_mode==='serial';});

  if(hasSerial){
    // 串行模式：计算每个字段的可用状态
    // 规则：第一个串行字段始终可填；后续字段需前一个字段有值才解锁
    var prevSerialCompleted=true;
    fields.forEach(function(f,i){
      if(f.collaboration_mode==='serial'){
        f._unlocked=prevSerialCompleted&&canEdit;
        f._prevFk=f.depends_on||null;
        var fv=valueMap[f.field_key]||'';
        prevSerialCompleted=!!fv;
      }else{
        f._unlocked=canEdit;
      }
    });
  ...
  html+='<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:10px;padding:16px">';
  html+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">';
  fields.forEach(function(f){
    var val=valueMap[f.field_key]||f.default_value||'';
    var fieldCanEdit=hasSerial?(f._unlocked!==false):canEdit;
    var lockedStyle=fieldCanEdit?'':'opacity:0.5;pointer-events:none';
    var lockIcon='';
    if(!fieldCanEdit&&f.collaboration_mode==='serial'){
      lockIcon=' 🔒<span style="font-size:10px;color:var(--text3)">需完成上一步</span>';
    }
    var modeTag=f.collaboration_mode==='serial'?'<span style="font-size:10px;background:#f59e0b20;color:#f59e0b;padding:2px 6px;border-radius:3px;margin-left:6px">串行</span>':'';
    html+='<div style="grid-column:'+(f.field_type==='textarea'?'1 / -1':'auto')+'" class="field-row" data-field="'+f.field_key+'" data-mode="'+(f.collaboration_mode||'parallel')+'">';
    html+='<label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">'+escHtml(f.field_label)+(f.is_required?'<span style="color:#ef4444">*</span>':'')+modeTag+lockIcon+'</label>';
    if(f.field_type==='textarea'){
      html+='<textarea data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" data-field-id="'+f.id+'" '+(fieldCanEdit?'':'disabled')+' rows="3" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:14px;background:var(--card-bg);color:var(--text);resize:vertical" placeholder="'+(f.placeholder||'')+'">'+escHtml(val)+'</textarea>';
    }else if(f.field_type==='select'||f.field_type==='radio'){
      var opts=[];
      try{opts=typeof f.field_options==='string'?JSON.parse(f.field_options):(f.field_options||[]);}catch(e){}
      if(f.field_type==='select'){
        html+='<select data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" data-field-id="'+f.id+'" '+(fieldCanEdit?'':'disabled')+' style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:14px;background:var(--card-bg);color:var(--text)"><option value="">-</option>';
        opts.forEach(function(opt){html+='<option value="'+escHtml(String(opt))+'"'+(String(val)===String(opt)?' selected':'')+'>'+escHtml(String(opt))+'</option>';});
        html+='</select>';
      }else{
        html+='<div style="display:flex;gap:12px;flex-wrap:wrap;padding-top:4px">';
        opts.forEach(function(opt){html+='<label style="display:flex;align-items:center;gap:4px;font-size:13px;cursor:pointer"><input type="radio" name="rf-'+f.field_key+'" value="'+escHtml(String(opt))+'" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" '+(fieldCanEdit?'':'disabled')+''+(String(val)===String(opt)?' checked':'')+'> '+escHtml(String(opt))+'</label>';});
        html+='</div>';
      }
    }else if(f.field_type==='multiselect'){
      var mopts=[];
      try{mopts=typeof f.field_options==='string'?JSON.parse(f.field_options):(f.field_options||[]);}catch(e){}
      html+='<div style="display:flex;gap:12px;flex-wrap:wrap;padding-top:4px">';
      mopts.forEach(function(opt){html+='<label style="display:flex;align-items:center;gap:4px;font-size:13px;cursor:pointer"><input type="checkbox" value="'+escHtml(String(opt))+'" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" '+(fieldCanEdit?'':'disabled')+''+(String(val).includes(String(opt))?' checked':'')+'> '+escHtml(String(opt))+'</label>';});
      html+='</div>';
    }else if(f.field_type==='checkbox'){
      html+='<div style="padding-top:4px"><label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer"><input type="checkbox" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" value="1" '+(fieldCanEdit?'':'disabled')+''+(val==='1'?' checked':'')+'> '+escHtml(f.field_label)+'</label></div>';
    }else if(f.field_type==='number'){
      html+='<input type="number" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" data-field-id="'+f.id+'" '+(fieldCanEdit?'':'disabled')+' style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:14px;background:var(--card-bg);color:var(--text)" value="'+escHtml(val)+'" placeholder="'+(f.placeholder||'')+'">';
    }else if(f.field_type==='date'){
      html+='<input type="date" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" data-field-id="'+f.id+'" '+(fieldCanEdit?'':'disabled')+' style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:14px;background:var(--card-bg);color:var(--text)" value="'+escHtml(val)+'">';
    }else{
      html+='<input type="text" data-panel="'+panel+'" data-step="'+seq+'" data-field="'+f.field_key+'" data-field-id="'+f.id+'" '+(fieldCanEdit?'':'disabled')+' style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);font-size:14px;background:var(--card-bg);color:var(--text)" value="'+escHtml(val)+'" placeholder="'+(f.placeholder||'')+'">';
    }
    html+='</div>';
  });
  html+='</div>';
  // 串行模式提示
  if(hasSerial){
    html+='<div style="margin-top:12px;padding:8px 12px;background:#f59e0b10;border:1px solid #f59e0b30;border-radius:8px;font-size:12px;color:var(--text2);display:flex;align-items:center;gap:6px"><span>🔗</span> 串行模式：按顺序填写，前一个字段保存后解锁下一个</div>';
  }
  if(canEdit){
    html+='<div style="text-align:center;margin-top:12px">';
    html+='<button onclick="savePanelData(\''+panel+'\','+seq+')" style="background:var(--primary);color:white;border:none;padding:8px 20px;border-radius:8px;cursor:pointer;font-size:14px">💾 保存</button>';
    html+='</div>';
  }
  html+='</div>';
  return html;
...
async function savePanelData(panel,seq){
  var inputs=document.querySelectorAll('[data-panel="'+panel+'"][data-step="'+seq+'"]');
  var values=[];
  inputs.forEach(function(el){
    if(el.disabled)return; // 跳过被锁定的串行字段
    var val=el.type==='checkbox'?(el.checked?el.value:''):el.value;
    if(val!==undefined){
      values.push({
        project_id:curDetailId,
        step_key:'step_'+seq,
        field_key:el.dataset.field,
        field_value:val,
  ...
  try{
    var {error}=await sb.from('project_step_field_values').upsert(values,{onConflict:'project_id,step_key,field_key'});
...
// ===== 项目详情步骤拖拽重排 =====
var pdDragSrcIdx=-1;
function pdBlockDragStart(e){
  var block=e.target.closest('.pd-block');
  if(!block)return;
  pdDragSrcIdx=Array.from(block.parentElement.children).indexOf(block);
  block.classList.add('drag-src');
  e.dataTransfer.effectAllowed='move';
  e.dataTransfer.setData('text/plain','');
}
function pdBlockDragOver(e){
  e.preventDefault();
  e.dataTransfer.dropEffect='move';
  var block=e.target.closest('.pd-block');
  if(!block)return;
  var all=document.querySelectorAll('#pd-blocks-container .pd-block.drag-over');
  for(var i=0;i<all.length;i++)all[i].classList.remove('drag-over');
  block.classList.add('drag-over');
}
function pdBlockDragEnd(e){
  var all=document.querySelectorAll('#pd-blocks-container .pd-block');
  for(var i=0;i<all.length;i++){all[i].classList.remove('drag-src','drag-over');}
  pdDragSrcIdx=-1;
}
async function pdBlockDrop(e){
  e.preventDefault();
  var dropBlock=e.target.closest('.pd-block');
  if(!dropBlock||pdDragSrcIdx<0)return;
  var container=document.getElementById('pd-blocks-container');
  var blocks=Array.from(container.querySelectorAll('.pd-block'));
  var dropIdx=blocks.indexOf(dropBlock);
  if(dropIdx<0||dropIdx===pdDragSrcIdx)return;
  // 移动 DOM
  var srcBlock=blocks[pdDragSrcIdx];
  if(dropIdx>pdDragSrcIdx){
    container.insertBefore(srcBlock,dropBlock.nextElementSibling);
  }else{
    container.insertBefore(srcBlock,dropBlock);
  }
  pdDragSrcIdx=-1;
  // 更新 curDetailBlocks 顺序
  var newOrder=[];
  var allBlocks=container.querySelectorAll('.pd-block');
  for(var i=0;i<allBlocks.length;i++){
    var key=allBlocks[i].dataset.key;
    var b=curDetailBlocks.find(function(x){return x.key===key});
    if(b)newOrder.push(b);
  }
  curDetailBlocks=newOrder;
  // 写回数据库：更新 project_step_assignees 的 sort_order
  try{
    var upserts=[];
    for(var i=0;i<curDetailBlocks.length;i++){
      upserts.push({project_id:curDetailId,step_key:curDetailBlocks[i].key,sort_order:i});
    }
    var {error}=await sb.from('project_step_assignees').upsert(upserts,{onConflict:'project_id,step_key'});
    if(error)throw error;
    showToast('✅ 步骤顺序已更新');
  }catch(e){showToast('❌ 保存顺序失败: '+e.message);}
}
function pdEditStep(blockKey){
  if(!isAdmin())return;
  var b=curDetailBlocks.find(function(x){return x.key===blockKey});
  if(!b)return;
  // 查找模板步骤定义
  var tplStep=null;
  try{
    if(curDetailProject.template_id&&allTemplates){
      var t=allTemplates.find(function(x){return x.id===curDetailProject.template_id});
      if(t){
        var steps=typeof t.steps==='string'?JSON.parse(t.steps):(t.steps||[]);
        tplStep=steps.find(function(s){return(s.key||'step_')===blockKey});
      }
    }
  }catch(e){}
  // 加载已有覆盖值
  var overrides={};
  sb.from('project_step_assignees').select('*').eq('project_id',curDetailId).eq('step_key',blockKey).maybeSingle().then(function(r){
    if(r.data){ overrides = { label: r.data.step_label || b.label || '' }; }
    showPdEditModal(b,blockKey,overrides,tplStep);
  }).catch(function(){
    showPdEditModal(b,blockKey,{},tplStep);
  });
}
function showPdEditModal(b,blockKey,overrides,tplStep){
  var old=document.querySelector('.pd-edit-overlay');
  if(old)old.remove();
  // 合并当前值
  var label=overrides.label||b.label||'';
  var seq=overrides.seq!=null?overrides.seq:b.seq;
  var phase=overrides.phase||tplStep?.phase||'';
  var panel=overrides.panel||b.panel||tplStep?.panel||'';
  var decision=overrides.decision||b.decision||tplStep?.decision||false;
  var decisionAsk=overrides.decisionAsk||b.decisionAsk||tplStep?.decisionAsk||'';
  var decisionYes=overrides.decisionYes||b.decisionYes||tplStep?.decisionYes||'';
  var decisionNo=overrides.decisionNo||b.decisionNo||tplStep?.decisionNo||'';
  var nextStep=overrides.next_step||'';
  // 构建所有步骤选项供下拉
  var stepOptions='<option value="">默认(顺序下一个)</option>';
  curDetailBlocks.forEach(function(s){
    if(s.key!==blockKey){
      stepOptions+='<option value="'+escHtml(s.key)+'"'+(String(nextStep)===String(s.key)?' selected':'')+'>#'+s.seq+' '+escHtml(s.label)+'</option>';
    }
  });
  var panelOptions='<option value="">无</option>'+
    ['competitor','training','communication','bidding','contract','delivery','payment','editor'].map(function(v){
      return '<option value="'+v+'"'+(panel===v?' selected':'')+'>'+v+'</option>';
    }).join('');
  var overlay=document.createElement('div');overlay.className='pd-edit-overlay';
  overlay.innerHTML='<div class="pd-edit-sheet">'+
    '<h3 style="margin:0 0 12px;font-size:16px">⚙ 编辑步骤属性</h3>'+
    '<label>步骤序号</label><input id="pd-edit-seq" type="number" value="'+seq+'" min="0">'+
    '<label>步骤名称</label><input id="pd-edit-label" value="'+escHtml(label)+'" placeholder="步骤名称">'+
    '<label>所属阶段</label><input id="pd-edit-phase" value="'+escHtml(phase)+'" placeholder="如：线索、商机">'+
    '<label>面板类型</label><select id="pd-edit-panel">'+panelOptions+'</select>'+
    '<label>下一步骤</label><select id="pd-edit-next">'+stepOptions+'</select>'+
    '<div style="margin-top:12px;padding:10px;background:#fff3e0;border-radius:8px">'+
    '<label style="display:flex;align-items:center;gap:6px;margin-top:0"><input type="checkbox" id="pd-edit-decision" '+(decision?'checked':'')+' onchange="var d=this.checked;document.getElementById(\'pd-edit-da\').style.display=d?\'block\':\'none\';document.getElementById(\'pd-edit-dy\').style.display=d?\'block\':\'none\';document.getElementById(\'pd-edit-dn\').style.display=d?\'block\':\'none\'"> 这是判断节点</label>'+
    '<div id="pd-edit-da" style="display:'+(decision?'block':'none')+'"><label>判断问题</label><input id="pd-edit-decision-ask" value="'+escHtml(decisionAsk)+'" placeholder="如：是否通过？"></div>'+
    '<div id="pd-edit-dy" style="display:'+(decision?'block':'none')+'"><label>✓ 是 → 跳转步骤</label><select id="pd-edit-decision-yes">'+stepOptions.replace(/默认/g,'不跳转')+'</select></div>'+
    '<div id="pd-edit-dn" style="display:'+(decision?'block':'none')+'"><label>✗ 否 → 跳转步骤</label><select id="pd-edit-decision-no">'+stepOptions.replace(/默认/g,'不跳转')+'</select></div>'+
    '</div>'+
    '<div class="pd-edit-btns">'+
    '<button onclick="this.closest(\'.pd-edit-overlay\').remove()" style="background:var(--bg2);color:var(--text)">取消</button>'+
    '<button onclick="pdSaveStepEdit(\''+blockKey+'\')" style="background:var(--primary);color:#fff">💾 保存</button>'+
    '</div></div>';
  overlay.addEventListener('click',function(e){if(e.target===overlay)overlay.remove();});
  document.body.appendChild(overlay);
}
async function pdSaveStepEdit(blockKey){
  var b=curDetailBlocks.find(function(x){return x.key===blockKey});
  if(!b)return;
  var seq=parseInt(document.getElementById('pd-edit-seq').value)||0;
  var label=document.getElementById('pd-edit-label').value.trim();
  var phase=document.getElementById('pd-edit-phase').value.trim();
  var panel=document.getElementById('pd-edit-panel').value;
  var nextStep=document.getElementById('pd-edit-next').value;
  var decision=document.getElementById('pd-edit-decision').checked;
  var decisionAsk='',decisionYes='',decisionNo='';
  if(decision){
    decisionAsk=document.getElementById('pd-edit-decision-ask').value.trim();
    decisionYes=document.getElementById('pd-edit-decision-yes').value;
    decisionNo=document.getElementById('pd-edit-decision-no').value;
  }
  var config={
    seq:seq,label:label,phase:phase,panel:panel||null,
    next_step:nextStep||null,
    decision:decision,decisionAsk:decisionAsk||null,decisionYes:decisionYes||null,decisionNo:decisionNo||null
  };
  // 删除 null 值
  Object.keys(config).forEach(function(k){if(config[k]===null||config[k]===undefined)delete config[k];});
  try{
    var {error}=await sb.from('project_step_assignees').upsert({
      project_id:curDetailId,step_key:blockKey,step_label:label,step_config:config,
      updated_at:new Date().toISOString()
    },{onConflict:'project_id,step_key'});
    if(error)throw error;
    // 立即更新 curDetailBlocks
    b.seq=seq;b.label=label;b.phase=phase;b.panel=panel||null;
    b.decision=decision;b.decisionAsk=decisionAsk;b.decisionYes=decisionYes;b.decisionNo=decisionNo;
    b.next_step=nextStep||null;
    document.querySelector('.pd-edit-overlay').remove();
    showToast('✅ 步骤属性已更新');
    renderDetailBlocks();
  }catch(e){showToast('❌ 保存失败: '+e.message);}
}
...
async function saveProjectAsTemplate(){
  if(!curDetailId||!curDetailProject)return;
  if(!isAdmin()){showToast('⚠️ 仅管理员可操作');return;}
  var name=prompt('新模板名称:',curDetailProject.name||'未命名项目');
  ...
  try{
    ...
    if(curDetailProject.template_id&&allTemplates){
      var t=allTemplates.find(function(x){return x.id==curDetailProject.template_id});
      if(t){try{steps=typeof t.steps==='string'?JSON.parse(t.steps):t.steps}catch(e){steps=[]}}
    }
    if(!steps||!steps.length){
      // 从 curDetailBlocks 生成步骤
      curDetailBlocks.forEach(function(b,i){
        steps.push({key:b.key||('step_'+i),label:b.label||('步骤'+(i+1)),panel:b.panel||'editor'});
      });
    }
    // 2. 收集字段配置
    var {data:fields}=await sb.from('project_step_fields').select('*').eq('project_id',curDetailId).order('step_key,sort_order');
    // 合并字段配置到步骤中
    if(fields)fields.forEach(function(f){
      var si=steps.findIndex(function(s){return (s.key||('step_'+steps.indexOf(s)))===f.step_key;});
      if(si===-1)si=parseInt(String(f.step_key).replace('step_',''))||0;
      if(si>=0&&si<steps.length){
        if(!steps[si].fields)steps[si].fields=[];
        steps[si].fields.push({key:f.field_key,label:f.field_label,type:f.field_type,options:f.field_options,required:f.is_required,placeholder:f.placeholder,sort:f.sort_order});
      }
    });
    // 3. 收集审批人配置
    var {data:assignees}=await sb.from('project_step_assignees').select('*').eq('project_id',curDetailId).order('step_key');
    if(assignees)assignees.forEach(function(a){
      var si=parseInt(String(a.step_key).replace('step_',''));
      if(si>=0&&si<steps.length){
        steps[si].default_assignee_type=a.assignee_type||'role';
        steps[si].default_assignee_role=a.assignee_role||'';
        steps[si].default_reviewer_type=a.reviewer_type||'role';
        steps[si].default_reviewer_role=a.reviewer_role||'';
      }
    });
    // 4. 创建新模板
    var body={name:name,description:curDetailProject.name||'',steps:steps,created_at:new Date().toISOString(),updated_at:new Date().toISOString()};
    var {error}=await callAdmin('insert','workflow_templates',{payload:body});
    if(error)throw error;
    showToast('✅ 模板"'+name+'"已创建');
    await loadTemplates();
  }catch(e){showToast('❌ 保存模板失败: '+e.message);}
}

async function syncProjectToTemplate(){
  if(!curDetailId||!curDetailProject)return;
  if(!isAdmin()){showToast('⚠️ 仅管理员可操作');return;}
  var tid=curDetailProject.template_id;
  if(!tid){showToast('⚠️ 该项目未关联模板');return;}
  if(!confirm('确定将当前项目的步骤配置（字段+审批人）同步到原模板？\n这将覆盖模板的字段和审批人配置。'))return;
  try{
    // 1. 读取原模板
    var t=allTemplates.find(function(x){return x.id==tid});
    if(!t){showToast('❌ 原模板不存在');return;}
    var steps=[];
    ...
    if(fields)fields.forEach(function(f){
      ...
      if(si>=0&&si<steps.length){
        if(!steps[si].fields)steps[si].fields=[];
        var existing=steps[si].fields.findIndex(function(ef){return ef.key===f.field_key});
        var fd={key:f.field_key,label:f.field_label,type:f.field_type,options:f.field_options,required:f.is_required,placeholder:f.placeholder,sort:f.sort_order};
        if(existing>=0)steps[si].fields[existing]=fd;
        else steps[si].fields.push(fd);
      }
    });
    // 3. 收集审批人配置
    var {data:assignees}=await sb.from('project_step_assignees').select('*').eq('project_id',curDetailId).order('step_key');
    ...
    // 4. 更新模板
    var body={steps:steps,updated_at:new Date().toISOString()};
    var {error}=await callAdmin('update','workflow_templates',{payload:body,filters:[{col:'id',op:'eq',val:tid}],query:'*'});
    if(error)throw error;
    showToast('✅ 已同步到模板"'+(t.name||'')+'"');
    await loadTemplates();
  }catch(e){showToast('❌ 同步失败: '+e.message);}
}

async function saveProjectFieldAndSteps(projectId){} // placeholder

function getCurrentPerm(){
  if(!curDetailProject)return 'none';
  if(typeof isSuperAdmin!=='undefined'&&isSuperAdmin)return 'admin';
  if(typeof currentUserRole!=='undefined'&&currentUserRole==='admin')return 'admin';
  if(curDetailProject.owner_id===currentUser.id)return 'owner';
  var my=curDetailAssignments.find(function(a){return a.user_id===currentUser.id});
  return my?my.project_role:'none';
}

function canViewBlock(b,perm){
  if(perm==='admin'||perm==='owner')return true;
  if(perm==='none')return false;
  if(!b.visible_to||b.visible_to.length===0)return perm!=='none';
  var myDept=currentUser&&currentUser.department;
  return b.visible_to.some(function(d){
    return d==='all'||d===myDept||d===perm;
  });
}

function canEditBlock(b,perm){
  if(perm==='admin'||perm==='owner')return true;
  if(perm==='edit'||perm==='approve'){
    if(!b.editable_to||b.editable_to.length===0)return true;
    var myDept=currentUser&&currentUser.department;
    return b.editable_to.some(function(d){return d==='all'||d===myDept||d===perm});
  }
  return false;
}
...
async function saveProjectDetail(){
  ...
  var inputs=document.querySelectorAll('.pd-block-input');
  var updates={};
  inputs.forEach(function(inp){
    var b=inp.dataset.block;
    var f=inp.dataset.field;
    if(!updates[b])updates[b]={};
    updates[b][f]=inp.value;
  });
  var merged={};
  curDetailBlocks.forEach(function(b){
    if(updates[b.key]){
      var idx=curDetailBlocks.indexOf(b);
      merged['step_'+idx]=Object.assign({},b.data||{},updates[b.key]);
    }
  });
  var r=await sb.from('projects').update(merged).eq('id',curDetailId);
  if(r.error){alert('保存失败: '+r.error.message);return}
  showToast('已保存','success');
  renderProjectDetail(curDetailId);
}

async function pdAction(action){
  if(!curDetailId)return;
  if(action==='return'){
    if(!confirm('确认退回？'))return;
    await sb.from('projects').update({current_state:'todo'}).eq('id',curDetailId);
    showToast('已退回待办','info');
    closeProjectDetail();
    loadProjects();
    return;
  }else if(action==='complete'){
    if(!confirm('确认办理完毕？此操作将项目状态置为办结'))return;
    await sb.from('projects').update({current_state:'done'}).eq('id',curDetailId);
    showToast('已办结','success');
  }else if(action==='read'){
    var myEmail=currentUser?currentUser.email:'';
    var r=await sb.from('project_block_state').select('*').eq('project_id',curDetailId);
    var alreadyRead=(r.data||[]).some(function(x){return x.notes&&x.notes.indexOf(myEmail)>=0});
    if(!alreadyRead){
      await sb.from('project_block_state').insert({project_id:curDetailId,block_key:'_read_'+Date.now(),status:'done',notes:'已阅 by '+myEmail,completed_at:new Date().toISOString()});
    }
    showToast(alreadyRead?'已记录阅件':'已标记为已阅','success');
  }
  renderProjectDetail(curDetailId);
}
...
function openProjectAssignmentModal(projectId){
  ...
  if(!m){
    ...
    m.style.zIndex='950';
    m.innerHTML='<div class="modal-sheet" style="max-width:760px;max-height:92vh;display:flex;flex-direction:column"><div class="modal-header"><h3>👥 项目成员与审批配置</h3><button class="modal-close" onclick="closeProjectAssignmentModal()" style="position:static">&times;</button></div><div id="pa-tabs" style="display:flex;border-bottom:1px solid var(--border);margin-bottom:12px"><button class="pa-tab active" onclick="switchPATab(\'members\')" data-tab="members">成员分配</button><button class="pa-tab" onclick="switchPATab(\'steps\')" data-tab="steps">步骤审批人</button><button class="pa-tab" onclick="switchPATab(\'fields\')" data-tab="fields">字段配置</button></div><div id="pa-content" style="flex:1;overflow-y:auto"><div id="pa-members-panel"></div><div id="pa-steps-panel" style="display:none"></div><div id="pa-fields-panel" style="display:none"></div></div></div><style>.pa-tab{padding:12px 20px;border:none;background:none;cursor:pointer;font-size:14px;color:var(--text);border-bottom:2px solid transparent}.pa-tab.active{border-bottom-color:var(--primary);color:var(--primary);font-weight:600}.pa-role-tag{display:inline-flex;align-items:center;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:500;margin-left:8px}.pa-user-row{display:flex;align-items:center;padding:12px;border-bottom:1px solid var(--border)}.pa-user-row:hover{background:var(--bg2)}.pa-step-row{padding:16px;border:1px solid var(--border);border-radius:12px;margin-bottom:12px;background:var(--bg)}.pa-step-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.pa-assignee-select{width:100%;padding:10px;border:1px solid var(--border);border-radius:8px;font-size:14px;background:var(--bg);color:var(--text)}.pa-field-row{padding:12px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px;background:var(--bg);display:flex;align-items:center;gap:12px}.pa-field-row input,.pa-field-row select{flex:1;padding:8px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--bg);color:var(--text)}</style>';
...
function closeProjectAssignmentModal(){var m=document.getElementById('project-assignment-modal');if(m)m.classList.add('hidden');}
function switchPATab(tab){document.querySelectorAll('.pa-tab').forEach(function(t){t.classList.remove('active')});document.querySelector('.pa-tab[data-tab="'+tab+'"]').classList.add('active');document.getElementById('pa-members-panel').style.display=tab==='members'?'block':'none';document.getElementById('pa-steps-panel').style.display=tab==='steps'?'block':'none';document.getElementById('pa-fields-panel').style.display=tab==='fields'?'block':'none';if(tab==='fields')loadPAFields(document.getElementById('project-assignment-modal').dataset.projectId);}
...
async function loadPAMembers(projectId){
  ...
  try{
    var {data:assignments}=await sb.from('project_assignments').select('*').eq('project_id',projectId);
    var {data:users}=await sb.from('profiles').select('user_id,display_name,email,department_id').eq('company_id',currentCompanyId);
    var {data:depts}=await sb.from('departments').select('id,name').eq('company_id',currentCompanyId);
    var deptMap={};if(depts)depts.forEach(function(d){deptMap[d.id]=d.name});
    var userMap={};if(users)users.forEach(function(u){userMap[u.user_id]=u});
    var html='<div style="margin-bottom:16px"><h4 style="margin:0 0 12px 0">已分配成员</h4>';
    if(assignments&&assignments.length){
      html+='<div style="border:1px solid var(--border);border-radius:12px;overflow:hidden">';
      assignments.forEach(function(a){
        var u=userMap[a.user_id]||{};
        var role=BUSINESS_ROLES.find(function(r){return r.value===a.business_role})||{label:'未分配',color:'#9ca3af'};
        html+='<div class="pa-user-row"><div style="flex:1"><div style="font-weight:600">'+escHtml(u.display_name||u.email||'未知用户')+'</div><div style="font-size:12px;color:var(--text2)">'+escHtml(u.email||'')+'</div>'+(a.department?'<div style="font-size:12px;color:var(--text2)">'+escHtml(a.department)+'</div>':'')+'</div><span class="pa-role-tag" style="background:'+role.color+'20;color:'+role.color+'">'+role.label+'</span><select onchange="updateMemberRole(\''+projectId+'\',\''+a.user_id+'\',this.value)" style="margin-left:12px;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--bg);color:var(--text)"><option value="">选择角色</option>'+BUSINESS_ROLES.map(function(r){return'<option value="'+r.value+'"'+(a.business_role===r.value?' selected':'')+'>'+r.label+'</option>'}).join('')+'</select><button onclick="removeMember(\''+projectId+'\',\''+a.user_id+'\')" style="margin-left:12px;padding:6px 10px;border:none;border-radius:6px;background:#ef444420;color:#ef4444;cursor:pointer;font-size:13px">移除</button></div>';
      });
      html+='</div>';
    }else{html+='<div style="text-align:center;padding:40px;color:var(--text2)">暂无成员</div>';}
    html+='</div>';
    html+='<div style="border:1px solid var(--border);border-radius:12px;padding:16px;background:var(--bg)"><h4 style="margin:0 0 12px 0">添加成员</h4><div style="display:flex;gap:12px;flex-wrap:wrap"><select id="pa-new-member" style="flex:1;min-width:200px;padding:10px;border:1px solid var(--border);border-radius:8px;font-size:14px;background:var(--bg);color:var(--text)"><option value="">选择用户</option>'+(users?users.filter(function(u){return!(assignments&&assignments.some(function(a){return a.user_id===u.user_id}))}).map(function(u){return'<option value="'+u.user_id+'" data-dept="'+escHtml(deptMap[u.department_id]||'')+'">'+escHtml(u.display_name||u.email)+' ('+escHtml(u.email)+')</option>'}).join(''):'')+'</select><select id="pa-new-role" style="padding:10px;border:1px solid var(--border);border-radius:8px;font-size:14px;background:var(--bg);color:var(--text)"><option value="">选择业务角色</option>'+BUSINESS_ROLES.map(function(r){return'<option value="'+r.value+'">'+r.label+'</option>'}).join('')+'</select><button onclick="addMember(\''+projectId+'\')" style="padding:10px 20px;border:none;border-radius:8px;background:var(--primary);color:#fff;font-size:14px;font-weight:600;cursor:pointer">添加</button></div></div>';
    container.innerHTML=html;
  }catch(e){container.innerHTML='<div style="text-align:center;padding:40px;color:#ef4444">加载失败: '+escHtml(e.message)+'</div>';}
}
...
async function loadPASteps(projectId){
  ...
  container.innerHTML='<div style="text-align:center;padding:40px">加载中...</div>';
  try{
    console.log('[DEBUG] loadPASteps projectId:', projectId);
    var {data:steps, error:stepsError}=await sb.from('project_step_assignees').select('*').eq('project_id',projectId).order('step_key');
    console.log('[DEBUG] steps result:', {steps: steps?.length, error: stepsError?.message});
    console.log('[DEBUG] loadPASteps currentCompanyId:', currentCompanyId, typeof currentCompanyId);
    var {data:depts}=await sb.from('departments').select('id,name').eq('company_id',currentCompanyId).order('name');
    var {data:profiles}=await sb.from('profiles').select('user_id,display_name,email,department_id,company_id').eq('company_id',currentCompanyId);
    console.log('[DEBUG] profiles raw:', profiles);
    console.log('[DEBUG] depts raw:', depts);
    if(!steps||!steps.length){container.innerHTML='<div style="text-align:center;padding:40px;color:var(--text2)">暂无步骤配置 (projectId: '+projectId+', steps: '+(steps?steps.length:'null')+')</div>';return;}
    // 构建部门-人员选项（key统一为字符串，避免类型不匹配）
    var deptUserMap={};
    if(depts)depts.forEach(function(d){deptUserMap[String(d.id)]={name:d.name,users:[]};});
    // 添加"未分配部门"分组
    deptUserMap['__none__']={name:'未分配部门',users:[]};
    console.log('[DEBUG] depts:', depts);
    console.log('[DEBUG] profiles:', profiles?.slice(0,5));
    var unmatchedProfiles=[];
    if(profiles)profiles.forEach(function(p){
      if(p.department_id&&deptUserMap[String(p.department_id)]){
        deptUserMap[String(p.department_id)].users.push(p);
      }else{
        deptUserMap['__none__'].users.push(p);
      }
    });
    console.log('[DEBUG] deptUserMap:', JSON.stringify(Object.keys(deptUserMap).map(function(k){return{k:k,name:deptUserMap[k].name,userCount:deptUserMap[k].users.length};})));
    // 生成按部门分组的人员选项HTML
    var userOptsByDept='';
    Object.keys(deptUserMap).forEach(function(deptId){
      var d=deptUserMap[deptId];
      if(d.users.length>0){
        userOptsByDept+='<optgroup label="'+escHtml(d.name)+'">';
        d.users.forEach(function(u){userOptsByDept+='<option value="'+u.user_id+'">'+escHtml(u.display_name||u.email)+'</option>';});
        userOptsByDept+='</optgroup>';
      }
    });
    var memberOpts='<option value="">选择人员</option>'+userOptsByDept;
    var roleOpts='<option value="">选择角色</option>'+BUSINESS_ROLES.map(function(r){return'<option value="'+r.value+'">'+r.label+'</option>'}).join('');
    var html='<div style="margin-bottom:16px"><h4 style="margin:0 0 8px 0">步骤审批人 + 查阅人配置</h4><p style="margin:0;font-size:13px;color:var(--text2)">每个步骤独立配置<span style="color:#3b82f6">审批人</span>（可编辑）和<span style="color:#10b981">查阅人</span>（只读查看）</p></div>';
    steps.forEach(function(s,i){
      var ar=BUSINESS_ROLES.find(function(r){return r.value===s.assignee_role})||{label:'未设',color:'#9ca3af'};
      var rr=BUSINESS_ROLES.find(function(r){return r.value===s.reviewer_role})||{label:'未设',color:'#9ca3af'};
      var checkedR=s.assignee_type||'role';var checkedRV=s.reviewer_type||'role';
      html+='<div class="pa-step-row">';
      // 步骤标题
      html+='<div class="pa-step-header"><div><span style="font-weight:600">'+(i+1)+'. '+escHtml(s.step_label)+'</span></div><div style="display:flex;gap:6px"><span class="pa-role-tag" style="background:'+ar.color+'20;color:'+ar.color+'">审批:'+ar.label+'</span><span class="pa-role-tag" style="background:'+rr.color+'20;color:'+rr.color+'">查阅:'+rr.label+'</span></div></div>';
      // 审批人区域
      html+='<div style="margin-bottom:12px;padding:10px;border-left:3px solid #3b82f6;border-radius:4px;background:var(--bg2)">';
      html+='<div style="font-size:13px;font-weight:600;color:#3b82f6;margin-bottom:8px">📝 审批人（可编辑提交）</div>';
      html+='<div style="display:flex;gap:12px;align-items:center;margin-bottom:8px"><label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:13px"><input type="radio" name="at-'+s.step_key+'" value="role"'+(checkedR==='role'?' checked':'')+' onchange="updateStepAssigneeType(\''+projectId+'\',\''+s.step_key+'\',\'role\')"> 按角色</label><label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:13px"><input type="radio" name="at-'+s.step_key+'" value="user"'+(checkedR==='user'?' checked':'')+' onchange="updateStepAssigneeType(\''+projectId+'\',\''+s.step_key+'\',\'user\')"> 指定人</label></div>';
      html+='<div id="ar-role-'+s.step_key+'"'+(checkedR==='user'?' style="display:none"':'')+'><select onchange="updateStepRole(\''+projectId+'\',\''+s.step_key+'\',this.value)" class="pa-assignee-select">'+roleOpts.replace(/value="'+s.assignee_role+'"/,'value="'+s.assignee_role+'" selected')+'</select></div>';
      html+='<div id="ar-user-'+s.step_key+'"'+(checkedR==='role'?' style="display:none"':'')+'><select onchange="updateStepUser(\''+projectId+'\',\''+s.step_key+'\',this.value)" class="pa-assignee-select">'+memberOpts.replace(/value="'+s.assignee_user_id+'"/,'value="'+s.assignee_user_id+'" selected')+'</select></div>';
      html+='</div>';
      // 查阅人区域
      html+='<div style="padding:10px;border-left:3px solid #10b981;border-radius:4px;background:var(--bg2)">';
      html+='<div style="font-size:13px;font-weight:600;color:#10b981;margin-bottom:8px">👁 查阅人（只读查看，不可编辑）</div>';
      html+='<div style="display:flex;gap:12px;align-items:center;margin-bottom:8px"><label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:13px"><input type="radio" name="rt-'+s.step_key+'" value="role"'+(checkedRV==='role'?' checked':'')+' onchange="updateStepReviewerType(\''+projectId+'\',\''+s.step_key+'\',\'role\')"> 按角色</label><label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:13px"><input type="radio" name="rt-'+s.step_key+'" value="user"'+(checkedRV==='user'?' checked':'')+' onchange="updateStepReviewerType(\''+projectId+'\',\''+s.step_key+'\',\'user\')"> 指定人</label></div>';
      html+='<div id="rr-role-'+s.step_key+'"'+(checkedRV==='user'?' style="display:none"':'')+'><select onchange="updateStepReviewerRole(\''+projectId+'\',\''+s.step_key+'\',this.value)" class="pa-assignee-select">'+roleOpts.replace(/value="'+s.reviewer_role+'"/,'value="'+s.reviewer_role+'" selected')+'</select></div>';
      html+='<div id="rr-user-'+s.step_key+'"'+(checkedRV==='role'?' style="display:none"':'')+'><select onchange="updateStepReviewerUser(\''+projectId+'\',\''+s.step_key+'\',this.value)" class="pa-assignee-select">'+memberOpts.replace(/value="'+s.reviewer_user_id+'"/,'value="'+s.reviewer_user_id+'" selected')+'</select></div>';
      html+='</div>';
      html+='</div>';
    });
    container.innerHTML=html;
...
async function updateMemberRole(projectId,userId,role){
  try{var {error}=await sb.from('project_assignments').update({business_role:role||null}).eq('project_id',projectId).eq('user_id',userId);if(error)throw error;showToast('✅ 角色已更新');loadPAMembers(projectId);loadPASteps(projectId);}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function removeMember(projectId,userId){
  if(!confirm('确定移除该成员？'))return;
  try{var {error}=await sb.from('project_assignments').delete().eq('project_id',projectId).eq('user_id',userId);if(error)throw error;showToast('✅ 成员已移除');loadPAMembers(projectId);loadPASteps(projectId);}catch(e){showToast('❌ 移除失败: '+e.message);}
}
async function addMember(projectId){
  var userSelect=document.getElementById('pa-new-member');var roleSelect=document.getElementById('pa-new-role');var userId=userSelect.value;var role=roleSelect.value;if(!userId){showToast('请选择用户');return;}
  try{var {error}=await sb.from('project_assignments').insert({project_id:projectId,user_id:userId,business_role:role||null,project_role:'edit'});if(error)throw error;showToast('✅ 成员已添加');userSelect.value='';roleSelect.value='';loadPAMembers(projectId);loadPASteps(projectId);}catch(e){showToast('❌ 添加失败: '+e.message);}
}
async function updateStepAssigneeType(projectId,stepKey,type){
  document.getElementById('ar-role-'+stepKey).style.display=type==='role'?'block':'none';
  document.getElementById('ar-user-'+stepKey).style.display=type==='user'?'block':'none';
  try{var {error}=await sb.from('project_step_assignees').update({assignee_type:type,updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function updateStepRole(projectId,stepKey,role){
  try{var {error}=await sb.from('project_step_assignees').update({assignee_role:role,assignee_user_id:null,updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;showToast('✅ 审批角色已更新');}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function updateStepUser(projectId,stepKey,userId){
  try{var {data:profile}=await sb.from('profiles').select('email,display_name').eq('user_id',userId).single();var {error}=await sb.from('project_step_assignees').update({assignee_user_id:userId,assignee_user_email:profile?.email||'',updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;showToast('✅ 审批人已更新');}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function updateStepReviewerType(projectId,stepKey,type){
  document.getElementById('rr-role-'+stepKey).style.display=type==='role'?'block':'none';
  document.getElementById('rr-user-'+stepKey).style.display=type==='user'?'block':'none';
  try{var {error}=await sb.from('project_step_assignees').update({reviewer_type:type,updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function updateStepReviewerRole(projectId,stepKey,role){
  try{var {error}=await sb.from('project_step_assignees').update({reviewer_role:role,reviewer_user_id:null,updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;showToast('✅ 查阅角色已更新');}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function updateStepReviewerUser(projectId,stepKey,userId){
  try{var {data:profile}=await sb.from('profiles').select('email,display_name').eq('user_id',userId).single();var {error}=await sb.from('project_step_assignees').update({reviewer_user_id:userId,reviewer_user_email:profile?.email||'',updated_at:new Date().toISOString()}).eq('project_id',projectId).eq('step_key',stepKey);if(error)throw error;showToast('✅ 查阅人已更新');}catch(e){showToast('❌ 更新失败: '+e.message);}
}

// ========== 字段配置功能 ==========
var FIELD_TYPES=[{value:'text',label:'文本'},{value:'textarea',label:'多行文本'},{value:'number',label:'数字'},{value:'date',label:'日期'},{value:'select',label:'下拉选择'},{value:'multiselect',label:'多选'},{value:'checkbox',label:'复选框'},{value:'radio',label:'单选'}];
async function loadPAFields(projectId){
  var container=document.getElementById('pa-fields-panel');
  container.innerHTML='<div style="text-align:center;padding:40px">加载中...</div>';
  try{
    var {data:steps}=await sb.from('project_step_assignees').select('step_key,step_label').eq('project_id',projectId).order('step_key');
    var {data:fields}=await sb.from('project_step_fields').select('*').eq('project_id',projectId).order('step_key,sort_order');
    if(!steps||!steps.length){container.innerHTML='<div style="text-align:center;padding:40px;color:var(--text2)">暂无步骤，请先创建项目</div>';return;}
    var html='<div style="margin-bottom:16px"><h4 style="margin:0 0 8px 0">步骤字段配置</h4><p style="margin:0;font-size:13px;color:var(--text2)">为每个步骤自定义表单字段</p></div>';
    steps.forEach(function(s){
      var stepFields=fields?fields.filter(function(f){return f.step_key===s.step_key}):[];
      html+='<div class="pa-step-row"><div class="pa-step-header"><div><span style="font-weight:600">'+escHtml(s.step_label)+'</span></div><button onclick="addField(\''+projectId+'\',\''+s.step_key+'\')" style="padding:6px 12px;background:var(--primary);color:#fff;border:none;border-radius:6px;font-size:13px;cursor:pointer">+ 添加字段</button></div>';
      if(stepFields.length){
        stepFields.forEach(function(f){
          var serialChecked=f.collaboration_mode==='serial'?' checked':'';
          html+='<div class="pa-field-row"><input type="text" value="'+escHtml(f.field_label)+'" onchange="updateFieldLabel('+f.id+',this.value)" placeholder="字段名称"><select onchange="updateFieldType('+f.id+',this.value)">'+FIELD_TYPES.map(function(t){return'<option value="'+t.value+'"'+(f.field_type===t.value?' selected':'')+'>'+t.label+'</option>'}).join('')+'</select><label style="display:flex;align-items:center;gap:4px;font-size:13px;cursor:pointer"><input type="checkbox"'+(f.is_required?' checked':'')+' onchange="updateFieldRequired('+f.id+',this.checked)"> 必填</label><select onchange="updateFieldCollabMode('+f.id+',this.value)" style="padding:4px 8px;border:1px solid var(--border);border-radius:4px;font-size:12px;background:var(--bg);color:var(--text)"><option value="parallel"'+(''+(f.collaboration_mode||'parallel')==='parallel'?' selected':'')+'>并行</option><option value="serial"'+(''+(f.collaboration_mode||'parallel')==='serial'?' selected':'')+'>串行</option></select><button onclick="deleteStepField('+f.id+')" style="padding:4px 8px;background:#ef4444;color:#fff;border:none;border-radius:4px;font-size:12px;cursor:pointer">删除</button></div>';
        });
      }else{
        html+='<div style="padding:20px;text-align:center;color:var(--text2);font-size:13px">暂无字段，点击上方按钮添加</div>';
      }
      html+='</div>';
    });
    container.innerHTML=html;
...
async function addField(projectId,stepKey){
  if(!isAdmin()){showToast('⚠️ 仅管理员可编辑字段配置');return;}
  try{var {error}=await sb.from('project_step_fields').insert({project_id:projectId,step_key:stepKey,field_key:'field_'+Date.now(),field_label:'新字段',field_type:'text',sort_order:99,collaboration_mode:'parallel'});if(error)throw error;showToast('✅ 字段已添加');loadPAFields(projectId);}catch(e){showToast('❌ 添加失败: '+e.message);}
}
async function updateFieldLabel(fieldId,label){
  if(!isAdmin())return;
  try{var {error}=await sb.from('project_step_fields').update({field_label:label}).eq('id',fieldId);if(error)throw error;}catch(e){}
}
async function updateFieldType(fieldId,type){
  if(!isAdmin())return;
  try{var {error}=await sb.from('project_step_fields').update({field_type:type}).eq('id',fieldId);if(error)throw error;}catch(e){}
}
async function updateFieldRequired(fieldId,required){
  if(!isAdmin())return;
  try{var {error}=await sb.from('project_step_fields').update({is_required:required}).eq('id',fieldId);if(error)throw error;}catch(e){}
}
async function updateFieldCollabMode(fieldId,mode){
  if(!isAdmin())return;
  try{var {error}=await sb.from('project_step_fields').update({collaboration_mode:mode}).eq('id',fieldId);if(error)throw error;showToast('✅ 已切换为'+(mode==='serial'?'串行':'并行')+'模式');}catch(e){showToast('❌ 更新失败: '+e.message);}
}
async function deleteStepField(fieldId){
  if(!isAdmin()){showToast('⚠️ 仅管理员可删除字段');return;}
  if(!confirm('确定删除此字段？'))return;
  try{var {error}=await sb.from('project_step_fields').delete().eq('id',fieldId);if(error)throw error;showToast('✅ 字段已删除');var projectId=document.getElementById('project-assignment-modal').dataset.projectId;loadPAFields(projectId);}catch(e){showToast('❌ 删除失败: '+e.message);}
}
function isAdmin(){
