...
async function loadFollowups(){
  ...
  for(var i=0;i<data.length;i++){
    ...
    h+='<div class="schedule-item" style="border-left-color:#8B5CF6"><div class="sch-title"><span style="background:#EDE9FE;color:#7C3AED;padding:1px 6px;border-radius:4px;font-size:11px">'+h(f.type||'其他')+'</span> '+(cl2?h(cl2.name):'未知客户')+'</div><div style="font-size:13px;margin:4px 0">'+h(f.content||'')+'</div>'+(f.outcome?'<div style="font-size:12px;color:#10B981">'+h(f.outcome)+'</div>':'')+(f.next_step?'<div class="sch-meta">下一步: '+h(f.next_step)+(f.next_date?' ('+f.next_date+')':'')+'</div>':'')+'<div class="sch-meta">'+new Date(f.created_at).toLocaleString('zh-CN')+'<button class="btn-sm" style="font-size:10px;padding:1px 6px;color:var(--danger);margin-left:8px" onclick="deleteEngagement('+escHtml(String(f.id))+')">删除</button></div></div>';
  }
  list.innerHTML=h;
}
