// mingluji 广东外贸公司爬虫 - 多城市 + 目标数量
// 用法: node crawl_mingluji.js
...
// 广东21城市
const CITIES = [
  '广州', '深圳', '东莞', '佛山', '惠州', '湛江', '中山', '珠海',
  '汕头', '清远', '江门', '梅州', '肇庆', '茂名', '河源', '韶关',
  '汕尾', '阳江', '揭阳', '潮州', '云浮'
];
...
for (const city of CITIES) {
  if (all.length >= TARGET) { console.log(`\n🎉 已达 ${TARGET} 家！`); break; }
  console.log(`\n=== ${city} ===`);
  ...
  for (let page = 1; page <= MAX_PAGES_PER_CITY; page++) {
    ...
    console.log(`    -> ${companies.length} 条，+${added}，累计 ${all.length}/${TARGET}`);

    if (companies.length === 0) break;
    if (all.length >= TARGET) break;
    execSync('powershell -Command "Start-Sleep -Seconds 2"', { encoding: 'utf8', stdio: 'ignore' });
  }
